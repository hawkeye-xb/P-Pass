...
- [ ] **ARCH-01 首批 P0 实施卡** — **2026-08-31 self-review 通过**：`ARCH-02`（D-01~D-04 账本/发现原子提交，现可接）→ `ARCH-03`（C-01~C-05 严格消费者）→ `ARCH-04`（E-01~E-04 完成凭据/范围）→ `ARCH-05`（X-01~X-05 取消本轮）。每卡先写新失败用例；后续卡等待前置，不重开 ARCH-01 产品语义。
