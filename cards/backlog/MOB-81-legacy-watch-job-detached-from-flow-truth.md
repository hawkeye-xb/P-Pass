...
- 2026-09-15：HarmonyOS 4.2 真机复现（详见 MOB-75 同日记录）：系统主动
  弹出「后台异常」提示，用户点击「立即修复」并显示修复完成，但后台唤醒
  实际仍未恢复。这是本卡推断的真机证据——该"修复"更可能只重排了 legacy
  JobScheduler，并未触达真正的 Flow 主链路，导致用户获得**虚假的"已修复"
  反馈**。这比原描述的"只是提示不准"更严重：用户会误以为问题已解决而
  不再追查真实的后台备份失效。建议下次拍板时把这条实证纳入考量，评估
  是否该从 backlog 提级回可接队列。
