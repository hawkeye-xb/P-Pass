...
手动发起备份遇到坏记录时与自动链路同样健壮：跳过坏条目、其余照传。⚠️
实际修法被用户改了方向（见实施记录）：不是照搬 MOB-09 的错误隔离，而是
**把第二条备份管线删掉**——手动只是又一种触发方式，天然复用自动链路的
全部防护。

## 验收标准

- [x] 单测：手动链路构造 1 个坏候选 + 若干正常候选 → 正常的全部进入
  offered，整体不抛异常（2026-08-20：`OneBackupPipelineTest` 11 个，
  全量 247/247 绿）
- [x] **反证**：去掉隔离 → 测试必须变红，真跑不凭"应该会红"下结论
  （2026-08-20 反证 27 条全红，明细见实施记录）
- [x] 源码级断言先剥注释行再判断（MOB-09 踩过假绿：把生产代码注释掉，
  `src.contains(...)` 照样通过；参考 `BadMediaRecordTest` 里加严版的
  `codeOf()`——它连 KDoc/块注释一起剥）
- [ ] 真机：备份进行中点「暂停」→ 停住且不 commit；再点「再试一次」→
  续传（挂用户验收；这两个触点都要先制造一次备份/失败状态才能出现）

## 范围

- 只准动：原卡写死 `BackupUiStateHolder.kt` 及其单测；用户定稿改方向后
  实动 `BackupWorker.kt`（统一入口）+ `BackupUiStateHolder.kt`（删第二
  管线）+ 相关测试，见实施记录。
- 不准动：不改 `BackupWorker.kt` 里 MOB-09 刚落地的 `buildCandidates`
  （复用它，不改它）。

## 阻塞与依赖

真机验收挂用户。「再试一次」/「暂停」两个触点需先制造备份/失败状态。
`manual_backup_entry` 死文案的去留待用户拍板（不在本卡验收内）。
...
原卡收尾要求：Android 单测全绿 + PROGRESS.md 一行 + 本卡移入 `done/`。
MOB-09 的四条决策原供照搬参考（坏记录只打日志不发通知；部分跳过水位照常
推进；整批读不了不 commit 不推进水位；候选构建里要有探针
`open().use { }`）——实际修法改为删除第二管线后，这些防护由统一管线
天然继承。
