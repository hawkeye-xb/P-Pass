...
库内移动被识别为「同一份内容换了住址」：索引行的 `rel_path` 就地改指用户
摆的新位置，行不删、文件不动，照片墙/时间线无感。

## 验收标准

- [ ] 真机：Finder 里把已备份照片拖进自建目录 → 照片墙/时间线仍可见，文件停在用户放的位置
- [x] `just ci` 全绿（合并时证据，见实施记录）
- [x] 反证有效：修复改回去，对应测试变红（见实施记录）

## 范围

- 只准动：`crates/core-index/src/ingest.rs`、`crates/storage`（`update_asset_rel_path`）、`crates/daemon`（端到端测试）
- 不准动：watcher 的对账方向逻辑（WATCH-02）、canonical 落位规则本身

## 阻塞与依赖

无。修 WATCH-02 时顺手挖出来的。
...
改动：`crates/core-index/src/ingest.rs` —— hash 命中时**先看记录的文件还在不在**：
...
这是产品决定，不是 bug，没动。（注：此遗留问题已由 WATCH-04 的宽容入库
裁决覆盖。）
