...
| **MOB-62（L2）断开重扫清空旧 Flow 会话** | 2026-09-08 | 本 commit | 🟡 代码完成，待真机 | 三星实证主动断开→重扫会 ANR；清理路径原先只删 pairing/confirmed cache 和一个 periodic work，遗留 `flow-state/<NodeId>`、native provider/runtime 及 catchup/manual/process/media-watch wakes，重扫同 NodeId 时旧 offer 重放。现已关闭/移除 runtime、删旧 Flow ledger、取消全部 wake 与 media watch，并把清理移到 IO；Android JVM + debug APK、`just ci` 通过，待三星回归。 |
