...
- [x] 找到当前 Flow 失败/永久失败回调链路的正确接线点（不是重新发明一套
      通知基础设施，复用已有的 Notification Channel 若存在，否则新建）。
- [x] JVM：`NotifyOnFailurePrefs.enabled() == true` 且触发一次失败 →
      通知发送被调用；`enabled() == false` 时不调用。
- [x] 反证：把开关判断去掉，上一条用例必须变红（或反过来验证关闭确实
      拦住发送）。
- [x] 真机：开启开关触发一次真实失败，收到系统通知；关闭开关触发失败，
      不收到通知。
...
- 当前 main debug APK（versionCode 21）在三星 SM-S9210 上，设置开关打开、
  临时授予 `POST_NOTIFICATIONS` 后，停止当前 daemon 并通过生产的「备份哪些相册
  → 开始备份」用户在场 wake 制造三次真实失败：attempt 1/2 保持 `QUEUED`，
  attempt 3 落为 `FAILED_NEEDS_USER`。系统通知栏出现固定 id **2027** 的 P-Pass
  通知：「照片备份没成功」/「1张照片没备份成功，打开看看」。
- 手势撤掉该通知、将设置开关关闭；对同一项走「再试一次」后再跑三次相同的生产
  失败路径，仍在 attempt 3 落 `FAILED_NEEDS_USER`，但通知栏无 P-Pass 失败通知。
  这证明开关只抑制通知，未抑制账本终态。
- 恢复当前 main daemon 后用「再试一次」使受控项 `CONFIRMED`；仅删除明确命名的
  手机测试源。相册范围恢复为 1 个、后台备份关闭、失败通知开关关闭；临时授予的
  `POST_NOTIFICATIONS` 和 device-idle 白名单均已撤回。桌面库的具名确认副本保留
  为验收证据。
