...
/// Official Ed25519 public key (32 bytes) — the release signing key.
///
/// UPD-01 (2026-08-04): real keypair generated by the user via
/// `tauri signer generate` (minisign/rsign). Minisign public-key layout
/// is `sig_alg(2B "Ed") || key_id(8B) || public_key(32B)` — the ed25519
/// key sits at offset 10..42, NOT 2..34 (offset 2..34 mixes key_id with
/// the key and fails point decompression; verified via PyNaCl +
/// openssl + blake2b-512 prehashed signature check).
/// daemon 自更新搁置期间此 key 仍须存在且可解析（客户端构建期校验）。
/// 注意：tauri 签名是 minisign hashed 模式（ED = blake2b-512 预哈希），
/// 与 update.rs 的直接 ed25519 verify_strict 不兼容——daemon 自更新恢复
/// 时需改 minisign 验证或独立签名（挂账）。
pub const OFFICIAL_PUBLIC_KEY: &[u8] = &[
    0x1a, 0x05, 0x51, 0x9a, 0x5c, 0xd9, 0xf3, 0x2f, 0x57, 0x49, 0x4a, 0x1e, 0x2a, 0x3a, 0x07, 0xc5,
    0x17, 0x08, 0xa8, 0x4d, 0xf0, 0x32, 0x78, 0xbf, 0x30, 0x9e, 0xe3, 0x8f, 0x79, 0xcc, 0xfa, 0xe9,
];
...
mod tests {
    ...
    #[test]
    fn official_public_key_parses_as_verifying_key() {
        // UPD-01: real keypair (2026-08-04, tauri signer minisign format,
        // 2-byte "Ed" magic stripped). A key that fails to parse would
        // silently disable signature verification in every client.
        assert_eq!(OFFICIAL_PUBLIC_KEY.len(), 32);
        let arr: &[u8; 32] = OFFICIAL_PUBLIC_KEY.try_into().unwrap();
        match ed25519_dalek::VerifyingKey::from_bytes(arr) {
            Ok(_) => {}
            Err(e) => panic!("official key rejected: {e:?}"),
        }
        verifying_key(OFFICIAL_PUBLIC_KEY).expect("official key must parse as VerifyingKey");
    }

    #[test]
    fn official_key_rejects_tampered_artifact() {
        // UPD-01 反证：真实密钥对下，篡改一字节的工件必须验签失败。
        use ed25519_dalek::{Signature, Signer as _, Verifier as _};
        let signing = SigningKey::from_bytes(&[7u8; 32]);
        let vk = signing.verifying_key();
        let bytes = b"official-key-tamper-check";
        let mut tampered = bytes.to_vec();
        tampered[3] ^= 1;
        let sig = Signature::from_bytes(&signing.sign(bytes).to_bytes());
        assert!(
