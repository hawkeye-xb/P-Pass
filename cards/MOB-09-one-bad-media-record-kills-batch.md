...
MediaStore 里存在「有记录但实体文件打不开」的条目时（MOB-08 排查 2026-08-18
实测撞到：用 `adb shell content insert` 造了几条 `_size=NULL` 的空记录后），
此后每一轮自动备份都是：
...
`doWork()` 里 `scan.items.map { … hashWithCache(…) }` 这一步，任何一条打不开
的记录都会让异常冒泡到外层 `catch (t: Throwable)`，**整批**记为失败，走短
退避重试，重试仍然撞同一条坏记录。watermark 不推进，于是**这一条坏记录会
永久卡住这台设备的所有后续备份**。
...
MediaStore 行未同步清理、云相册/占位文件、外部存储卸载、第三方 App 写坏的
记录。用户看到的现象会是「备份莫名其妙从某天起再也不动了」，且没有任何界面
提示（失败通知只在放弃本轮时发一次）。

**来源**：MOB-08 排查过程中实测撞到（2026-08-18）。不属于 MOB-08 范围，按
`docs/AGENT_PROTOCOL.md` §C.2 另开。

## 期望行为

MediaStore 里存在「有记录但实体文件打不开」的条目时，备份跳过这些条目并把
其余照片正常传完，而不是整批失败进重试。

## 验收标准

- [x] 单测：构造一个 `open` 抛 `FileNotFoundException` 的候选 + 若干正常
  候选 → 正常候选全部进入 offered，坏候选被跳过，整体不抛异常
  （`BadMediaRecordTest.kt` 6 个测试，2026-08-19 全绿）
- [x] **反证**：把「跳过坏项」的逻辑去掉 → 该测试必须变红
  （2026-08-19 实测 4 个行为测试全红，见实施记录）
- [ ] 真机：用 `adb shell content insert`（不带实体文件）造 1 条坏记录 +
  `adb push` 真实 jpg 造 1 条好记录 → 触发一轮自动备份 → logcat 出现
  `auto backup: offered=… pushed=…`（好记录传到），且**没有** ENOENT 导致的
  RETRY/FAILURE，另有 `skipped 1/2 unreadable media record(s)`
- [x] 坏记录至少打一条 `PPassBackup` 日志记录被跳过的条目数，不静默吞掉
  （部分验过：2026-08-20 真机用 `content insert` 造探针记录实测到
  `W PPassBackup: auto backup: skipped 1/1 unreadable media record(s)`
  且无 ENOENT 导致的 RETRY/FAILURE；坏+好同批的对照 `skipped 1/2` 未做）
...
真机验收未做完（坏记录与好记录同批的对照 `skipped 1/2` 未验）。无其它前置。
...
- **真机验收未做完**（无设备接入）：`adb shell content insert` 造坏记录 +
...
MediaScanner，未动它。（已另开 MOB-19。）

## 备注

- 2026-08-20 复核：真机侧**部分完成**——用 `content insert` 造探针记录时
  实测到 `W PPassBackup: auto backup: skipped 1/1 unreadable media record(s)`
  且无 ENOENT 导致的 RETRY/FAILURE。**未做**：坏记录与好记录同批的对照
  （卡面原验收要的是 `skipped 1/2`）。
- 原卡收尾要求：Android 单测全绿 + PROGRESS.md 一行 + NEXT.md 状态更新 +
  本卡移入 `done/`。
