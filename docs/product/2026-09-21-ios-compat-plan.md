...
**本轮范围（2026-09-21 用户定调）：只做前台备份。** 后台自动备份不在本轮交付里，方案另议（见第三之二节）。这不是能力上限的结论，是先后顺序的选择——前台这条路语义与 Android 完全同源，不依赖任何未验证前提。
...
## 三之二、后台转发方案的开放设计问题（未决，不在本轮范围）

若第 2 问答案为否（同网段也打不通），iOS 的后台备份就只剩「手机把字节交给一个公网 HTTP 端点，再由它送到桌面」。

**2026-09-21 用户提出的形态：不落存储，Worker 收到即直转桌面。** 这比先前假设的 store-and-forward spool 好得多，两条原顾虑基本消解：
...
| **Worker 直转** | **看得到** | Worker + 桌面持久外连 | 覆盖率最高，但隐私表达必须改口 |

### 选 Worker 直转的话，还剩四个必须先答
...
**桌面不在线怎么办**：这条通道下 job 会失败，而失败只允许重试一次（`retry(destination:)`）。之后只能等扩展下一次 `process()` 重新登记 —— 粒度约一天。所以「桌面关机三天」= 备份滞后三天，不是丢失。这一点要写进用户可见的表达里。
