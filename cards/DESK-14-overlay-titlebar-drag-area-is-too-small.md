...
> 🟡 状态：待拍板
> 级别：L3 · 阻塞：拖拽区必须保持透明 Overlay 的连续底色，不能以有色横带或布局分隔换取面积
> 当前节点：首次「独立顶部带」方案已撤回，源码已还原；下一步：先确定不产生色差的拖拽交互方案，再编码。
...
首次方案已撤回：专用 40px 顶部带会破坏透明 Overlay 的连续视觉（形成明显色差/横向分隔），不符合产品设计。所有未提交的 UI、拖拽逻辑与测试改动均已还原，源码零差异；待确定保持透明底色的方案后再实施。
