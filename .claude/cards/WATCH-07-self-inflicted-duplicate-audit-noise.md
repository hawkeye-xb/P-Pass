...
- [ ] 先证明这些 `ingest.duplicate` 行是谁写的：查审计行的 actor 字段 + 复现（确认是监听而不是备份管线）
...
- 只准动：（未记录——实施前根据选定的改法划定）
- 不准动：watcher「事件 = 去看这个路径，磁盘才是真相」的铁律语义（除非改法 1 被拍板且做了充分论证）

## 阻塞与依赖

无。候选改法实施前需拍板（见下，倾向方案 2）。
