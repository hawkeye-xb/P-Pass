...
## 设计纪律：禁止「最小能跑通」（2026-09-14 验收人定调）

> 事故背景：`flow.fetch` 把「提交任务」和「等结果」焊死在一次同步 RPC 上，
> LAN 时代不显形，跨 relay 后传一半必超时；随后的 UX-11（加 15s 保险丝）、
> MOB-54（补 wake）、UX-14/15（改呈现）全是绕着错误形状打的补丁，没人回头
> 修形状本身。验收人原话：「每次都是妥协导致问题」。本节的目的是让"妥协"
> 在方案阶段就过不去，而不是在事故复盘时才看见。

1. **根治方案是默认方案。** 提出方案菜单时每项必须标注「是否终态」。止血
   （调数字/加兜底/绕症状）只有在根治被外部条件（真机窗口、拍板依赖）卡住
   时才可以作为**过渡**交付，且必须同时：卡面上写明「终态方案是 X、本改动
   是过渡、X 合入后评估是否回退」，并把 X 开成卡进 QUEUE。只交止血不开
   根治卡 = 违规。
2. **先找标准答案，没有再发明。** 协议/状态机/交互形状先对照两类现成物：
   业界成熟方案（如长任务 = HTTP 202+任务句柄+轮询，推送为加速）与本仓已有
   范式（`timeline.subscribe`、幂等 receipt）。偏离标准答案要在卡内写一条
   **可证伪**的理由（"标准做法是 A，我们选 B 因为 C，C 由 <实验/数据> 证伪
   就切回 A"）。"那样简单/改动小"不构成理由。
3. **症状型补丁不许静默落地。** 修"看起来不对"之前必须先回答：底层形状是对
   的吗？若补丁掩盖的是形状问题（超时值不对、状态不显示、事件不唤醒），补丁
   可以打，但**同一次提交**必须附根因卡或把根因写进本卡"阻塞与依赖"并挂
   QUEUE——"先这样吧"不许过夜。
4. **库能力先行核查。** 设计与库（iroh、tauri、WorkManager…）交互的机制前，
   先查库自带的事件/进度/状态接口并在卡内列明"自带却未用的能力"，不得默认
   "库是哑巴"。iroh 类传输事实（连接路径、字节进度）以库事件为唯一来源，
   禁止用定时器/猜测冒充（NET-05「不准动」节的口径推广为全仓口径）。
...
### 批次并行与验收节奏（2026-09-01 用户定调）

独立卡不因等待上一张的成功 CI 而串行：按文件/域无交集拆入并行 worktree，
各自完成本地 RED→GREEN 后合入一个批次。CI 仍是每次 push 的硬门，但**成功
不要求验收人逐张转报或人工盯住**；agent 继续下一张独立卡。只在以下情况打断
验收人：产品/架构决定、真机/真实环境验收、同一批次 CI 失败或跨卡冲突。

每个批次完成时再汇总：包含的卡、最终 commit、受影响域 CI、真机欠账。不要把
24 张可接卡伪装成 24 次人类审批。
...
- 设计基准：`docs/design/2026-08-17-layout-v3/` 是当前 UI 工程基准；
  v1/v2 已过期，仅供历史追溯，不再对照。新一轮设计稿迭代必须先归档
  成新目录（`docs/design/YYYY-MM-DD-*/`）并把本条指向切过去，旧目录
  标记过期保留，不允许外部活设计稿绕过归档直接进实现（归档规则详见
  v3 目录 README 的"更新规则"）。UI 卡验收必须含双端全页面走查
  （还原/走样/未实现三定性 + 截图）。
