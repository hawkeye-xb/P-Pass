...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **DEV-03（L1）清理工具漏报一半，而它的测试从来不会失败** | 2026-09-17 | `9da51b6b` | ✅ 代码完成，待归档 | 三个洞叠加：测试 18 条裸 `[[ ]]` 断言在 macOS bash 3.2 下不触发 errexit（与 QA-02 同型，是另两个能潜伏的原因）；target 发现写死 `$worktree/target` 漏掉嵌套的 2.6G；「已合并」判据用 ancestry，而本仓全走 squash 合并故永远为假。修法：断言改 `\|\| fail`、夹具统一 `pwd -P` 路径并补 Cargo.toml、target 发现改「名为 target 且（同级有 Cargo.toml 或内含 CACHEDIR.TAG）」、「已合并」加上游已删信号。E2 证据：修复中该测试连续 4 次 `ASSERT FAILED` 才转绿，改前对这四种错误全部报 ok。实测清理 22G→1.9G。 |
| **SITE-04（L1）site lane 红了一周没人管** | 2026-09-17 | `22625980` | ✅ 全部验收闭环，待归档 | `5abb8acb` 改 tokens.json 只动文案未重生成，site 自 09-11 连红 6 天，后四步（icons/build/零第三方/upload）整整 6 天没执行。根 `token-check` 没坏——它按设计只比数值，文案漂移不在覆盖范围；而 site 生成器把文案嵌进产物。修法一行生成物 + `just site-check`。被卡死的四步逐个本地补跑后才提交。合入后 main 的 site run success。 |
| **SEC-02（L1）提交身份白名单门禁** | 2026-09-17 | `b19d6c1e` | ✅ 全部验收闭环，待归档 | 仓库对「谁能提交」零约束，越界身份进 main 时无任何报警。白名单而非黑名单（黑名单要把禁止的字符串写进仓库）。新 lane `ci-identity` 不加 paths 全量触发、`fetch-depth: 0`（浅克隆失败方向是「查不到=放行」）、先跑反证再跑门禁。六变异反证含 D/E 两条专证 name 与 email 各自都在管。E2：对历史 6 个真实越界提交逐条命中共 12 条；全史 1065 提交不误伤。PR #70 上 CI Identity success。 |
| **SEC-01（L1）全史身份改写 + 账本修复** | 2026-09-17 | `7cbad733` | ⏸ 暂缓（残留项明确接受） | 6 个提交带非白名单身份，`filter-repo --mailmap` 全史改写并强推 8 分支 9 tag；1081 提交逐个比对 tree 哈希 1081/1081 一致（只改元数据）。远端受影响 ref 实测 16→6。文档 335 处失效 SHA 按 commit-map 重映射，176 个新 SHA 全可解析。**残留**：6 个只读 `refs/pull/*` PR ref 推不动，2026-09-17 拍板不提 GitHub 工单——代价是 60 个 PR 的 diff 永久消失，不划算。重启条件见卡。 |
| **QA-04（L1）多会话并行无规则 → 改走一卡一分支 + PR** | 2026-09-17 | 本 PR | 🟡 待验收人过目（本卡两处事实被验收人点破后更正） | 验收人问「很多人在做项目，过程中怎么做？有指引吗？」——查完确认 `AGENTS.md` 只有三句半相关内容，而当天 main 已有 24 提交 / 至少 3 条并行工作流、15:06–15:27 的 21 分钟里两个会话交替推了 6 次，全靠临场判断。随后验收人拍板改走标准分支 + PR。**本卡两处自我更正，都是验收人一句话点破的**：①我把「agent 开不了 PR」写成「固定成本、不可优化」还叮嘱别去绕——错，GitHub 支持预填链接 `/compare/main...<分支>?expand=1&title=&body=`，推完分支给链接、验收人点开即已填好；`gh` 不可用只挡「agent 自己合并」，而合并本来就该人做。教训：把没查过的限制写成「不可优化」比漏掉它更糟，会让接卡人也不去找办法。②我以为要把 `push` 触发限定到 main，查完发现四条 lane 的 `push:` **本来就写着 `branches: [main]`**——推分支零 CI、开 PR 跑 pull_request lane、合入再跑 push lane，正是验收人要的形状，**一行 yml 都不用动**。**落地**：`AGENTS.md` 交付段首条改为「一卡一分支、走 PR、不直推 main」（含命名规则、停在 push 后、预填链接格式）；新增「分支上快验，PR 上全验」并写明零 CI 的依据；红线 5 口径统一（删 `wip/<卡号>`——一卡一分支本身即隔离，红测留在自己分支别开 PR 或标 draft）；新增 `.github/PULL_REQUEST_TEMPLATE.md`（卡/改了什么/自验含反证与 E 级/账本/留白挂号）。`.github/workflows/**` 一字未动。**本 PR 自己就是第一次走新流程**（分支 `docs/QA-04-branch-pr-workflow`，推分支零 CI，`just ci-docs` 绿）。**仍只有验收人能定的三件**（GitHub 仓库设置，不在仓里）：合并方式建议 squash（main 线性，单文件二分才有用）；分支保护 require-up-to-date 建议开（今天发现并行会话红测正是靠 rebase 后跑 CI，纯分支模式下 PR CI 只测自己那份）；账本行时机（分支不消除 PROGRESS/QUEUE 冲突，只是推迟并集中）。 |
| **NET-15（L1）daemon 重启后 status() 必须重拉丢失的交付任务** | 2026-09-17 | `c01a02f`+`b67b0a7` | ✅ 代码完成，待归档（L1 无需真机） | 卡面验收全做：daemon 重启（或任何 task 丢失）后 grant 仍 Active、内存登记表归零 → 手机 ≤8s 的 `flow.status` 轮询永远收到「active + task_running=false」、无人自愈、严格头队列被堵死，只能靠外部重新 offer 才治愈。修法照卡面：`status()` 检出 `grant Active && !tasks.is_running` → 从 grant 行重建 `FlowFetchRequest`（`capture_at_ms` 不落库填 0，协议明文的老客户端值，ingest 退回 EXIF→mtime）复用现有 `spawn_fetch_task`，不新造第二套启动路径。**RED 先行**：新集成测试 `status_respawns_the_delivery_task_lost_to_a_daemon_restart`——同一 db + 同一 retained store 新建 `FlowDelivery` 模拟进程重启（suspend 作进程死亡的进程内等价物，与既有 `suspend_then_resume` 同手法），改前真红；断点续传断言真 partial（2MB<KILL<24MB）重拉后收敛 completed 且 `local_bytes == PAYLOAD`，不从零重长。**反证真跑**：注释重拉分支 → 新用例变红，还原复绿。**实施中追加 1**：`spawn_fetch_task` 的「is_running 检查 + register」原两次独立加锁，并发 offer/fetch/status 理论上可双 spawn，`FlowTaskRegistry` 新增原子 `try_register`（键结构不变）。**实施中追加 2（语义变更，显式登记）**：`suspend_interrupts_an_in_progress_fetch_and_keeps_the_grant_active` 原断言「suspend 后 status 读到 task_running=false」——新语义下 status 轮询本身就是恢复信号，升级为「status 轮询重拉任务」；安全性依据 grant 按 peer 隔离 + 手机暂停会停掉本 peer 轮询。**不 bump 版本**：无协议帧/手机侧变化。**计数**：`cargo test -p daemon --test flow_delivery` **33/33**（32+1 新）；`just ci` 全绿（fmt/clippy/nextest 全量/arch/queue/md/token）。**发现分岔（挂号建议，未越权修）**：`network_fetch_failure_records_a_fetch_failed_error_at_fetch_stage` 为时序敏感既有测试，本次全量跑约 1/5 概率多 1 条 conn 事件变红；无改动基线 4/4 绿、带改动 8/10 绿，且该测试不调 status()、与本草改无调用路径交集，判定既有抖动被布局扰动，建议另开卡根治。NET-06 崩溃恢复勾项已随本卡勾掉。 |
| **QA-03（L1）卡归档时相对链接集体失效，且没门禁能发现** | 2026-09-17 | 本 commit | 🟡 代码完成，等验收人过目（本卡两次自我证伪，结论与原计划不同） | 发现过程即证据：IDX-01 归档后自查，发现**我自己刚造了 2 处断链**（卡移进 `done/` 相对深度变了没跟着改），全量扫一遍才知道是系统性问题。**根因是门禁缺口**——`check-queue-sync.sh` 三条断言全围绕「QUEUE ↔ cards 目录」，没有一条检查卡内部的链接，所以这类错误 100% 靠人眼，而它恰好发生在人最容易分神的时刻：移文件。**做了三件事**：①新增第 4 条断言，遍历 `cards/**/*.md` 的相对链接逐个 `[ -e ]`；②修掉全部 8 处断链；③新增 `just ci-docs` 快车道（queue-check + md-check + token-check，不含 fmt/lint/test），`just ci` 依赖列表一字未减。**判据设计上的三个坑都踩过并修了**：⒜必须按**含该链接的文件所在目录**解析、不能按仓根——按仓根解析那 8 处会全部通过，断言等于白写；⒝不能按后缀过滤——AUDIT-03/DESK-15 指的是 `docs/`、`assets/` 下的文件，同样因少一层 `../` 而断；⒞**代码里长得像链接的不是链接**——卡面常用 `` `](相对路径)` `` 讲规矩，第一版正则把中文当链接、误报了自己这张卡，已补「围栏块整段跳过、行内反引号先剥掉」。**上线即抓到原挂号时漏掉的 3 处，且覆盖 bug 的反方向**：`DEV-02`→`IDX-01`（IDX-01 今天被我移进 `done/`，于是**指向它的别的卡**断了——原挂号只看到「卡自己移走、自己的链接断」）、`MOB-55`→`MOB-58`×2（MOB-58 早已归档）。**两次自我证伪，结论与原计划相反**：⑴原打算加「PROGRESS 不许同一卡号重复」断言兜住 union 风险，写完一跑红了 9 张卡——而 union 当时还没启用，说明那些重复是**正常历史**：`PROGRESS.md` 是按**交付事件**记的编年账，H-10b 占 9 行、NET-06 占 4 行且四行日期与 commit 列全同，(卡号) 与 (卡号+日期+commit) 三元组都不唯一 → 判据原理上不成立，撤。⑵那份数据顺带否掉了 `merge=union` 本身：union 只在两边改同一行时产生重复行，而该文件**本来就合法存在重复行**，意味着 union 残留与正常历史**不可区分、会静默永久留在账本里**。拿「可能静默污染事实源、且原理上不可检测」换「每次约一分钟且过程可见的手工合并」，对红线写着「不捏造」的仓是亏的——**不引入自动合并策略**，`.gitattributes` 不建。本会话两次冲突各不到一分钟，其中一次还让我看见并行会话把 TEL-05 挪去五区，那是想要的信息。根治方向（未开卡）：账本拆成一卡一文件 + 生成索引，冲突从源头消失，但那是重构账本格式。**关于「红线是不是太严」**：核实后结论是**不改 AGENTS**。远端 CI 的 paths 过滤本来就对（纯文档提交只点起 ci-docs）；真正的浪费在本地 `just ci` 对文档改动也跑全量 Rust，已由 `ci-docs` 解决。本会话真正花掉的时间是三件事：`target/` 涨到 98G 塞满磁盘、为凑一个汇报数字起了一次十分钟全量 nextest、两次 rebase——**没有一条是红线造成的**。**反证**：①改断一条链接→门禁退出码 1，还原→0；②行内与围栏代码里的假链接→不误报（仍 51 条）；③`QUEUE_FILE` 注入口未破，`test-queue-archive-gate.sh` 的 3/3 反证仍绿。 |
| **IDX-01（L2）索引重建没有任何运行时入口——索引一丢照片就永久看不见** | 2026-09-17 | 本 commit | ✅ 真库 E3 通过，已归档 | 验收人报「只能看到 13 张，本题远远不止」。取证后**范围更正**：不是「rebuild() 缺个按钮」，是**启动对账只有一个方向**——`Reconcile`(SYNC-01) 遍历 asset 行、文件不在就删行，只做「磁盘没了→清索引」；`LibraryWatcher` 两个方向都有但 `spawn()` 只装 notify 监听、**不做首次扫描**，所以 daemon 启动*之前*就躺在盘上的文件永远等不到事件。ADR-006「originals 是真相，索引可重建」在运行时因此没有执行路径（`rebuild()` 全仓零生产调用点，只有测试调它）。**事故本身不是产品造成的**：当前索引的迁移 `installed_on` 是 09-16 18:38 本地、`identity.key` mtime 同刻，而那批文件 15:12 就落盘；回放该时间窗 39 条 agent 命令无一写操作；仓里唯一清场脚本 `reset-local.sh` 是 `rm -rf` 整个库连原图一起删——验收人确认是重装时人工清了 `.ppf/`。**`originals/` 留下是对的**，真「清干净」等于删掉用户原图。**修法（不清表）**：`rebuild()` 那个循环本身就是增量收编（hash 命中即跳过），唯一破坏性的是开头的 `clear_assets()`，而它想覆盖的方向 Reconcile 已经在做 → 抽出共用私有 `index_missing_files`，`rebuild()` = clear + 循环（对外行为一字未改，T-012 六条仍绿），新增 `adopt_orphans()` 只跑循环；独立 `AdoptReport{adopted, already_indexed}`，不复用 `RebuildReport.duplicates`（清表后「跳过」只可能是本轮同内容副本，增量下压倒性是「早就在册」，同数字两种事实）。竞态：insert 撞主键先回查 `get_asset`，确认对方真落了行才咽下，否则原样上抛。`Reconcile::run_once` 接上收编方向，`with_local_node_id` 作开关（watcher 借用 `remove_asset` 的那份不接，不该顺带扫全库）；收编失败只 warn 不中断。**不查 `audit_tombstone`**：墓碑记的是内容离开过库，而文件此刻确实在盘上说明有人放回来了，按墓碑永久拉黑比本卡要修的问题更糟。**动手前四项核实**（决定了自动收编安全）：①全仓无「删索引行但留文件」的产品路径（三处删除全部先确认文件已消失，`product_delete` 只存在于一句文档注释，墓碑表 0 行）；②`timeline_page` 纯 `FROM asset` 不 join `device`；③缩略图按需生成；④`insert_asset` 撞主键报错。**替验收人做的判断并显式登记**：本卡不做桌面按钮，自动收敛已解决问题。**测试新增 9 条**：`adopt.rs` 5、`reconcile` 3、`idx01_wiring` 1（断言 main.rs 真接了开关——专堵 DEVLOG-01 那个「函数写好测好但生产没调、单测全绿而生产是死代码」的坑）；T-012 六条仍绿。**三组反证全真跑**：①停掉收编调用→2 条红；②让 adopt 偷偷先清表→adopt 3 红 + daemon 1 红；③main.rs 摘掉接线→接线断言红。三处均已还原。`just ci` 全绿。版本 `0.5.4-test.6`→**`0.5.4-test.7`**，versionCode 27→28。**真库 E3**：清空构建产物后从零重建、完整打包、替换 `/Applications/P-Pass.app`、重启 daemon（包内与安装后均报 test.7，运行中二进制 md5 `a31fa29d…` 与新构建逐字节一致，`lib/` 6 个，codesign 过）。**跑前先下预测**（36 个磁盘文件、25 行索引、11 个孤儿含 1 份与 be03 内容相同的副本、字典序 be03 赢）：25→**35**、`d9b69ede` **10** 行。**实测逐位命中**，审计 `index.adopted {"adopted":10,"already_indexed":26}`，10+26=36 与磁盘闭合。**看得见而非只是查得到**：用桌面壳自己那条 IPC（`App.svelte:664` 的 `timeline.page`）拉全量返回 35 张、其中 `d9b69ede` 10 张；另核实 `photoWall.js` 只在变更检测里用 `src_device`、不做设备过滤。**缩略图** 10 条全部 `thumb.get` **10/10 成功**、首条 11244 字节真 JPEG、请求后 `thumb_state` 全变 1。**媒体类型无漂移**：收编的 4 个 `.mp4` 库内均为 `video/mp4`，与正常 ingest 一致（中途误看到 `video` 以为漂移，那是 IPC 给 UI 的粗分类投影，已更正）。**反证①在真环境自然成立**：同库同文件，整个会话里 test.6 始终 25 行、那 10 张一直不可见，只换 daemon 到 test.7 重启即变 35，变量只有 daemon 一个。**顺带**（同卡范围）：启动日志原本只报移除数、收编多少一字没有，已补成「移除幽灵资产 N 条，收编孤儿文件 M 条」，每小时那轮同样补上并沿用「只在真收编了才记」的噪声纪律。 |
| **NET-16（L1）completed 后重复 status/fetch 零重传字节的直接断言** | 2026-09-17 | 本 commit | ✅ 代码完成，待归档（L1 无需真机） | 从 NET-06 拆出的纯测试卡。`fetch_inner` 的 completed 短路分支（`flow_delivery.rs:606`）本已存在——**零生产代码 diff**，与卡面预判一致。新增集成测试：grant 完成并确认字节真实落进本地 blob store 后 `provider_transport.close()` 掐死数据面，再对同一 grant 连续 fetch()+status() 三轮——任何重新触碰网络（重拉字节/重 spawn 任务）都只会打死端点；断言三轮 receipt_id 不变、`task_running` 恒 false、status 回 completed。**反证真跑**（E3，之后还原）：删短路分支 → 重复 fetch 落进 Cancelled 臂即红，还原复绿——卡面要堵的「防御性重拉保险」式回归确能被抓到。`flow_delivery` **32/32**（31+1 新）、nextest **430 passed/1 skipped**、`just ci` 全绿。已回写 NET-06 勾掉幂等验收项。**顺带挂号 QA-02**：`just ci` 暴露归档门禁变异 C 自 1968008 起空转（分区七成末节后 python `next()` 抛 StopIteration、生成文件缺失仍被记「符合期望」），与本卡无关不顺手修。另本机磁盘满致首跑编译失败，清 `target/debug/incremental`(12G) 恢复。 |
| **TEL-05（L1）遥测外呼加超时与队列封顶** | 2026-09-17 | `413f9d1` | ✅ 代码完成，待归档（L1 无需真机） | 焊点 T1 闭环：`reqwest::Client::new()` 无总超时——半死端点（TCP 通、永不回响应）把 `flush_now` 永久挂起 → 5min 周期循环停摆 + 队列只进不出。修法照卡面四条：①`Client::builder().timeout(20s)`（常量注释钉死「死活判定窗非预期耗时」）+ `with_timeout()` 测试 seam；②`record()` 封顶 500、超限 drain 最旧、debug 计数不造遥测遥测；③`run()` 不加第二层超时（单一真相源）；④超时并入既有丢班臂，best-effort 零漂移、字典/URL/Worker 端未动。测试：黑洞端点用例（5s watchdog 内必返回）+ 600→500 丢最旧用例（首条 ms=100 末条 ms=599），`telemetry_flow` 4/4；反证真跑——撤 timeout 挂红、禁 cap 变红，还原复绿；`query_telemetry` 3/3、`flow_delivery` 31/31 零回归、`just ci` 全绿。行业对照领卡前核实：OTel `exportTimeoutMillis` 30s/`maxQueueSize` 2048 满了丢、OTLP timeout 10s——同形状。**实施中追加**：本地 `just ci` queue-check 步恒红的既存 bug（`test-queue-archive-gate.sh` `$got（` 全角括号被 macOS bash 3.2 吞进变量名，CI bash 5 不吞故隐身），不修则本卡验收④不能成立，花括号修复。卡面验收①字面「返回 0」按 `flush_now` 既有契约（返回离队批量数）记录偏差口径。 |
| **NET-25（L2）真实传输路径上 `flow.delivered` 推送是否送达，从未正面验证** | 2026-09-17 | `b4713ed` | ✅ 真机验收通过，已归档——**结论：推送是好的，无需修复** | NET-24 那轮「每张 0.3 秒」看着漂亮却不作数：三条信号解析成功后走同一行 receipt 赋值，跑多少轮都分不出是谁结的账。**修法是纯可观测性、零行为变更**——手机侧三个终结点各一条 `Flow resolved by=offer_reply|push|status`；daemon `emit_flow_delivered` 加一行 `peer_subscribed=`。**没有用 `broadcast::Sender::receiver_count()`**：手机的 `timeline.subscribe` 与桌面壳的 `events.subscribe` 挂同一条总线，桌面常驻订阅让那个计数恒大于 0、回答不了「这一台在不在」，改查 `SubscriptionRegistry` 按 NodeId 的登记表（新增只读 `is_subscribed()`）；日志带 seq，否则 `DedupGuard` 会把连续几项折成一行。**测试**：subscriptions 单测 5 passed（新增 2）；Android 新增 `NET25ResolutionAttributionTest` 3 条源码合同测试（三条标签齐全且各一次、各落在所声明的分支、`flowWaitStep` 的 `Resolved` 分支仍只有推送一条——哪天本地事件也产出 `Resolved`，`by=push` 就开始说谎，那时必须当场变红）；反证真跑：摘掉 `by=push` 那行 → 3 条里 2 条变红，报错正是「缺少 Flow resolved by=push」，已还原。`just ci` 全绿；nextest **427 passed / 1 skipped**（改前 425）；Android **81 类 / 406 tests / 0 failures / 4 skipped**（改前 80/403）。版本 `0.5.4-test.5`→**`0.5.4-test.6`**，versionCode 26→27。**真机验收（三星 SM-S9210）**：两端版本均已核实（桌面运行中二进制 md5 `9b8ca7cd…` 与新构建逐字节一致、`lib/` 6 个、`codesign --verify --deep --strict` 过；手机 `-r` 覆盖装，`firstInstallTime` 11:18:40 未变只 `lastUpdateTime` 变 → 身份数据保留）。验收人选「Lark」相册，一次操作自然跑出两轮：**轮①真实传输 12 项（seq 14–25）→ 6× `by=push` + 6× `by=status`**；**轮②重新配对后重发同一批 25 项 → 25× `by=offer_reply`**。daemon 侧 37 条推送全部 `peer_subscribed=true`；资产 13→25（新增 12，与轮①逐项对上），审计同窗口 12 条 `ingest.new`。**6 行 `by=push` 就是本卡要的正面证据，不是推断。**那 6 条 `by=status` 不是推送丢了——全部是 `local=Completed(hash=…) elapsedMs=1`，即手机自己的 iroh 发送端事件在等待循环起步前就已终态，`flowWaitStep` 按 NET-14「本地判活优先于任何远端回声」直接走 `CheckStatusNow`，推送连上场机会都没有；全程 **0 项**碰 30 秒兜底。**反证成立**：轮②与轮①标签完全分开，判别真能区分。**必须写明的局限**：`peer_subscribed=` 是按 NodeId 查、不是按本次尝试查，连续轮次里可能陈旧地为真，**不能用它否定 NET-24**（NET-24 说的是「本次尝试自己的订阅还没建立」，两者不矛盾）；本卡结论只站在那 6 行直接证据上。**顺带产出**：同一份日志坐实新缺陷——会话内重新配对成功后不自动开传、杀 App 重开才动，已拆 **MOB-87** 挂号。 |
| **BUILD-05（L1）本地桌面打包在预期内的 updater 签名失败处整个中断** | 2026-09-17 | 本 commit | ✅ 已完成 | 2026-09-17 做 NET-24 真机验收时踩到，当场手工绕过才完成。`tauri.conf.json:45` 是 `createUpdaterArtifacts: true`，本地没有 `TAURI_SIGNING_PRIVATE_KEY` 时 `pnpm tauri bundle` **必然**在 `.app` 已产出之后因 updater 签名失败而非零退出；脚本头部 `set -euo pipefail` 于是当场中断，把步骤④嵌 `lib/`、⑤重签、⑥dmg 全跳掉，留下一个**看起来存在、实际起不来**的 `.app`（bundle 后 daemon 的 rpath 是 `@executable_path/lib`）。AGENTS.md 明写无凭据路径签名失败是预期行为——问题不在它失败，在它连累后三步。**改法**：步骤③用 `set +e` 取退出码后按凭据分流——有 `TAURI_SIGNING_PRIVATE_KEY` 或传了真 identity → `FATAL` 退出（**CI 严格性一字未放宽**），两者都无 → warn 后继续；两条路径之后都仍过原有 `[ -d "$APP" ]` 断言。**顺带**（验收人 2026-09-17 定调「本地构建是开发环境，可以不签名公证、不做很仔细的 CI/CD 验证，项目必须先讲究快速」）：ad-hoc 身份默认跳过 dmg（hdiutil+挂载+AppleScript 布局对狗粮验证零价值），要 dmg 用 `PPF_BUNDLE_DMG=1`；CI 路径行为不变。这是唯一行为变更，已在卡面显式登记。**验收**：本地无凭据跑 → 退出码 0、`lib/` 6 个、`codesign --verify --deep --strict` 过、包内 daemon 打印 `0.5.4-test.5`；**反证真跑**（注释掉嵌 lib）→ daemon 直接 `dyld: Library not loaded: @executable_path/lib/libheif.1.dylib`，**而退出码仍是 0**——恰好证明判据不能只看退出码（也正是原 bug 瞒过人的原因），已还原；**CI 严格性对照**：传 `"Developer ID Application: FAKE"` → 打印 FATAL 并退出码 1，去掉 identity → 退出码 0。`tauri.conf.json` 一字未动（属 UPD-01 范围）。 |
| **NET-24（L1）NET-20 去重命中必丢 flow.delivered 推送——订阅晚于 offer** | 2026-09-17 | `496cf68` | ✅ 真机验收通过，已归档 | 验收人体感「第二次发同样的照片特别慢，但状态都是对的」。根因源码核实：手机 `NativeFlowDeliveryPort.kt:293` 先 await `desktop.offer`、`:321` 才异步 `subscribeTimeline`；而 daemon 在 offer 处理中**同步**走完 NET-20 去重（`flow_delivery.rs:485` `has_durable_copy` → `complete_without_fetch` → 落 receipt → `emit_flow_delivered`），事件总线又是 `broadcast::Sender`（`events.rs:4` 明写「无订阅者时 send 直接丢弃」）→ 推送在订阅上线前被丢弃且永不补发。去重意味着数据面零字节，本地 iroh 永无事件、`idleForMs` 恒 null，只剩挂钟兜底 `LOCAL_IDLE_STALL_THRESHOLD_MS=30_000`。**范围更正**：原卡写「断链重连场景」，实为**确定性必中**且与重连无关——真实传输的建连+搬字节延迟本可让订阅抢先建立，NET-20 把它降到零。**修法（验收人否掉「调订阅/offer 顺序」：靠时序赢竞态不是合适修法；并拍板测试阶段不考虑旧手机兼容）**：`flow.offer` 应答改返回 `FlowStatusReply`——即「offer 完立刻调 status 会得到什么就给什么」。标准答案先例：HTTP 异步任务的 200/201 vs 202 按每次请求决定而非按端点固定；Docker Registry v2 blob mount 已有则 201 零字节、没有则 202 走上传（同为内容 digest 寻址 + 去重短路）；HTTP 304；Git have/want；rsync 校验和先行——共同点是「我已经有了」一律在发起方还在等的那个应答里回答。也正是 NET-14 自己写的「推送为加速，不是唯一路径」。零新类型（复用 `proto/msgs.rs:342` 的 `FlowStatusReply`）、手机零新解析（复用 `flowStatusPollOutcome`）。三个终态分支全带回终态（含 NET-22 rebind 分支，它注释里自己记着也踩过同一个坑）；输给并发 cancel 时据实回当前状态、**不伪造 receipt**；`router.rs` 那句把 receipt 扔掉的 `.map(\|_\| None)` 删除。手机侧应答为空即明确报错，不静默退回老路。**测试**：daemon `flow_delivery` **31 passed**（29+2 新）+ 反证真跑（两终态分支退回改前形状 → 两条断言双双变红，已还原）；Android 新增 `NET24OfferReplyTerminalTest` 3 条（源码合同式，锁「应答先于订阅被消费」这个顺序不变量；`start()` 依赖 ContentResolver/Uri/Blake3/原生 bridge，行为测试成本远大于所锁之物，本仓已有同款惯例）+ 反证真跑（撤回接线 → 2/3 变红）。`just ci` 全绿；nextest **425 passed / 1 skipped**（改前 423）；Android **80 类 / 403 tests / 0 failures / 4 skipped**（改前 79/400）。版本 `0.5.4-test.4`→**`0.5.4-test.5`**，versionCode 25→26。**顺带修既有漂移**：bump 脚本拒跑，报 `apps/desktop/src-tauri/Cargo.lock` 停在 test.3——09-16 那次 bump 到 test.4 漏了它（脚本断言是好的，是那次 bump 绕过了）；用 `cargo metadata` 让 cargo 重算而非手改 lock，diff 确认只动两个本地包版本行。**真机验收（2026-09-17 通过）**：测试机重装重配对（设备上读出 versionName 0.5.4-test.5 / versionCode 26；macOS 端 `lsof` 核实持库锁的就是新二进制，运行中文件 md5 与新构建一致、与旧包不同）。logcat 以 `Flow epoch preflight`（每次交付尝试在 offer 前的唯一一行）为判据逐项计时：**11 张 3.30 秒**（每张平均 0.330s、最大 0.396s）、另两批 2 张 1.02s / 13 张 3.58s（平均 0.298s、最大 0.466s），**三批合计 ≥5s 的项 0 个、≥30s 的项 0 个**。去重确实发生：库内总计 13 条资产（旧设备身份 11 + 本次新身份 2），11:22 后全库只新增 2 条——即 13 张那批里 11 张走零字节去重短路。对照同机同批照片改动前 2026-09-16 实测约 3 分钟（每张耗满 30 秒兜底）。**新路径在跑的直接证据**：手机侧对空应答加了硬失败，22351 行 logcat 里该错误 0 次、flow 报错 0 次、cancelled 0 次——旧 daemon 回 null 必然触发它。**遗留**并发 cancel 竞态分支无直接用例（无注入点，不硬造，卡面标 `[~]`）；真实传输路径的推送是否送达仍未正面验证，已拆 **NET-25** 挂号单独跟踪。 |
| **DEVLOG-01（L3）开发期前台 daemon 日志不落盘** | 2026-09-17 | 本 commit | ✅ 已完成 | 状态对齐时发现账外工作：外部 worktree（`devlog/persistent-local-logs`，落后 main 8 个提交、零独有提交）里 `log_guard.rs` 有 184 行未提交改动且无对应卡。收口回 main 时补了原改动缺的三处：①**接线**——原改动加了 `DedupGuard::for_file()`/`persistent_log_path_from_env()` 但 `main.rs` 仍调 `DedupGuard::new()`，两个新函数生产零调用是死代码；打不开路径照 `parse_cli` 先例 `exit(2)` 而非静默退回 stderr。②**格式化**——原改动未过 `cargo fmt`，直接 push 会重演 8/7 fmt 红。③**修死循环测试**——原 `file_sink_truncates_past_the_cap` 写成 `while *written < SINK_CAP_BYTES`，而 `write_line` 在跨过上限*之前*就 truncate 并重置计数器，条件恒真、实测 60s 后 SIGKILL，说明该测试从未被作者跑过；改固定行数上界后 0.35s 通过。`cargo test -p daemon --lib log_guard::` **7 passed**；反证真跑（上限判据改恒假 → `got 10486230` 变红，已还原）。端到端隔离 `HOME` 验证三分支：设了→文件出现 377 字节真实日志且 stderr 0 行、不设→stderr 4 行同改动前、打不开→exit 2 带明确报错（全程未碰生产库，生产 daemon 进程全程存活）。**取舍**：either/or 不是 tee，设了就不写 stderr；**生产永远不要设 `PPF_LOG_FILE`**，否则 DESK-10 导出包依赖的 plist `.err` 会变空。与 DESK-10 零交互已核实（导出包从 launchd plist 读路径，不认环境变量）。 **顺带修复既存漂移**：`1968008`（09-16 QUEUE 重构）把 `tools/check-queue-sync.sh` 与 `tools/test-queue-archive-gate.sh` 的执行位删掉了（`:100755 → 100644`），`justfile:59-60` 用 `./tools/...` 调用 → **本地 `just ci` 在 queue-check 步直接 `Permission denied`（exit 126）**；CI 的 ci-docs lane 用 `bash tools/...` 调用不受影响，所以 CI 绿着、只有本地门禁哑火，没人发现。已 `git update-index --chmod=+x` 恢复两个文件。 |
| **NET-23（L0）Flow 等待循环在本地信号永远不触发时必须有出口** | 2026-09-16 | 迁移自 QUEUE 分区五 | ✅ 已修复 | 三星真机复现：断链重连后 11 张全部命中 NET-20 去重，手机等待循环因本地 iroh 从未有活动（`idle_for_ms` 永远 null）而没有兜底出口，永久卡在「正在备份…0/11」；补一条基于调用方自身挂钟耗时的兜底超时分支；`NET14PushFirstDeliveryTest`/`ARCH01StrictConsumerTest` 全绿；真机复测同一批 11 张从 0/11 推进到 11/11。推送为何缺失另拆 NET-24。 |
| **NET-22（L1）rebind 一个已完成的 Flow grant 必须补推送** | 2026-09-16 | 迁移自 QUEUE 分区五 | ✅ 已修复 | rebind 一个已完成的 Flow grant（新 lease 重新 offer 同一 tuple）时补发 `flow.delivered` 推送，行为与 NET-20 的 `complete_without_fetch` 分支一致；新增集成测试，`cargo test -p daemon --test flow_delivery` 29/29。rebind 路径缺推送的缺口已堵。 |
| **AUDIT-03（L2）审计合同 v1：Case Matrix 与可信边界** | 2026-09-10 | 迁移自 QUEUE 分区五 | ✅ 设计已收口 | 审计 Case Matrix、【本地】/名称+短指纹、支持级可信、全库保留与无专用导出边界均已收口；实现前置转入 AUDIT-04。 |
| **NET-20（L1）Flow 单通道 `offer` 补传输前哈希对齐 + NET-21（L1）flow-staging 孤儿回收** | 2026-09-16 | 本 commit | ✅ 两卡均代码完成，`just ci` 全绿；均未做真机验证 | 用户先追问"是不是没有传输前哈希对齐"，核实后开 NET-20；追问"NET-06 之后是不是没有 GC"，核实后开并立即修复 NET-21；随后拍板"顺手把 NET-20 也做了"。**NET-20**：`offer_inner` 原先从不查 `db.get_asset(&hash)`，内容已存在也会整份重传字节，事后才靠 ingest 判重丢弃。新增 `Ingestor::has_durable_copy`（与 `ingest_inner` 自己的 Duplicate 判据同构：索引有行且文件真实存在于磁盘），`offer_inner` 里 `rebind_completed_flow_grant`（tuple 级幂等）之后、`spawn_fetch_task` 之前插入短路——命中直接复用现有 `complete_flow_grant` 完成路径推送 receipt，不新增协议字段、手机侧零改动。RED 先行 2 条集成测试（跳过网络抓取 / 未命中时行为不变）+1 条反证（去掉短路后状态从"立即 completed"变"active"，证明测试真的钉住这条逻辑）。`cargo test -p daemon --test flow_delivery` 28/28（26 原有+2 新）、`cargo test -p core-index` 全绿。**NET-21**：Flow 导出中转目录 `.ppf/flow-staging` 没有孤儿回收——`export_to` 落盘的文件只在 ingest 成功分支被删，ingest 真失败/materialize 中途 cancel/daemon 崩溃都会留下孤儿，此前无任何定时任务扫描（旧上传收件箱 `.ppf/staging` 有 `sweep_orphans`+`reclaim_staging`，新装卸台没接上）。`inbox.rs` 新增 `sweep_flow_staging_orphans`（判据同构：命名不合契约/落地不足宽限期/`protected` 内的 content_hash 均不碰），`main.rs` 启动时+每小时各调一次，保护集直接复用 `Db::active_flow_content_hashes()`（与 flow-blobs 的 iroh GC 回调同一张持久表）。RED 先行 5 条纯函数单测+1 条反证（临时禁用保护判据后用例从 9 变 16）。新增 `tools/verify-flow-staging-gc.sh` 供用户自行验证：只读模式检查真实库现存孤儿，`--dry-run` 在隔离沙盒里直接调用生产函数 `daemon::sweep_flow_staging_orphans` 断言孤儿被清、有主/刚落地文件被保留（已本地跑通两种模式，退出码 0）。`cargo test -p daemon --lib inbox::` 10/10、`cargo test -p daemon` 全量无回归、`just ci` 全绿。版本 bump `0.5.4-test.1`→`0.5.4-test.2`。**留白**：两卡均只做了本地测试，未做真机验证（NET-20 需要真实"桌面已有某照片、手机重offer同内容不同tuple"场景；NET-21 需要真实制造 materialize 失败/cancel 场景确认目录清零）。 |
| **UI-12/UI-13（L2）通知呈现层重做 + 移动端手写控件换 Material3** | 2026-09-15 | 本 commit | 🟡 代码已合并，等真机验收（真机截图已确认，待验收人复核） | 用户三轮对话逐步拍板：①后台备份被系统限制的提醒不该埋在设置页 CellRow 里；②要符合 Material 设计规范但用自家设计语言；③"总体处理一遍"。**UI-12**：`HomeNotice` 新增 `critical`/`dismissLabel`/`onDismiss` 三字段，`NoticeCard` 按 `critical` 分强色/温和色两套视觉；`NoticeHost` 新增消费 `BackgroundBackupState`，`NeedsSystemAuthorization`/`SystemStoppedWatcher` 接入全局提示宿主（此前绕开了 `NoticeHost` 自己声明的"全局唯一"契约）；非阻断类新增独立"知道了"忽略动作（`backgroundBackupNoticeDismissed`，以状态本身为 `remember` key，状态跃迁自动重置，符合"暂时忽略非永久偏好"语义）；`resolveBackgroundBackup` 提炼单一 lambda，`NoticeHost`/设置页 `CellRow` 共用同一入口不再各写一遍。**UI-13**：`TabIcons.kt` 齿轮手绘 Canvas 删除改用 `Icons.Filled.Settings`（相机图标因 `material-icons-core` 无对应符号保留手绘，已核实枚举）；设置角标手写圆点改 `Badge`/`BadgedBox`；`PhotosScreen.kt` 私有 `FilterChip` 改用 Material3 同名组件（避免撞名重新发明轮子）；`HomeScreen.kt` `HeroSecondaryButton` 改用 `OutlinedButton`（真正 disabled 语义）；`TwoTabs` 的 `NavigationBar` 与 `CellRow`/`RuleSwitchRow` 的 `ListItem` 两项因量出的视觉尺寸冲突（80dp vs 64dp、56dp vs 52dp）保留现状并在代码注释记录理由，未静默跳过。RED→GREEN：`BackgroundBackupPresentationContractTest`/`HomeNoticesTest` 新增源码合同测试。**测试**：Android JVM `./gradlew :app:testDebugUnitTest --rerun-tasks` **378/378 绿**（时间戳本次生成）、`:app:assembleDebug` 绿。**真机验收**：三星 SM-S9210（`0.5.2-test.1`）`adb install -r` 覆盖安装，截图确认——琥珀通知条在 Photos/Backup 两个 tab 均可见、"知道了"/"去处理"两个独立动作正常工作、点"知道了"后跨 tab 保持一致 dismiss 状态；设置齿轮图标/角标渲染正常；Photos 过滤胶囊、设置页"暂停/继续"按钮视觉与改前一致，无渲染异常。 |
| **MOB-68 补丁：设置图标红点误把正常运行中的后台备份当"需要提示"** | 2026-09-15 | 本 commit | ✅ 已完成 | 用户反馈"设置图标一直有个小红点点"：`MainActivity.kt` 的 `settingsAlert` 判据误写成 `backgroundBackupState != BackgroundBackupState.OffByUser`——凡是用户开着后台备份（哪怕状态是正常运行中的 `Armed`）都算"需要提示"，红点永久亮着不消失。改为只在两个真出问题的状态触发：`NeedsSystemAuthorization`（待授权白名单）与 `SystemStoppedWatcher`（监听被系统清掉）。新增源码合同测试 `settings_tab_alert_is_not_raised_by_a_healthy_running_background_backup` 反证硬编码复活。顺带修复无关的既存漂移：`assets/i18n/{en,zh}.json` 新增的 `ui.device_open_folder*`（NET-13）未同步进 Android 打包资源，导致 `DiagTextTest.bundled_assets_never_drift_from_repo_source` 常红，重新拷贝后归零。Android JVM **69 类 / 374 tests / 0 failures / 0 errors / 4 skipped**（较改动前 373 净增 1）、debug APK、`just ci` 全绿。另开 **MOB-80**（backlog）：移动端视频预览默认静音——用户当场决定推迟到下次评估/更换视频播放器组件时一并做，理由是现有 Media3 `PlayerView` 默认控件无音量滑块/静音按钮，静音需要先加一层自定义控制 UI，不值得在将被替换的组件上先做定制。 |
| **NET-13 桌面设备列表：备份状态误报根因修复 + ID 列 + 打开目录 + 在线态独立列** | 2026-09-15 | 本 commit | ✅ 已完成 + 本机真机验证通过 | 用户报告"明显备份过却显示没备份过"，根因非展示口径：`device.watermarks.last_backup_at` 只读遗留批量扫描表 `backup_watermark`（`backup.commit` 才写），Flow 交付路径（`flow_delivery.rs::run_fetch_body`）从不调用 `set_watermark`，只会真的 `ingest()` 落地资产。`list_device_watermarks` 改为同时读 `asset.added_at`（按 `src_device` 分组取 max）与 `backup_watermark.updated_at`，两者取较大值（`NULLIF(MAX(COALESCE(...,0),...),0)` 规避 SQLite 多参 MAX 遇 NULL 即空的坑）。RED 先行：新测试改前真红（`watermark_falls_back_to_latest_asset_ingest_time_without_legacy_scan_row` panic），改后 `cargo test -p storage --lib device_repo` 11 passed。UI 三项（clarify 拍板）：设备 ID 列 8 位短指纹+悬停全量；打开该设备 `originals/<node_id>/` 目录按钮（无照片时退回 `originals/` 根目录，与 `openLibrary()` 同套退路）；在线状态从行内文案挪到固定宽度独立列（不做真表格，卡片式列表不变）。i18n 两新 key 中英对称，`crates/diag/src/keys.rs` 注册表同步（`ALL.len()` 100→102）。测试基线：`cargo test -p storage -p daemon` 全绿无回归；`apps/desktop` `pnpm run build` 成功 + `pnpm test` 10 files/62 tests 全过；`just ci` 全绿。**本机真机验证**：`cargo build --release -p daemon` + `tools/bundle-macos.sh` + `tools/bundle-desktop-macos.sh` 重新打包，`launchctl unload/load` LaunchAgent 让新二进制接管（md5 核实一致），真实打开桌面 App 截图确认：5 台已配对设备 ID 列/在线态独立列/打开文件夹图标均正确渲染；直接查询本机库 `index.sqlite` 证实 5 台设备 `backup_watermark` 全空、其中 4 台 `asset_cnt>0`——改前全部误判"还没备份过"，改后全部显示真实时间，与截图一致；唯一显示"还没备份过"的第 5 台经 SQL 核实 `asset_cnt=0`，确认是真的从未备份，非误报。 |
| **NET-06 daemon 侧核心机制（flow.status/flow.suspend）** | 2026-09-14 | 本 commit | 🔄 daemon 完成，Android 未接线 | 协议新增 `FlowTupleRef`/`FlowStatusReply`/`flow.status`/`flow.suspend`（proto 5 新 roundtrip + 既有 13 snapshot 全绿,旧帧未破)。daemon 核心:`fetch_inner` 数据面拉取(此前唯一无检查点的阻塞 await)改 `tokio::spawn`+`AbortHandle`;新增 `FlowTaskRegistry`/`FlowTaskGuard`(键=`(NodeId,queue_sequence,lease_token)`,不含 content_hash,照抄 `FlowPathGuard` Drop 模式与 `SubscriptionRegistry` generation 比对);`status()` 组合 durable 状态+进程内 task_running;`suspend()` 只 abort 任务不改 grant 状态(对照 `cancel_inner` 标 Cancelled)。8 个新 daemon 集成测试(4 status 态+跨设备隔离+suspend 中断/resume/cancel-GC 三个 24MB kill-race,手法照抄 `blobs_resume`)。**真实反证**:临时注释 `FlowTaskGuard::register` 后核心用例立即真红(suspend 变 no-op,fetch 悄悄传完),恢复后复绿。测试基线:`cargo test -p daemon --test flow_delivery` 22/22;`just ci` 全绿,`cargo nextest run --all-features` **397 passed/1 skipped**(较认领前 345/1 净增 52)。**未完成留白**:崩溃自动重拉、旧手机兼容验证用例、幂等零重传直接断言、迟到边界竞态用例、Android 全部接线(DaemonClient/NativeFlowDeliveryPort/StrictConsumer/AndroidFlowRuntime/CancellationRoundController + JVM 测试)、L3 真机硬门。卡片实施记录节已诚实标注每条验收标准的完成/留白状态。 |
| **NET-08 全仓焊点普查** | 2026-09-14 | 本 commit | 🟡 普查完成（待后续卡闭环归档） | 只读审计五域逐域打勾（daemon 全 handler/Android 全 call 点 14 处/src-tauri 16 command/tools/infra workers）。已知链自检:N1(DaemonClient 单值三用)/N2(fetch 回声即状态+超时回队)/N3(fetch_from_observing_path 无中断点) 全部命中并标 NET-06/07 覆盖。新焊点 4 张开卡:**NET-09** 数据面字节停滞看门狗(downloadAsset/APK 下载/upload 收流无 idle 上限)、**NET-10** pair.request 拆提交≠等人(120s 硬等+daemon 无限挂流,等 NET-06 形状)、**NET-11** 桌面壳 IPC call 无读超时(daemon 挂死拖冻整壳)、**TEL-05** 遥测 reqwest 无超时+队列无顶。豁免登记:update worker 外呼(平台保险丝+无账本三条件)、tools/(非产品链)。legacy manifest/push/commit 定性=REBUILD-00 已冻结、不开卡挂账。零生产代码 diff。 |
| **NET-06（定稿）控制面语义 + flow.suspend** | 2026-09-14 | 本 commit | ⬜ 卡已补全 | 结合验收人裁决与另一 agent 源码核实重写控制面节：三原则（意图先行不等回声/暂停≠取消且账本语义相反/观察只在需要时发生）；政策 (b) 定稿——暂停=当场掐断、恢复断点续传，撤销 QUEUE 四区的政策待拍板行；新增第五动词 **`flow.suspend`**（中断拉取任务但 grant 留 Active、GC 保护名单护住 partial），与 `flow.cancel`（标 Cancelled、partial 随保护失效被 GC 清理）共用中断机制、落账相反。核查确认地基全对（GC 保护名单/FsStore 跨重启续传/cancel 登记拒新拉取），真实缺口仅一处：`fetch_from_observing_path` 阻塞 await 无中途检查点——spawn+abort task handle 修法入范围。验收 +5 条：suspend 真停、续传零重传、cancel 清理、迟到边界竞态双确定、暂停路径零网络观察。纯文档提交。 |
| **NET-06（补）控制面语义全案** | 2026-09-14 | 本 commit | ⬜ 卡已补全 | 外部 review 指出暂停/继续/取消未纳入异步方案。补「控制面语义」节：同步模型把 租约/执行/意图 三概念焊在连接寿命上（rebuild-05、MOB-54 毛刺的共同来源）；新模型收敛为四动词 offer/status/claim/cancel——暂停=停签发(+当前张政策待拍板 a drain/b abort)、继续=重新 offer 幂等复用、取消=先过 irreversible 边界者赢（规则是数据不是时序）、恢复=先查账再动作。实做两件原缺的：cancel 持中断句柄真断字节流、status 词表含 cancelled/failed 码。验收 +4 条竞态/续传用例；政策缺口挂 QUEUE 四区。纯文档提交。 |
| **NET-06/07/08（L2/L1/L2）传一半停止全案开卡** | 2026-09-14 | 本 commit | ⬜ 未开工（纯文档，零 CI） | NET-01 复盘定性：根因不是超时值，是「提交任务与等结果焊死在一次同步 RPC」的形状错误（客户端靠回声猜状态、两端账本不对账、单值超时在两个相反目标间摆钟）。按验收人「禁止最小能跑通」拍板开三卡：**NET-06** 根治（Flow 交付 HTTP-202 化：offer 秒回 + 新增 `flow.status` 对账门 + 幂等领回执 + 重试前先查账 + 崩溃重拉 + timeline 推完成事件、轮询兜底；旧 fetch 保留兼容旧手机）；**NET-07** 过渡止血（超时按 建连/控制/fetch 三档拆开，卡头按设计纪律第 1 条登记过渡身份与回退评估）；**NET-08** 全仓同类焊点只读普查（W1-W5 判据，产卡不改码，NET-01 本身必须在清单内否则不合格）。验收人同轮要求在 AGENTS.md 新增「设计纪律」四条（已先行合入 `ccf391f`）。 |
| **MOB-54（L1）相册停摆重新取证** | 2026-09-14 | 本 commit | 🔄 P0 取证进行中 | 三星真机 32 张相册停摆不能直接套用 09-07 的瞬态失败根因：canonical audit 显示当日三轮为 **4 confirmed / 5 confirmed + 4 cancelled / 12 confirmed**，无本次 delivery failed；第二轮 cancel **82 秒后已有 restore**，不能归因为本次停摆。手机账本为 **21 CONFIRMED、OPEN/IDLE、无 lease、discoveryRequested=true**。MOB-54 从待共享回归移入进行中，下一步重放 receipt 与 discovery wake 的交错，并以 MediaStore、账本、审计三方对账定位未确认项，不改代码掩盖证据。 |
| **MOB-79（L2）照片网格视频播放角标** | 2026-09-14 | 本 commit | ✅ 真机验收通过 | 用户实机反馈：网格里视频和照片长得一模一样，点进去才知道是视频。`PhotosScreen.kt` 的 `ThumbCell` 新增按 `mediaType` 判断的播放三角角标（Canvas 手绘，半透明黑底+白色三角，22dp，右下角），跟系统相册/主流图库同一套视觉语言，不引入图标库依赖。同步补 `androidx.compose.foundation.layout.size` 缺失 import。Android JVM **69 类 / 346 tests / 0 failures / 0 errors / 4 skipped**（与改动前一致，纯 UI 叠加不影响既有断言）、`just ci` 全绿。三星真机确认：2026.03/2024.08 两个视频缩略图右下角均出现播放三角，同屏照片缩略图无角标，视觉可清晰区分。 |
| **MOB-74（L2）视频白框方案 B 真机验收通过** | 2026-09-14 | 本 commit | ✅ 真机验收通过 | macOS 本机重新 `cargo build --release --bin daemon` → `tools/bundle-macos.sh`（自包含依赖打包）→ `tools/bundle-desktop-macos.sh`（updater 签名步骤因无 `TAURI_SIGNING_PRIVATE_KEY` 失败属预期，`.app` 本体已在该步骤前产出，手动补 lib 嵌入+重签）覆盖安装 `/Applications/P-Pass.app`；此前运行中的 daemon 仍是 `0.5.0-test.8`（不含本卡修复），`launchctl` 确认新二进制 `0.5.1-test.1` 已接管。库内两个真实视频（`VID_20260329_151742.mov`、`Health20240811161633complete.mp4`）此前 `thumb_state=2`、磁盘缩略图为 256×256/3.6KB 灰色占位图；删除缓存+重置 `thumb_state=0`+ App 重启触发重新生成后，三星真机确认两者均显示可辨认首帧（考拉爬树/地图截屏），缩略图文件变为 19–221KB 真实图像，点开正常播放。顺带发现照片墙里一张白框实为 `MOB-67` 测试残留的 59 字节假 JPEG 混入生产库，与本卡无关，已开 `MOB-78` 单独跟踪。 |
| **MOB-77（L2）取消轮次恢复入口从常驻警告条挪进备份设置卡** | 2026-09-14 | 本 commit | ✅ 真机验收通过 | 用户拍板：`MOB-59`/X-05 把取消轮次的恢复入口做成常驻不可关闭的琥珀警告条，与"用户主动取消是正常操作"的语义相悖。改法：`HomeNoticeKind.CANCELLED_ROUND` 与优先级项、`NoticeHost` 对应候选分支全部删除；恢复入口挪到「备份」设置卡内，做成一行 `CellRow`（已跳过的照片 / N 张·点击恢复），紧跟"备份哪些相册"下方，`cancelledRoundCount == null` 时不渲染，不占位不常驻；点击复用既有 `restoreCancelledRounds()` 管线未改动触发逻辑。同批顺带文案精简：「备份」section 标签删除（整页即备份设置页，标签零信息增量）；「其他」改为「关于」（仍需一个标签把存储电脑/版本与上方备份卡区分开）。`MainActivity.kt`/`HomeScreen.kt`/`HomeNotices.kt`/`HomeNoticesTest.kt`/中英字符串资源同步。Android JVM **69 类 / 346 tests / 0 failures / 0 errors / 4 skipped**（真实生成，`HomeNoticesTest` 6/6）。三星真机确认：常驻警告条消失，恢复入口变为卡内一行，点击后 4 张跳过照片真实重传完成且该行自动隐藏。 |
| **MOB-66（L2）Android 品牌字体接入 Newsreader/Manrope** | 2026-09-14 | 本 commit | 🟡 代码完成，待真机/模拟器截图走查 | `res/font/` 新增 6 个静态字重（Newsreader 400/500/600、Manrope 400/500/700），来源 `github.com/google/fonts` 的 `ofl/newsreader`/`ofl/manrope` 可变字体，用 `fonttools varLib.instancer` 切出静态实例，SIL OFL 1.1 离线打包（不用 Google Fonts 运行时下载 API）。`ui/Tokens.kt` 新增 `PPFont`（`Serif`/`Sans`，字重映射对齐 `tokens.json` 的 `font.serif`/`font.sans`）；5 个消费文件（BucketScreen/ScanScreen/PhotosScreen/HomeScreen/Onboarding）11 处硬编码 `FontFamily.Serif` 全部改用 `PPFont.Serif`，并给每页顶层套 `ProvideTextStyle` 让隐式吃系统默认字体的正文显式改用 `PPFont.Sans`；`FontFamily.Monospace`（配对码/诊断等宽展示）未动。Android JVM **69 类 / 346 tests / 0 failures / 0 errors / 4 skipped**（本次生成）、`just ci` 全绿。`assets/design/README.md` 的过时 "*(future)* Tokens.kt" 描述已改成如实的已完成状态。欠：真机/模拟器截图对比标题/正文渲染前后差异——需要用户或后续走查补上。 |
| **MOB-74（L2）视频首帧走系统 Quick Look（方案 B）** | 2026-09-13 | 本 commit | 🟡 代码完成，待真机回归 | 验收人拍板「能用系统的就先用系统的」：ffmpeg 发现链全空时 macOS 用 `/usr/bin/qlmanage` 抽首帧 PNG 进既有 JPEG 管线，零包体零许可证；发现优先链不变（PPF_FFMPEG/bundled/PATH 仍先赢）。实测钉两事实：qlmanage 保持宽高比出真 PNG（`-o` 目录须先存在）、**对不可解码输入永久挂起**——`tokio::timeout` 弃 future 不杀进程，故 4s deadline+kill 做在 spawn 循环内，垃圾输入用例即其反证。arch-check B.2 拦下 `cfg!(target_os)`，改绝对路径存在性探测（非 Mac 天然 None，与 ffmpeg.rs 同纪律）。验证：media-codec lib 10/10 + thumbs 8/8 + `ql_fallback` 端到端 1/1（本机确认无 ffmpeg，真跑回退分支）+ nextest 217/217 + `just ci` 全绿。Windows 覆盖面外维持占位图，C（显式降级态）挂账。真机：验收人 Mac 覆盖安装看照片墙。 |
| **MOB-74（L2）视频白框取证** | 2026-09-13 | 本 commit | 🔎 根因锁定，等 B 线拍板 | 代码考古钉死：视频首帧硬依赖外部 ffmpeg，找不到时 `make_thumbs` 按契约静默写灰色占位图（`thumb_state=2`）；CI/E2E 全绿靠的是 Linux runner `apt-get install ffmpeg`，而 **release.yml 与 mac/windows 打包脚本对 ffmpeg 零引用**（`fetch-ffmpeg.sh` 注释引用的 T-071 不存在）——发布的桌面端从未有能力生成视频首帧，手机与桌面读同一个 daemon `thumb.get`，跨设备同症状自洽。方案菜单 A（ffmpeg 进包，+~25MB+GPL 声明）/ B（macOS qlmanage 零依赖兜底）/ C（显式「视频·无封面」降级态，卡面硬验收）写入卡内与 QUEUE 四区，等验收人拍板。纯文档提交，零代码改动。 |
| **MOB-76（L1）Wi-Fi 约束必须是交付硬闸门** | 2026-09-13 | 本 commit | 🟡 代码完成，待真机回归 | 取证钉死断点：触发入口判据实算没错，但 `FlowRunner` 事件后 wake 共 6 处硬编码 `constraintsSatisfied = true`（完成回执推进、失败重排、重试、缺源跳过、取消/恢复收尾）+ `BackupWorker.doWork` 把 WorkManager 调度放行直接当业务闸门——限制开启时第一次唤醒正确进等待态，之后任何一张回执/失败都把下一个队头照常推上蜂窝（与 09-12 观察 5→6 同形）。修法（结构性消除）：`FlowRunner` 注入 `constraintsProvider` 实时闸门端口，事件后 wake 全部改读；`requestFlowWake`/`runFlowWake`/`continueFlow` 默认值由常量改为 `flowConstraintsSatisfied(context)`（`wifiOnly × NET_CAPABILITY_NOT_METERED`，`isOnUnmetered` 自 MainActivity 上移共用）；Worker 不再转发调度放行。与 MOB-19 关系：卡面（09-12）已拍板手点路径在交付层过闸，`constraintsFor(MANUAL)` 调度层不动。新 `MOB76WifiGateTest` 3 例（回执不越闸、重试同闸、源码合同反证防硬编码复活）；Android JVM 全量 **69 类 / 346 tests / 0 failures / 4 skipped**（含新用例，QUEUE 基线行 347 为旧数）、`assembleDebug` 绿、`just ci` 全绿。真机验收：蜂窝+限制开启→发起备份→零传输+Wi-Fi 等待态；连回 Wi-Fi 自动续传。 |
| **真机狗粮记录（OPPO / v0.5.1）** | 2026-09-12 | 本 commit | 🟡 已开卡、未修复 | 走查全文见 `docs/evidence/2026-09-12-oppo-051-dogfood.md`。新开 **MOB-76（L1）**：「仅 Wi-Fi 时备份」开启时 5G 蜂窝下仍发起传输（约束闸门缺口，与 MOB-71 呈现层分卡）。**MOB-54 真机复核失败重开**：25 张相册直传约 10 张后停摆、剩 16 张无提示不自动续传，改选第三个相册才恢复——先取证停点失败码，不凭症状认领旧根因。MOB-52 同族症状再现（后台截图 1 分钟零同步、重开即补捞；配对后进 App 一次卡崩溃；「确认断开」卡顿），崩溃栈仍未到手，L0 前置不变。MOB-64 入账新场景：手机残留配对/桌面无记录时冷启动「尝试传输」长期零提示（拒绝码待取证）。MOB-71 追加「重开再关限制不唤醒」回归项；MOB-74 视频白框跨设备再现。覆盖安装保留后台授权未再弹权；通知授权在进 App 时出现一次（MOB-68 相关，卡内既有节点覆盖）。 |
...
| **MOB-71（L1）关闭 Wi-Fi 限制后暂停复活等待提示** | 2026-09-11 | 本 commit | 🟡 代码完成，待三星回归 | 根因是触发时 `wifiDeferred` 留存、关闭限制时未清除，传输期间被 busy 遮住、暂停后复现。HomeScreen 现在同时检查当前 `wifiOnly`，开关变更即时清除/重算该标记。RED 复现后 GREEN；focused JVM **3/0/0/0**、Android JVM **341/0/0/4**、debug APK、`just ci` 全绿。待蜂窝网络下的开始→暂停→继续，以及重新开启 Wi-Fi 限制回归。 |
| **MOB-64（L2）撤销设备后的配对失效投影** | 2026-09-11 | 本 commit | 🟡 代码完成，待三星回归 | 根因是 Flow delivery 只识别 `err.not_paired`，而三星真实撤销在 `hello` 返回 `err.not_authorized`。新增 RED 用例后，delivery 复用既有共享拒绝码判据，两个错误码均置 pairingLost 并复用红卡；定向 JVM **3/0/0/0**、Android JVM **338/0/0/4**、debug APK、`just ci` 全绿。当前无 ADB 连接设备，尚未冒充完成真机撤销→下一次 Flow 调用的验收。 |
| **MOB-62（L2）断开重扫 ANR 回归** | 2026-09-14 | 本批次 | 🟡 代码完成，待完整重扫回归 | 三星 bugreport 证明 UI 快照在主线程等 `flowRuntimeLock`，而 scope-wake 线程持锁执行 `nativeOpen()`，导致 Input dispatch ANR。`runtimeFor()` 改为短锁读/发布、锁外耗时初始化；若竞争线程已发布或 epoch 变更，关闭候选 runtime。新增 RED→GREEN 合同测试；Android JVM **354/0/0**。三星修复版选择隔离图片并开始备份 `1/1` 成功、无 ANR，范围与测试图片已恢复/删除；欠完整断开→重扫路径。 |
...
| **MOB-70（L2）Flow retry staging 竞态** | 2026-09-11 | 本 commit | ✅ 三星真机通过 | 三星 72 MB 隔离视频在未修复 daemon 上已 fetch 后 `MaterializeIngest` 报 staging 源 ENOENT，手机三次重试进 `FAILED_NEEDS_USER`；源码排除“先 place 后 metadata”后确认是 Android RPC retry 与前一次 daemon materialize 重叠，共用 staging 路径。`FlowDelivery` 新增每 control peer async fetch 锁；等待 retry 读取前一轮的持久 receipt，不会重导出/删除同一文件。新增并发 retry 回归；`cargo test -p daemon --test flow_delivery`、`just ci` 全绿。当前 main daemon 下同一视频 3 秒 `CONFIRMED`、attempt 0；测试源、scope、后台开关和 device-idle 已恢复。 |
| **MOB-67（L2）失败系统通知接回 Flow 终态跃迁** | 2026-09-11 | `0fa1318` / 本验收 | ✅ 三星真机通过，已归档 | 根因考古：不是从未实现——UX-02（PR #36）曾在 BackupWorker 有完整通知设施，REBUILD-04（`d72ea44`）删 1124 行时随批次管线一起消失，`NotifyOnFailurePrefs` 只剩设置页自读。修法：`StrictConsumer.recordPermanentFailure()` 在 `ledger.update` 突变内部捕获 attempt-3 终态跃迁并返回；`FlowRunner` 跃迁后且开关开启时从账本数 `FAILED_NEEDS_USER` 总数发一条固定 id 通知（第二次失败是更新不是再响；`runCatching` 吞发送异常——账本先落盘，通知是补充渠道，MOB-37 裁决同构）；新 `FailureNotifier` 接口 + `SystemFailureNotifier`（沿用历史渠道 `ppass.backup.failed`/id 2027，用户已调的渠道设置不丢），渠道名进 en/zh 字典。瞬态重试（attempt 1/2）保持沉默。新增 `MOB67FailureNoticeTest` 3 例；全量 JVM **337/0/4 ignored**（65 XML 本次生成）、`just ci` 全绿、queue-check 绿。三星真实 Flow 受控断 daemon 后第三次失败显示系统通知「照片备份没成功」/「1张照片没备份成功，打开看看」；关闭开关后同一项再三次失败仍进 `FAILED_NEEDS_USER`，通知栏无 P-Pass 通知。手机测试源清理、相册范围/后台开关/通知权限/device-idle 均恢复。衍生发现：SENT-01/DOG-02b/MOB-29 三条通知发送端同批被删且至今零调用点（字符串与 ROADMAP 状态均与代码漂移），开 MOB-69。 |
| **MOB-68（L2）后台备份状态真实化** | 2026-09-14 | 本批次 | 🟡 代码完成，待鸿蒙 4.2 回归 | 用户意图、Android 标准白名单、MediaWatch 健康拆成独立事实；开关仅在三项满足时亮，拒绝/撤销/监听被清均转为设置页可处理状态，不再走跨 tab 黄条。onboarding 仅在用户选“开启后台备份”时申请白名单；通知权限完全移出 onboarding，仍只由失败通知设置开关主动申请。首装页两个选择等高，次要选择改为“暂不开启”并使用项目墨黑描边；扫码页移除绿色取景装饰。三星回归发现断开清理遗留 `setEnabled(true)`，导致重连后覆盖用户的“暂不开启”；现改为同时清除意图与 producer 状态，并新增 RED→GREEN 合同测试。Android JVM 355/0/0、debug APK、`just ci` 待本批复跑。 |
| **AUDIT-04（L2）审计核心重建：operation/item evidence/tombstone/decision** | 2026-09-10 | `97a33b6` + 三星验收 | ✅ 真机通过，归档 | 直接淘汰 AUDIT-01 的 `audit_event` 泛用表（新 migration `0006_audit_canonical.sql`，不迁移不兼容历史行），新建四张 canonical 表：`audit_operation`（一次操作终态，`operation_id`=Flow round 时即手机的 `round_id`，非另铸）、`audit_item_evidence`（每张照片的确认/失败/缺源/对账事实，`evidence_id` 幂等键）、`audit_tombstone`（asset_ref 非外键，删 asset 行不连带删 tombstone/evidence）、`audit_decision`（取消/继续/恢复/范围变更/撤销授权，引用 causal_operation_id/causal_object_ref）。新增 `crates/daemon/src/audit_route.rs` 作为手机 Flow outbox → 四表的唯一分发点：`flow.round.finished`→operation（`evidence_summary` 从实际持久化的 item evidence 重新聚合，不信手机自报的 `final_counts`，故意造假 payload 测试验证真实计数覆盖谎报值）；`flow.item.confirmed/source_missing/attention`→item evidence；`flow.reconciliation.resolved` 按 disposition 分流（UNRECOVERABLE→tombstone，NEEDS_DECISION→item evidence）；`flow.round.controlled/scope.changed/epoch.invalidated`→decision；未知 kind 兜底落 operation，绝不静默丢弃。全部路径幂等（INSERT OR IGNORE + 命中已存在也算 accepted，防手机重放卡死重试循环）。**card decision #5 硬边界修正**：原实现遗留的 `device.connected` 走 `last_audit_ts` 去重写入 `audit_operation` 违反"连接性从不进长期审计"的合同，本轮删除该写入路径与 `CONNECTED_AUDIT_DEDUPE_MS`/`last_audit_ts`，presence 改为纯 `last_seen` 刷新零审计副作用；`device.revoke` 新增 `audit_decision`（`revoke_authorization`，此前只有兼容层 operation 行，未满足 card decision #4"撤销授权是决定"）。Android：`AuditKinds` 新增 `ITEM_CONFIRMED`/`ITEM_SOURCE_MISSING`，`CompletionAndScope.acceptCompletionReceipt` 在与状态转换同一原子快照写入 item evidence（首次确认才写，receipt 重放不重复）、`StrictConsumer.skipMissingSource` 补齐此前完全没有审计轨迹的终态。验证：Rust `cargo nextest run --workspace` **377/377 passed 1 skipped**；Android `./gradlew testDebugUnitTest --rerun-tasks` **333/333，0 failures/errors**（64 个本次生成 XML）；`just ci`（fmt/clippy -D warnings/nextest/arch-check/queue-check/md-check/token-check）全绿。验收标准 8 条全部满足：以 current-main 重建安装的 Android + daemon 跑隔离图，新增 receipt 后 `audit_item_evidence`=1 且关联 `audit_operation`=1；精确外部删除 asset 后 asset=0、evidence=1、`external_delete` tombstone=1，归档。 |
| **TEL-03（L2）error 事件 taxonomy 设计与接线** | 2026-09-10 | 本 commit | ✅ 完成 | 用户裁决：错误码更细分（用户原话"多改代码就多改代码嘛"）+ 需要限流（用户原话"同一个错误反复发生，我们需要做限流，避免一直刷"）。`DeliveryError::Materialize` 拆成 `MaterializeStaging`/`MaterializeExport`/`MaterializeIngest` 三个变体（文件系统/blob 存储/索引是不同子系统，混在一起报错没有诊断价值）；`telemetry_code()` 方法把 8 个变体映射到固定英文码（`guard_mismatch`/`cancelled`/`invalid_request`/`fetch_failed`/`materialize_staging_failed`/`materialize_export_failed`/`materialize_ingest_failed`/`storage_failed`），绝不传出变体内部可能带路径的原始错误文本。`GuardMismatch`/`Cancelled` 不上报（正常控制流不是问题，上报会稀释信号）。`stage`=触发的公开方法名（offer/fetch/cancel），通过 `xxx_inner()` 包装模式在唯一出口处统一判断上报。`Telemetry` 新增 `error_dedup: HashMap<(code, stage), Instant>`，同一 key 在 5 分钟窗口（=FLUSH_INTERVAL）内只记一次，在 `record()` 门口拦截，仅影响 `Error` 事件。新增 6 个测试：`telemetry.rs` 3 个（10 次相同错误只入队 1 个、code 或 stage 任一不同不互相抵消、非 Error 事件不受影响）；`flow_delivery.rs` 3 个（空 file_name 触发 `invalid_request`/`offer` 阶段 + 红线扫描确认无原文泄漏、真实关闭 provider 连接触发 `fetch_failed`/`fetch` 阶段、连续 5 次相同失败通过完整 FlowDelivery 路径仍只产 1 个事件）。`cargo test --lib telemetry` 6/6、`--test flow_delivery` 13/13（10 既有 + 3 新增零回归）、`cargo nextest run --all-features` 365/365 passed 1 skipped、`just ci` 全绿。**遥测五件套（TEL-01/02/03/04）至此全部闭环**：`daemon_alive`/`conn`/`flow_item`/`first_byte`/`error` 全部真正接线到生产路径，OBS-02 裁决的字典 v2 完整落地。 |
| **TEL-04（L1）接线 first_byte 事件** | 2026-09-10 | 本 commit | ✅ 完成 | `QueryEngine` 同构接入 `Option<Telemetry>` + `with_telemetry()`。`thumb()` 缓存命中/生成/占位图三条路径均记录（每条都真返回字节）；`original()` 仅在成功读到文件字节后记录，`NotFound`/超尺寸/非图片分支不产出字节，故意不计入延迟（避免"记了没交付的东西"）。`blob_ticket()` 只发票据不传字节，不算 first_byte（真实传输走 TEL-02 已覆盖的 iroh-blobs 路径）。新增 `crates/daemon/tests/query_telemetry.rs`（3 测试）：成功交付恰好 1 个 `first_byte` 事件、未命中零事件、`enabled=false` 反证零网络。`cargo test -p daemon --test query_telemetry` 3/3；既有 `browse_flow`/`desk_flow`/`sync_flow` 5/5 零回归；`cargo nextest run --all-features` 359/359 passed 1 skipped；`just ci` 全绿。**遥测四件套（TEL-01/02/04）至此闭环**：`daemon_alive`/`conn`/`flow_item`/`first_byte` 全部真正接线到生产路径。仅剩 TEL-03（`error` taxonomy）——需要先跟验收人过一遍错误码粒度和去重策略，不是纯机械接线，暂缓。 |
| **TEL-02（L2）接线 conn/flow_item 到生产代码** | 2026-09-10 | 本 commit | ✅ 完成 | `FlowDelivery` 新增 `Option<Telemetry>` + `with_telemetry()` builder，`main.rs` 用已构造的 daemon 心跳同一个 `Telemetry` 实例接入。`fetch()` 记录：①`conn`——`fetch_from_observing_path` 回调里捕获 `ConnectionStatus` 同时算出遥测 `path`，整个 fetch 调用计时，成功 `fail_stage=None`/失败 `fail_stage="fetch"`；②`flow_item`——计时从 `fetch()` 函数最开头到 receipt 持久化前，`bytes` 取暂存文件大小（导出后、入库前，无额外 IO），仅成功路径记录一次；`resumed` 按 TEL-01 卡面允许暂恒 `false`（真实断点续传检测另开卡，注释写明不编造）。顺带修正 TEL-01 遗留的字段错误：`conn.path` 卡片草稿里的 `lan` 值在 `ConnectionStatus::as_str()` 实际枚举里不存在（真实值 `direct`/`relay`/`offline`/`unknown`），daemon 枚举注释 + Worker `schema.ts` 的 `z.enum` + 两侧测试同步改。新增 2 个测试（`crates/daemon/tests/flow_delivery.rs`）：真实 iroh-blobs fetch 后断言恰好 1 个 `conn` + 1 个 `flow_item` 事件字段正确；`enabled=false` 走同一路径反证零网络调用。`cargo test -p daemon --test flow_delivery` 10/10（8 既有 + 2 新增，零回归）；`cargo nextest run --all-features` 356/356 passed 1 skipped；`just ci`（fmt/clippy -D warnings/arch-check/queue-check）全绿。TEL-03（error taxonomy）、TEL-04（first_byte）依赖已满足，待实施；TEL-03 仍需先跟验收人过一遍错误码粒度。 |
| **AUDIT-01（L2）Flow 审计 v2：持久 outbox 与直接切换** | 2026-09-10 | 本 commit | 🟢 代码完成（含 review 后补齐的手机→daemon 投递管线），本地全量 `just ci` 绿；待真机验收 | `audit_event` v2 成为唯一长期审计事实源，整表 DROP 旧 `audit_log`（migration `0005_audit_event_v2.sql`），不迁移不兼容历史行。Android：`DiscoveryLedger` 的用户事实与 outbox event 原子快照提交（新 `AuditOutboxEvent`/`AuditKinds`），`AUDIT01LedgerOutboxTest` 锁死该契约；pause/continue/scope/epoch-invalidate/permanent-failure/reconciliation-resolved 六类事件全部接线。**投递闭环**：新增 `proto::FlowAuditSubmit`/`FlowAuditAccepted` + `flow.audit.submit` 方法，daemon `handle_flow_audit_submit` 按手机 event_id 幂等落库并回传被接受 id；Android `AuditOutboxDispatcher` 在每个 Flow 触发点之后异步 flush，仅对 daemon 确认的 event_id 调用 `acknowledgeAuditEvents`，未确认的留在 outbox 等下次重放（合同测试：`crates/daemon/tests/flow_audit_submit.rs` + `AuditOutboxDispatcherTest`）。Rust：`crates/storage/src/audit_repo.rs` 重写为 v2 repository（`event_id` 幂等键 + `INSERT OR IGNORE`），daemon 全部生产路径审计写入（pairing/reconcile/router/ipc）改为 v2 `kind`/`payload`；旧 batch `backup.started`/`backup.finished` 会话审计随之下线（不在 v2 事件矩阵内）。Desktop：`App.svelte` 活动页改读 v2 字段，依赖旧会话审计的「本周新备份/去重跳过」统计与「备份耗时」派生 UI 一并移除（无对应数据源，不编造数字）。验证：Android 333/333（较首轮 328 + 补齐 5 例）、Rust nextest 354/354（较首轮 346 + 补齐 8 例，`just ci` 全绿）、desktop-rust 18/18、desktop-vitest 57/57。真机验证（新活动文案/Flow 真实终态）显式列后续批次。 |
| **BLOB-01（L2）ingest 收件仓不再留双份** | 2026-09-10 | `0b6aadc` + 真机 | ✅ 真机通过，归档 | 同一 Android 设备重新配对后连续传 21 张：`originals` **292 KiB → 45,520 KiB**，`.ppf/blobs/data` **0 KiB → 0 KiB**，`.ppf/blobs` 总量稳定 **1,032 KiB**（空 redb store 固定开销）。新照片只落 originals，没有隐藏第二份 payload。 |
| **BLOB-02（L2）flow-blobs 周期 GC** | 2026-09-10 | `a123688` + 真机 | ✅ 真机通过，归档 | 自动 2 秒采样捕获 Flow 总仓 **564 KiB → 47,788 KiB 峰值 → 492 KiB**；payload `.data` **452 KiB → 47,468 KiB → 0 KiB**。完成后等待 70 秒（GC 周期 60 秒），44/44 completed；只余 19 个 `.obao4` outboard 元数据（136,832 B），无照片内容。 |
| **BLOB-03（L2）Android 发送端 provider store 回收** | 2026-09-10 | `ca6c5ed` + `85d6d4f` + 三星验收 | ✅ 真机通过，归档 | 根因不是「没接 GC」，而是导入走 `AddProgress::with_tag()`（落持久 named tag，GC 永不回收）。修复：`with_config` 用 `FsStore::load_with_opts` 打开 60s 周期 GC；`register_path_async`/`register_file_async` 改 `.temp_tag()`（ephemeral），`ActiveProvider.retained: Option<TempTag>` 在 `activate` 写入、`revoke()` 释放；新增 `release_retention()` 只掉 lease 的 tag、保住 endpoint/ALPN 复连（决策 5）。Kotlin 成功边界在 `NativeFlowDeliveryPort.acceptReceipt` 四字段校验通过后、推进 strict head 前调 `bridge.releaseRetention`。自动化反证：Rust `android_provider` **5/5**（含旧 named tag 升级迁移回收），Android JVM 4/4 + 10/10。三星覆盖安装修复版后，原 7 个 `CONFIRMED` provider data 文件 **4,100,446 B → 0 B**，ledger 7 条确认不变；余项仅为下次自然新增来源的机会性回归。**最终真机验收**：验收人授权隔离 Screenshots 测试，正常 Flow 后 ledger **7→9**，9 项均 `CONFIRMED` 且有 receipt；receipt 后与 75 秒后 provider `data` 都为 **0 文件**，三张测试截图已清理，归档。 |
| **REBUILD-07（L2）同一设备重配对 receipt 隔离** | 2026-09-10 | `0b6aadc` + 真机 | ✅ 真机通过，归档 | 现场旧 schema v3 升 v4，旧 24 条 completed receipt 保留；移除同一 Android 安装后重扫，新 epoch sequence **1–21** 全部 completed、无 `err.not_authorized`。稳定 NodeId 保持历史照片归属，epoch 隔离临时 Flow 序号。 |
| **REBUILD-07（L2）同一设备重配对 receipt 隔离** | 2026-09-10 | 本 commit | 🟡 代码完成，待同机重配对真机验收 | 同一 Android 安装的 NodeId 保持为历史照片来源身份；新 pairing epoch 是独立的 Flow 生命周期，故将 `flow_delivery` 主键从 `(node_id, queue_sequence)` 迁为 `(node_id, pairing_epoch, queue_sequence)`。迁移原样保留历史 receipt；grant/receipt 查询均精确带 epoch，既有同 epoch lease/hash/provider guard 与 completed receipt 重放不变。RED：旧 epoch completed 的 sequence 7 使新 epoch sequence 7 offer 实测 `GuardMismatch`；GREEN：真实 iroh provider/receiver 新旧两张 asset 均 indexed，storage **24/24**、daemon flow **8/8**、`just ci` 全绿。待桌面升级后执行同一手机移除→重扫→连续传图，再恢复 BLOB-01/02 磁盘回归。 |
| **TEL-01（L1）遥测字典 v2 schema** | 2026-09-10 | 本 commit | ✅ 完成 | 按 OBS-02 裁决把 `crates/daemon/src/telemetry.rs` 与 `infra/workers/telemetry/src/schema.ts` 同步改成字典 v2：`conn` 删 `ipver`/`country`/`isp_hash`；`backup_session` 改名 `flow_item`、删 `files`/`trigger`；新增 `error{code,stage}`。两端字段逐一对齐，无兼容层（生产当时只有 `daemon_alive` 在传，无历史数据负担）。验证：`cargo test -p daemon telemetry` 3/3 + `--test telemetry_flow` 2/2 全绿；`infra/workers/telemetry` `npm test` 14/14、`npm run typecheck` 绿；`cargo fmt --check`/`clippy -D warnings`/`cargo nextest` 345 passed 1 skipped/`arch-check`/`queue-check` 均绿，`just ci` all green。TEL-02（接线 conn/flow_item 到生产路径）、TEL-03（error taxonomy）、TEL-04（first_byte）依赖本卡，尚待实施。 |
| **DESK-14/DESK-15/MOB-63/MOB-65 真机验收批次关卡** | 2026-09-10 | `3729fb6`（macOS 重建）+ 三星真机 | ✅ 全部通过，已移入 `cards/done/` | macOS 重建 `/Applications/P-Pass.app` + 三星重装 debug APK（均基于当日 main HEAD）后集中验证：DESK-15 五组件在真实 Tauri 窗口视觉正常；DESK-14 拖拽区正常，且发现并修复顺带问题——`tauri.conf.json` 缺 `hiddenTitle: true` 导致原生标题文字与侧栏品牌重复显示，加此字段后重建验证通过；MOB-63 暂停/最后完成回执两种交错时机均收敛 Idle；MOB-65 传输中关自动备份不中断当前轮、只停自动唤醒、重开恢复自动。同时发现新问题顺手开卡：MOB-67（"备份失败时通知我"开关是死开关，从未真正发送系统通知）。 |
| **MOB-65（L2）自动开关与当前轮暂停解耦** | 2026-09-09 | `27a6e66` | 🟡 代码完成，待共享真机验收 | `AutoBackupPrefs` 改为唯一的 `autoEnabled` 策略事实，并兼容旧 `paused` JSON；关闭开关只取消周期/前台/进程/媒体等自动 wake，删除对 Flow `pauseFlow`/`continueFlow` 的调用，显式 manual work 不受影响。现有当前轮和英雄区 Pause/Continue/Cancel 状态机零改动，故开关不会再伪造“继续/取消当前轮”。反证：临时重新引入 `pauseFlow(`，新守卫实测 **8 tests / 1 failed**；还原后 Android JVM **62 XML / 322 tests / 0 failures / 0 errors / 4 skipped**，debug APK 与 `just ci` 通过。待真机核实空闲开关无轮次按钮、在传轮次收尾且后续自动 wake 停止。 |
| **DESK-15（L2）桌面设计系统收口：基准/token 链 + 五个基础组件** | 2026-09-09 | 本 commit | 🟢 代码完成，本地全量 `just ci` 绿；跨断点自动化回归明确不做 | 排查推翻卡片原始假设——DESK-07 早把 shadcn 语义槽位接到 `--pp-*`，颜色/圆角不是缺口；真正缺的是尺寸下限消费、派生链检查、组件收口。①基准：`AGENTS.md`/v2 README 过时的 v1 指向改成 v3。②新增 `tools/check-token-drift.py` 接进 `just ci`（真测过能抓漂移）。③Button/Card/Dialog/Notice/NavItem 五个组件收口：App.svelte/Wizard.svelte/WizardWindows.svelte 里全部 `BTN*` 常量、裸 `<button>`、`.primary`/`.link-more`/`.modal-backdrop`/`.modal`/`.message`/`.nav-item` 等重复视觉 CSS 已删，Card 的 10 处重复 override（`rounded-xl border shadow-none ring-0 ring-transparent`）收成 `size=default\|flush` + `variant=default\|danger`。④轻量门禁：Button/Card 各有一份合同测试，两份都验证过真会抓 bug（子串匹配踩过 `min-h-11` 包含 `h-11` 的坑，改成精确 token 匹配后故意改错重跑确认变红）。⑤新增 `assets/design/token-reuse-checklist.md`，逐类核对复用/缺口判断。`pnpm build` 0 error、`pnpm test` 58/58、全量 `just ci`（cargo fmt/clippy/nextest 345 passed/1 skipped/arch-check/queue-check/md-check/token-check）全绿；浏览器实测 Button 四变体、Dialog 开关、Notice 开关、NavItem 两档宽度、Card 三变体的真实计算样式，与迁移前像素一致。挂账：Button 图标动作变体移 backlog（低优，`docs/QUEUE.md` 八）；跨断点/Dialog 自动化像素回归需要新引入 Playwright 类工具，用户拍板暂不做，不是遗漏。 |
| **DESK-14（L3）macOS Overlay 透明拖拽命中区** | 2026-09-09 | 本 commit | 🟡 代码完成，待共享桌面回归 | 以行业通用的透明 hit area 替换原先给侧栏/向导业务容器打标的拼接方案：主界面与向导各有 `#app` 直系 32px 空元素，macOS 才由 `titlebar.js` 直接标记为 Tauri drag target；绝对定位、无背景、不占布局，保留原 34/40/44px 顶部留白，视觉无新色带。Vitest **47/47**、`pnpm build` 均通过；Tauri dev 启动到运行态。macOS Accessibility/Screen Recording 权限未授予，默认与 <1080px 的真实拖拽/截图未冒充完成，留待共享回归。 |
| **CI-05（L1）Rust 工具链检查放行 CI-02 的 artifact consumer** | 2026-09-09 | 本 commit | ✅ 完成 | `tools/check-rust-toolchain-ci.sh` 的 `e2e.yml` 期望 job 集从 `{"e2e", "scenarios"}` 改为 `{"build"}`：CI-02 把 release 编译收敛到 `build` job 后，`e2e`/`scenarios` 只 `download-artifact` 运行已构建二进制，不跑 Cargo，不该被要求工具链 setup；`build` job 本身仍受检查覆盖。反证：临时删掉 `build` job 的 `Set Rust toolchain` 步骤，检查器正确报错退出非零；恢复后 `bash tools/check-rust-toolchain-ci.sh` 退出 0（`Rust CI toolchain check passed: 5 workflows`）。`just ci` 全绿（fmt/clippy/nextest/arch-check/queue-sync）。 |
| **MOB-64（L2）设备撤销后的被动反馈** | 2026-09-09 | 本 commit | 🟡 代码完成，待共享真机验收 | Flow delivery 的既有通用失败 catch 继续记录永久失败；仅 `err.not_paired` 同时记为当前 pairing epoch 的失效事实。Holder 在既有刷新中投影到原 `pairingLost` 红卡，未新增 UI 状态、轮询或 daemon 改动。JVM `MOB64RevokedFlowDeliveryTest` **2/0/0/0**：拒绝置位、普通网络错误不置位；隔离副本 `just ci` 绿。共享主工作树同期 SYNC-05 的未完成 JVM 测试未计入本卡验收。待撤销设备后发起下一次手机业务调用的真机回归。 |
| **CI-02（L3）e2e nightly release 二进制只编一次** | 2026-09-09 | `c4c2394` | ✅ 完成 | `e2e.yml` 新增 `build` job（`cargo build --release -p daemon -p testclient` 只跑一次，之前 `e2e`/`scenarios` 两个并行 job 各编一遍，同 run 内 cache 互相看不见），打包 `release-bins.tar.gz`（tar 保留可执行位，规避 upload-artifact zip 打包丢 +x 的已知坑），两个测试 job 各自 `needs: build` + `download-artifact` + `tar -xzf` 解包并 `test -x` 断言。`e2e`/`scenarios` 仍是两个独立 job，失败隔离不变（未合并成一个 job）。`actionlint` 零告警。实跑 `workflow_dispatch`（[run 34317704548](https://github.com/hawkeye-xb/P-Pass/actions/runs/34317704548)）：三 job 全部 `success`，两个下游 job 的 unpack 步骤均确认可执行位存活。 |
| **LINT-01（L3）Android lint 补 CI 门禁** | 2026-09-09 | 本 commit | ✅ 完成 | `ci-android.yml` 新增 `lintDebug` 步骤（`testDebugUnitTest` 之后、`assembleDebug` 之前）。BucketScreen.kt 点名的 `ProduceStateDoesNotAssignValue` 及另外 3 处同模式误报（PhotosScreen×2、VideoScreen×1，均为赋值裹在条件分支/提前返回里，lint 静态分析看不穿）显式 `@Suppress` 并写理由；AndroidManifest 的 `PermissionImpliesUnsupportedChromeOsHardware` 是真错误（CAMERA 权限缺 `uses-feature required=false`），已修。首次开门禁跑出 0 errors/72 warnings，72 条存量告警按规则分类记录"为什么本卡不修、归哪类后续工作"（UnusedResources 23/GradleDependency 16/PluralsCandidate 12/Recycle 10/InlinedApi 4/AutoboxingStateCreation 3/其余各 1）后全量收进 `lint-baseline.xml`。反证：临时插入 API 31+ 类引用（无版本守卫）→ `BUILD FAILED` 退出码 1，确认后撤回；最终 `./gradlew :app:lintDebug` → `BUILD SUCCESSFUL` 退出码 0。验证环境 `JAVA_HOME=openjdk@17` 与 CI 对齐。 |
| **MOB-63（L1）暂停与最后完成回执竞态** | 2026-09-09 | 本 commit | 🟡 代码完成，待共享真机验收 | 仅在原子账本转换后没有 `QUEUED`/`TRANSFERRING` 项时归位 `OPEN + IDLE + 无 lease`：`pauseByUser()` 覆盖 receipt 先到的无 lease 分支，`acceptCompletionReceipt()` 覆盖 Pause 先到的迟到回执分支；未改 UI 投影、取消路径或 wire 协议。新增双顺序单项测试均断言 `Idle + AllSafe`，两项反证锁住有可续传项时仍为 `PausedByUser` 且 command 为 Continue。RED 3 项中 2 项失败 → GREEN；定向 XML **3/0/0**，全量 Android JVM **317/0/0/4 skipped**（61 个本次生成 XML），`just ci` 全绿。真机验收欠回执先后两种操作时机。 |
| **BLOB-02（L2）flow-blobs 收件仓周期性 GC** | 2026-09-09 | `a123688` | 🟡 代码完成，待共享真机回归 | `transport::Blobs::open_with_periodic_gc` 新增带 GC 的构造路径，只对外暴露 `[u8;32]` 哈希集合回调（不泄漏 `iroh_blobs` 类型）；`storage::Db::active_flow_content_hashes()` 查 `flow_delivery` 表 `state='active'` 的 `content_hash` 集合作为唯一保护判据；`main.rs` 以 60s 周期接入。反证与真实 iroh 传输单测均绿：`transport::blobs_resume` 2 个（含回调清空后中途 Active grant 被判定应删的反证）、`daemon::flow_delivery` 1 个（真实 provider/receiver + FlowDelivery 全链路）、`storage` 1 个（completed/cancelled 不被误判受保护的反证）。`just ci` 全绿。真机 `du -sh .ppf/flow-blobs` 峰值→GC 后回落的观测留给共享真机回归。 |
| **SYNC-05（L1）AssetMeta 来源设备字段，移除 Android 归属影子状态** | 2026-09-09 | 本 commit | ✅ 完成 | Rust `AssetMeta`/Kotlin wire mirror 新增可选 `src_device`；daemon 仅在合法 32-byte source NodeId 时输出完整 hex，旧 frame 缺字段安全解为 `None`。PhotosScreen 删除 `backup-state` 与 `flow-state` hash 集合读取，筛选器仅以 `src_device` 与本机 NodeId 分类；`null`/空字段只在「全部」显示。反证测试实际删除 `backup-state` 后仍正确分类。`just test` 退出 0；Android 全量 JVM **314/0/0/4**（60 个 fresh XML）。 |
| **验收状态批次（UI-08 / MOB-47 / MOB-09 / MOB-19 / DESK-10 / UI-04b）** | 2026-09-09 | 本 commit | 🟡 已按证据收口 | UI-08 与 MOB-47 获验收人通过、移入 `done/`；MOB-09/MOB-19 的旧批处理验收被当前 Flow 生产路径取代，归档且由 MOB-61 承担现行“缺源不重试”风险；DESK-10 的真实正常包通过，daemon 不可达分支仍需隔离实证；UI-04b 脱离文档流方向正确但视觉不通过，退回重做。当前 main Android JVM **314/0/0/4 skipped**（60 个 XML，本次生成）。 |
| **BUILD-04（L1）本地构建缓存与已合入 worktree 回收** | 2026-09-09 | `e85994f` | ✅ 完成 | 新增 `just cleanup-local` / `tools/clean-local-builds.sh`：默认只预览；删除必须 `--apply` 加显式 scope。`--targets` 仅清可再生 `target/`，`--worktrees` 仅移除已注册、clean、已合入 `origin/main`、非当前且无活动构建的 worktree；其余逐项说明跳过原因。临时 Git fixture 覆盖 preview 不删、cache-only 保留源码、merged clean 删除、dirty/unmerged/active build 保留；`just test-cleanup-local` 与 shell 语法检查通过。 |
| **NET-05（L2）Flow 数据面路径状态随传输生命周期上报** | 2026-09-08 | 本 commit | 🟡 代码完成，待共享真机回归 | 不再把设备最近存活的任意 ctrl/数据连接当成正在传输的路径：`FLOW_FETCH` 经授权后登记 `flow_connection=unknown`，`connect_raw(provider, ALPN_BLOBS)` 成功后读取 iroh selected path 并上报 `direct/relay/unknown`，成功、错误、取消与旧 lease 终止均按精确 lease 清回 JSON `null`；三次有效变化各发 `device.changed`。IPC 保留既有 `connection`/presence 语义，新字段只代表 active Flow；桌面仅在新字段非空时展示「正在连接/传输」「正在直连传输」或「正在经中继传输」。反证：旧 lease 清新 lease 的测试先红；transport loopback callback、daemon lifecycle/IPC、桌面纯函数均覆盖。`just ci`、transport/daemon focused tests、桌面 build + Vitest **43/43** 全绿。真机待以慢速大文件确认状态依次出现且 Pause/Cancel/失败无残留。 |
| **UI-08（L3）选相册长名称截断与缩略图清晰度链路** | 2026-09-08 | 本 commit | 🟡 代码完成，待共享真机回归 | 根因：`BucketScreen` 原先对相册封面无条件请求 `loadThumbnail(Size(200, 200))`，但网格卡按实际显示像素绘制，导致低分辨率位图被放大；长名称 `Text` 使用非填满权重且无单行溢出约束，Row 测量可换行撑高。修复：`onSizeChanged` 读取封面 Box 实际 `IntSize`，请求与缓存 key 均带显示宽高；名称改 `weight(1f)` + `maxLines=1` + `TextOverflow.Ellipsis`，未改选中集合/桶计数语义。新增 `UI08BucketScreenTest` 2 条源码守卫，RED 2 failures → GREEN；Android JVM 全量 **313/0/0/4 ignored**（`--rerun-tasks`，60 个 fresh XML）。截图和真实清晰度证据按要求留给共享真机回归。 |
| **UI-04b（L2）桌面改名反馈统一为官方 Sonner 通知** | 2026-09-10 | 本 commit | ✅ 验收完成 | 验收人先否决手写黑色 Toast 和设备行旁 Message，最终在真实 Tauri 窗口确认 shadcn-svelte 官方 Sonner 的右上角形态：改名成功/失败及既有短反馈统一走 `svelte-sonner`，不参与文档流。官方组件保留形态/动效；其 success/warning/error 类型直连 `safe/waiting/act` 三种设计 token（绿/黄/红），普通色通知被消除。用户实窗确认绿色改名成功通知正常、无标题或设备行位移；桌面 Vitest **57/57**（26 files，0 failed）与 `pnpm build` 通过。 |
| **UI-04a + UI-04c（L2）全局提示容器与最高优先级单条呈现** | 2026-09-08 | 本 commit | 🟡 代码完成，待共享真机回归 | 两张卡共享同一提示呈现层，合并落地：`ui/HomeNotices.kt` 新增 `NoticeHost`（把电池白名单/通知引导/中断恢复/取消轮入口/重传告知五条提示的输入集中构造候选列表 → `topNotice` 只渲染最高优先级一条，其余收起），`TwoTabs` 顶部加 `notice` slot 让提示在 Photos/Backup 两页都可见（中断提示不再只有总览页），`MainActivity` 把五条输入接进 `NoticeHost`，`HomeScreen` 删掉五段局部渲染块（触发条件逐字保留，只迁呈现）。优先级口径：阻塞备份 > 需要授权 > 补充信息。反证单测 `priority_selection_is_not_list_order` 锁死「选择行为」而非「列表顺序」。Android JVM **309 / 0 failures / 0 errors / 4 skipped**（59 XML 本次生成，HomeNoticesTest 6/0/0），`just queue-check` 通过。真机欠账：所有 tab 见中断提示、同时多条件只显示一条最高优先级——不移动卡片到 done/。 |
| **MOB-62（L2）断开重扫清空旧 Flow 会话** | 2026-09-08 | `8430991` / 本 commit | 🟡 本轮三星验证通过，待共享回归 | 三星实证主动断开→重扫会 ANR；清理路径原先只删 pairing/confirmed cache 和一个 periodic work，遗留 `flow-state/<NodeId>`、native provider/runtime 及 catchup/manual/process/media-watch wakes，重扫同 NodeId 时旧 offer 重放。现已关闭/移除 runtime、删旧 Flow ledger、取消全部 wake 与 media watch，并把清理移到 IO；Android JVM + debug APK、`just ci` 通过。本轮 Samsung SM-S9210 已完成主动断开→新单次配对串→daemon pending 立即允许→选相册，无 ANR、崩溃或旧 offer UI；未点开始备份，统一回归补首轮传输。 |
| **MOB-61（L2）手机源删除时终态跳过** | 2026-09-11 | 本 commit | ✅ 三星真机通过 | 当前 main debug APK 覆盖安装后，三星 SM-S9210 在隔离相册将已持久入队的照片从系统存储删除：同一生产 Flow 落为 `SKIPPED_SOURCE_MISSING` + `MISSING` + `UNRECOVERABLE`、lease 清空，App 未崩且未入 `FAILED_NEEDS_USER`；新增后续隔离照片 **2 秒** `CONFIRMED`。冷启动 Settings/Home 显示「已跳过 1 张已从手机相册删除的照片，不会再重传。」且无重试动作。Android JVM **337/0/0/4**、`just ci` 全绿；手机测试源已清，明确测试副本留在桌面库作证据。 |
| **NET-01（L2）蜂窝热点 relay 对照** | 2026-09-11 | 本 commit | 🟡 等 OPPO 复现，降出当前槽位 | 三星 SM-S9210 连手机蜂窝热点、Mac 保持原网络；当前 main daemon 的 `transport=debug` 记录 inbound 为 n0 `Relay`。强制生产 `MediaWatchJob` 后，已选范围的隔离照片 `CONFIRMED`（attempt 0），daemon 审计有 `ingest.new`，两端无 timeout/`ConnectionLost`。这是 relay 成功对照，不是原始 15 秒失败的反证；在 OPPO 不在场且没有新失败时，NET-01 移入「待你复现或拍板」，不占当前开发槽位。手机测试源、后台开关和 device-idle 白名单均已恢复。 |
| **I18N-02（L1）主 Android Kotlin 可见中文迁移** | 2026-09-08 | 本 commit | ✅ 完成 | 逐处确认 `MainActivity.kt` 更新提示四处、`PairFlow.kt` 配对失败/不可解析/存储端默认名三处为用户可见文案；`BackupWorker.kt` 无用户可见中文。新增非注释代码中文守卫并将文案迁入对称 `values/` + `values-zh/` 资源。TDD 红测命中 4 处硬编码后转绿；Android JVM **309 tests / 0 failures / 0 errors / 4 ignored**，i18n focused **3/3**，`just ci` 全绿。 |
| **CI-03（L0）桌面壳 workspace 补 fmt/clippy 门禁** | 2026-09-08 | `815bf43` | ✅ 完成 | `ci-desktop.yml` 在 `cargo test --lib` 前加 `cargo fmt -- --check` + `cargo clippy --all-targets -- -D warnings`（working-directory `apps/desktop/src-tauri`），一次性 `cargo fmt` 修掉既有漂移（ipc.rs 两处：subscribe_events 签名折行、std::fs::write 调用折行）。反证：故意未格式化行 → fmt 步骤 exit 1（证明门禁在看这个 workspace）；clippy 0 告警、lib 测试 18/18 通过、actionlint 绿。`just ci` 语义不变。 |
| **REL-06（L3）历史 v0.3.1 Release 资产恢复取消** | 2026-09-08 | 本 commit | ✅ 用户决定归档 | `v0.3.1` tag 保留；验收人明确决定测试期 Release 产物清理后无需恢复，故不从 Release #24 下载或上传缺失的 7 个资产，现有 2 个资产维持现状；不改 `v0.5.0-test.4`、`dogfood` 或任何源代码。队列重排为「待你真机验收 → 可接 → 待复现/拍板 → 已完成/归档」，MOB-51 置于真机验收首项。 |
| **NET-04（L2）连接缓存与路径追踪** | 2026-09-08 | 本 commit | 🟡 代码完成，待真机 | Android `DaemonClient` 提升到 `PPassApplication` 进程级实例，Main/Flow delivery 注入同一 Endpoint；daemon transport 用 `(NodeId, ALPN)` 缓存单条活连接、关闭观察与 120 秒空闲回收，`path_of` 按 ALPN 返回既有 `conninfo` 分类，`Blobs::fetch_from` 经缓存连接开 stream。兼容性回归：router 让每条拒绝 stream 终止，但后续 stream 必须重新过 authz，避免连接复用改变 revoke→re-pair 行为。RED：同 key 二次 ctrl stream 原先另起连接；GREEN：loopback 确认同 key 复用、不同 ALPN 分开且两个 `path_of` 均为 LAN。已过 `cargo test -p transport`、关键 daemon pairing/revoke/flow delivery 测试、Android JVM **308/0/0/4**、`just ci` 全绿；三星真机初跑暴露 native blobs provider 每项新建 Endpoint（5 个不同 NodeId），随即将 provider 改为生命周期内单 Endpoint + 单 store，并让相邻已完成项不再 revoke。修正后 `NET04-test-e` 5 项全为 `CONFIRMED`（attemptCount=0、各有 receipt）；daemon debug 日志只有一次 `ppf/blobs/1` connection / hole-punch，后续 5 个 `iroh_blobs::get::fsm` 请求走其上。Pause/Cancel/Retry 真机回归仍待补。 |
| **MOB-26（L2，合并 MOB-45）统一 Android 媒体查看器与系统返回** | 2026-09-08 | 本 commit | 🟡 代码完成，待 Mate 60 验收 | 查看器以冻结的过滤后页序打开，官方 `HorizontalPager` 在图片与既有 Media3 视频页之间切换；Telephoto `zoomable` 负责图片缩放/嵌套横向手势，未缩放且下拉 ≥96dp 才关闭。查看器 `BackHandler` 先关闭网格覆盖层，顶层 Screen reducer 只拦二级页（手动配对、存储详情也各自优先消费），根页继续由 Android 退出。MOB-45 归档为已并入。RED→GREEN：新增页序/返回/缩放门控与 Screen 返回表；Android JVM **307 / 0 failures / 0 errors / 4 skipped**（`--rerun-tasks`，59 XML 本次生成），`assembleDebug` 成功；Telephoto 令 debug APK **48,934,824 → 49,642,264 bytes**（+707,440，+1.4457%）；`just ci` 全绿。`lintDebug` 在本机 JDK25 复现既有 BUILD-01，未伪称通过；真机录屏仍欠。 |
| **队列核实批次（E2E-03/REL-05/BUILD-03/CI-04/MOB-39/42/48/41 关闭 + DOG-03/MOB-46 拍板 + MOB-44 删除 + UI-04 拆分）** | 2026-09-07 | `bc79a83`/`2f4fe72`/本 commit | ✅ 纯文档 | 用户质疑队列卡片是否为旧文字，逐条用 GitHub Actions 实际运行记录 + 代码走查核实：①**E2E-03/REL-05/BUILD-03** 已在 `v0.5.0-test.2~test.7` 真实 Release/E2E 运行中验证通过，非待办，移入 `done/`；②**CI-04** 用 Release #51 实测缓存与拆分上传均生效，移入 `done/`；③**MOB-39/MOB-42/MOB-48**（原"冻结待 ARCH-01 重拆"）源码走查确认 REBUILD-00~05 生产切换已落地，`BackupWorker` 已是纯 wake adapter、`pauseAutoBackup` 已覆盖全部通道、Pause/Continue 语义已转移到 `backup/flow/`，旧问题域不存在，移入 `done/`；④**MOB-41** 同批核实，点名函数（`calibrateIfReachable`）已不存在，`calibrateConfirmed`/`planReuploads`/`noteReuploadNotice` 生产代码零调用，移入 `done/`；⑤用户拍板：**DOG-03** 不做（不进 onboarding），移入 `backlog/`；**MOB-46** 上一代问题不再追，移入 `done/`；**MOB-44** 删除（鸿蒙 NEXT 不支持后台运行，产品范围排除）；⑥**UI-04** 按用户"一个任务一条"要求拆分为 UI-04a（中断提示仅总览可见）/UI-04b（改名反馈占布局）/UI-04c（多提示无优先级）/UI-04d（重传通知渠道名不对）四张独立卡；⑦**CI-03/BUILD-01** 现场复核仍未开工/仍可复现（`JAVA_HOME=$(brew --prefix openjdk)` 即 25.0.1 时 `lintVitalAnalyzeRelease` 报错依旧），卡片文字更新为"已复核仍存在"。全程 `just queue-check` 通过。 |
| **SITE-03（L1）备份核心重建故事** | 2026-09-07 | `07f7a97` | ✅ Pages 已发布 | 新增中文工程复盘《为什么我们把备份核心整个换掉了》：从旧批次模型无法回答单张照片状态出发，说明手机逻辑账本、原子发现、严格队头、原生 fetch/resume、完成凭据，以及 Pause/条件等待/取消/对账的不同语义；只引用 ARCH-01 / REBUILD-00~06 公开归档，不把真机回归包装成结案。`npm ci && npm run build` 成功，新文章路由、RSS 与 sitemap 均在产物中；Pages workflow `34102057353` 成功，线上文章、RSS、sitemap 均 HTTP 200。 |
| **MOB-60（L1）取消当前轮完成后仍展示暂停/继续** | 2026-09-07 | 本 commit | 🟡 代码完成，等真机 | 用户复测 MOB-58/59 时追问逼出根因：`cancelCurrentRound()` 把取消前的安全前置暂停（`pause()`）误当作取消完成后的终态保留，导致 `flowUiStateOf` 永远命中 `PausedByUser` 分支，走不到已有的 `Idle` 兜底——取消完之后没有 `QUEUED`/`TRANSFERRING` 项，跟"从未开始"或"已传完"事实等价，理应直接复用现有判断链路落到 `Idle`。GREEN：`finishRound()` 后追加 `continueFlow(constraintsSatisfied = true)`，复用已有归位路径，不新增状态、不新增判断分支；`HomeScreen` 的"取消当前轮"按钮渲染条件本就是 `state is BackupUiState.Paused`，闸门归位后自动消失。改写 `REBUILD03FlowRunnerTest`/`REBUILD04WorkerCutoverTest` 两处原本断言"取消后仍是 PausedByUser"的用例（这两条断言本身编码了要修的错误行为）。Android JVM **298/0/4**（净变化：0 新增/0 删除，2 处断言改写）、`just ci` 全绿。真机复核待补。 |
| **MOB-51（L1）英雄区暂停无轮级粘性** | 2026-09-06 | `a876195` | 🟡 代码完成，等真机 | RED（stub 复刻今日行为，2 条缝隙用例真红）→ GREEN：`flowRoundActive` 从持久账本推导轮级粘性（gate OPEN ∧ lease∨QUEUED/TRANSFERRING），不新增调度真相字段；`backupUiStateOf` 统一 snapshot→首页态生产链，缝隙渲染真实计数不编假进度；`flowCommandOf` 点击路由与文案读同一事实（可见 Pause 必暂停）。反证恒 false → 3 用例真红（先 commit 再反证，checkout 单行还原）。Android JVM **274/0/4**、`just ci` 全绿、debug APK 出包。 |
| **UI-09（L2）聚合状态必须读 Flow 账本** | 2026-09-06 | `8a02271`（tag v0.5.0-test.5） | 🟡 真机部分通过，余项挂 MOB-51 | RED（4/4 真红复现验收观察）→ GREEN：`completedAt` 于账本提交点盖章（回放不漂移）；`flowAggregateOf/flowIsAllDone` 纯函数派生；Holder 换源、`ConfirmedStore` 读路径清零；Idle+全确认 → AllSafe 首次可达。反证恒零投影 → 3 用例真红。Android JVM **268/0/4**、`just ci` 全绿。真机 test.5 反馈：进度数字（20/30、待备份）**实时跳动——换源达成**；暂停/继续入口不可达为独立缺陷 → 派生 MOB-51；同轮反馈另派生 MOB-52（OPPO 闪退 L0，等崩溃证据）、DESK-11（桌面摄入不实时）、DESK-12（摄入丢拍摄日期）、MOB-45 补侧滑样本。环境：本机补装 `aarch64-linux-android` target + NDK 27、JDK 注册表仅剩 17。 |
| **MOB-47（L2）跨端视频预览** | 2026-09-02 | `97fcdd3` | 🟡 代码合并，待真机 | 桌面以 hash→daemon 记录→canonical regular file→精确 `allow_file` 加载原视频，拒绝任意前端路径；视频失败回退 `thumb.get`。generation + keyed remount 阻止旧 video error 覆盖新 viewer。Android 换 Media3 ExoPlayer/PlayerView 并在 `DisposableEffect` 释放。独立 L2 复审通过；桌面 Vitest 40、Tauri lib 18、Android JVM 261/0/0/4，欠双端真机。 |
| **REBUILD-06（L2）Flow offer/fetch 授权与 receipt 一致性** | 2026-09-02 | `b038682` + `f322443` + `df82bca` | ✅ 三星通过 | 真机将 `err.not_authorized` 精确收敛为 completed grant 的 recovered lease 不同；同 epoch + content hash 才可 rebind lease/provider 并回放同一 durable receipt，epoch/hash 不同仍拒绝。已认证 member 的 `hello` 返回当前 epoch，Android delivery preflight 可刷新陈旧 pairing。新队头 30→31 `CONFIRMED`，并以 512MB 隔离测试媒体通过 Pause → kill → reopen → Continue → Cancel。Rust flow/pairing/proto 全绿；Android JVM + debug APK 全绿。 |
| **MOB-49（L1）取消本轮后 UI 永久卡死** | 2026-09-03 | 本 commit | 🟡 代码合并，待真机 | `FlowRunner.cancelCurrentRound` 现于当前账本窗口取消终态化后调用 `finishRound()`，清除活动 `CancellationRound`，UI 回到 `PausedByUser`，取消项仍保留 `CANCELLED_BY_USER_ROUND`。RED 命中，去掉唯一接线反证再次失败；Android JVM 263 / 0 / 0 / 4、debug APK、`just ci` 全绿。真机端到端项与 MOB-50 的 cursor 修复合并后，以隔离测试相册一次验证。 |
| **MOB-50（L1）取消本轮后 upload cursor 卡死** | 2026-09-03 | 本 commit | 🟡 代码合并，待真机 | `CancellationRoundController.startPausedRound` 的同一原子账本提交现从取消映射后的队列选取首个 `QUEUED` cursor，无项则 `INITIAL`，不改变 `StrictConsumer` 的严格游标契约。RED：取消→新发现→wake 无法启动新项；去掉重算反证再次失败。Android JVM 264 / 0 / 0 / 4、debug APK、`just ci` 全绿；待与 MOB-49 在隔离测试相册完成组合端到端验收。 |
| **REBUILD-05（L2）对账语义：迟到完成回执被 Pause/Cancel 竞态误判丢弃** | 2026-09-02 | 本 commit | ✅ 三星真机自然复现通过 | 只读比对 Desktop `flow_delivery`（sqlite）与手机 `discovery-ledger.json`（adb run-as，无写操作）定位根因：`CompletionAndScope.acceptCompletionReceipt` 把「fetchLease 已被 Pause/Cancel 清空」误判为「被更新尝试取代」，直接丢弃 Desktop 迟到的完成回执，产生卡面所述的「Desktop 多 1 条历史 completed grant」。RED：新增测试模拟 Pause→Cancel 后 Desktop 迟到回执，先跑出真实 `AssertionError`。GREEN：仅当存在**另一个活跃 lease**（同队列位、不同 token）才算被取代拒绝；lease 已清空不再算取代，迟到回执正常确认并清除 `cancellationRoundId`。反证：真正被新尝试取代时仍必须拒绝（防恒真式）。验证：Android JVM **260 tests / 0 failures / 4 skipped**、`assembleDebug`、`just ci` 全绿。**2026-09-02 三星真机验收**：向测试相册连续导入 15~500MB 合成媒体制造真实 Pause/toggle 竞态，item #38（120MB）自然出现目标场景——Desktop 已 `completed` 写入 receipt，手机仍 `QUEUED` 且 `completionReceiptId` 已 stamp；点击「继续」重新签发 lease 后 4 秒内收敛为 `CONFIRMED`，回执 id 保持一致。最终手机 38 项/37 `CONFIRMED`，与 Desktop 37 个不同 content hash 一致，验收标准全部达成。验收中另发现两个独立衍生缺口（不在本卡修复，已分别开卡）：MOB-49（取消本轮后 UI 永久卡死，`cancellationRound` 无生产清除路径）、MOB-50（取消本轮后 `uploadCursor` 不重置，后续新项永久卡 `QUEUED`）。 |
| **REBUILD-05（L2）Flow 范围扩展补扫接线** | 2026-09-02 | `180d453` | 🟡 scope backfill 真机通过；跨端收敛待 REBUILD-06 | 保存范围扩大时，持久 ScopeRevision + historical-backfill boundary/progress；Flow 不移动 live cursor、不打断 strict head，stableId 去重。三星将原 26 ledger 项补到 30 `CONFIRMED`（revision=2、无遗留请求）；随后为 Pause 加入的测试媒体暴露 `flow.fetch` 15s timeout 与 `flow.offer err.not_authorized`，不清数据掩盖，已开 REBUILD-06。Android JVM **255 / 0 / 4**、debug APK。 |
| **REBUILD-04（L2）Worker Flow cutover 与最小状态呈现** | 2026-09-02 | `df82bca` | ✅ 三星通过 | 修复后新队头得到确认；512MB 隔离测试媒体传输中 Pause 后强杀 App，重开仍为 `PAUSED_BY_USER` 且无 lease；Continue 取得原队头，第二次 Pause 后 Cancel 仅写 `CANCELLED_BY_USER_ROUND`，32 项 `CONFIRMED` 未变。 |
| **REBUILD-03（L2）新生产 Flow runner 与触发接管** | 2026-09-01 | `0772afb` | ✅ 代码完成（Android JVM **392 tests / 0 failures / 4 skipped**；debug APK；`just ci` + deny） | `FlowRunner` 将 discovery request、原子入账、严格单头、Android native provider、Desktop fetch/receipt 与账本确认接成同一新路径。process wake / MediaWatch 只请求 Flow discovery；旧 Worker 未承载任何新 Flow 逻辑，R4 再降为 framework wake adapter 并接 UI/三星验收。 |
| **REBUILD-01（L2）Android iroh-blobs provider bridge** | 2026-09-01 | `8274190` | ✅ 代码完成（provider 1/1；Android focused 2/2；debug APK） | `transport` 以 Android `cdylib` JNI 实装 `FsStore` / `BlobsProtocol` 的标准 ticket provider；Kotlin gate 只接受匹配 pairing epoch、lease 与 token 的当前项。revoke 停止活动 fetch / 关闭 provider 而不删除接收端 partial；APK 同时含 `libiroh_ffi.so` 与 `libtransport.so`。 |
| **REBUILD-02（L2）Desktop native fetch 与 completion receipt** | 2026-09-01 | `a8d530e` + `10b12fd` | ✅ 代码完成（flow 3/3、pairing 10/10、`just ci` all green） | `flow.offer/fetch/cancel` 走 native blobs fetch/resume，epoch + lease + hash 双重核验；receipt 只在 `Ingestor` durable materialize 后持久化。独立 `.ppf/flow-blobs` 保留 partial，旧 batch/inbox 清理不再影响新 Flow。 |
| **REBUILD-00（L2）旧线冻结与 Flow 边界** | 2026-09-01 | 本 commit | ✅ 代码完成（Android JVM **387 tests / 0 failures / 4 skipped**；`just ci` all green） | ARCH-02～09 的账本/消费者/receipt/取消/epoch/对账骨架迁入 `backup/flow/`，新增边界测试禁止该包引用 `BackupWorker` / `BackupRunner` / `ConfirmedStore` / `ReuploadQueue` / `WatermarkStore`。旧 Worker、Runner、confirmed 与重传队列标记 `LEGACY`；旧测试已按“重写的产品不变量 / frozen legacy 机制 / 待后续裁决”分类，R1a Android blobs bridge 与 R1b Desktop fetch/receipt 已释放并可并行。 |
| **备份核心重建主线（REBUILD-00~05）** | 2026-09-02 | 本 commit | 🟡 REBUILD-05 阻塞 REBUILD-04 精确真机验收 | REBUILD-00~04 已形成生产纵切；真机对账发现范围扩展未接入 Flow backfill，30 项中 4 项未入账。先由 REBUILD-05 补齐范围/游标语义，随后只用测试相册完成 Pause → kill → Continue → Cancel。 |
| **批次 A（REL-03 / I18N-01 / DESK-09 / BUILD-02）** | 2026-09-01 | `5623f63` | 🟡 自动 CI 成功；仅欠 2 项真机/真环境验收 | 并行 worktree 完成四张独立卡后统一合入：REL-03 给每个版本目标独立读取/断言并验证 lock；BUILD-02 让 5 个 workflow 的 13 个 Cargo job 从 `rust-toolchain.toml` 派生工具链且有 TOML 反证；I18N-01 将空相册名交给资源渲染，英文真机验收欠账；DESK-09 透出 daemon stderr 并为迁移不兼容给出升级提示，旧 daemon/新库真机验收欠账。批次本地验证：`just ci`、版本脚本正反例、工具链脚本反证、Android JVM **385 tests / 0 failures / 4 skipped**、desktop vitest 26、vite build、desktop lib 16 均通过；用户确认 Actions 成功。 |
| **ARCH-09（L2）P1 对账分页协调与源存在性裁决接线** | 2026-09-01 | `db5d3ab` | ✅ 代码完成（Android JVM **381 tests / 0 failures / 4 skipped**；`just ci` + CI Android #104 全绿） | `ReconciliationCoordinator` 只取当前 epoch confirmed hash 的 ≤500 项，remote present 零 source probe；remote missing 才调用独立 source probe，ARCH-07 落 `NEEDS_DECISION` / `UNRECOVERABLE`，不改队列状态。`ContentResolverSourcePresenceProbe` 仅开关流不读内容；反证让 present 项 probe 源时 R-01 实际变红。 |
| **ARCH-08（L2）P1 Desktop 存在性探测与分页协议** | 2026-09-01 | `5b1044d` | ✅ 代码完成（Rust/Android 合同通过；`just ci` + CI Rust #71 + Dogfood #204 全绿） | 新增 side-effect-free `backup.presence` / `BackupPresenceQuery`，≤500 个 hash 只查索引、返回缺失集合；Android `RemotePresenceProbe` 校验页面并通过 `DaemonClient` 解码结果。反证实际变红：临时委托旧 `backup.manifest` 后错误创建 session，测试报 `left: 1`, expected `0`；已还原。 |
| **ARCH-07（L2）P1 远端对账事实与恢复裁决** | 2026-09-01 | `1162e0d` | ✅ 代码完成（R-01/R-02；Android JVM **377 tests / 0 failures / 4 skipped**；`just ci` + CI Android #100 全绿） | `TransferItem` / `CompletionReceipt` 新增持久内容身份与远端/源存在性、恢复裁决；`RemoteReconciliation` 对当前 epoch 的已确认项只写事实：源仍在 → `NEEDS_DECISION`，两端缺失 → `UNRECOVERABLE`，远端仍在则无恢复裁决。不会自动重新入队、fetch、递增失败计数、改 UploadCursor 或删除源。新合同 3/3；反证实际变红：临时重新入队→R-01/R-02 两条失败，源缺失误裁可恢复→R-02 失败，均已还原。旧批次校准、WorkManager、UI、proto、Rust/desktop 零改动。 |
| **ARCH-06（L2）换 Desktop 的配对 epoch 隔离** | 2026-09-01 | 本 commit | ✅ 代码完成（P-01/P-02；目标 3/3、全量 Android JVM **374 tests / 0 failures / 4 skipped**；`just ci` 全绿） | 新增持久 `PairingEpoch`、`PairingEpochController` 与 `ARCH01PairingEpochTest`。换 Desktop 在单一账本替换中保留 ScopeRevision，清空旧 epoch 的发现/上传游标、队列、取消轮、lease、backfill 与 partial 归属；新发现项携带当前 epoch。完成回执同时校验账本 epoch 与 item epoch，旧 Desktop 的迟到回执被拒、同 epoch 回执照常确认。旧 PairingStore/UI/WorkManager/Daemon/proto/传输 adapter 零改动。 |
| **ARCH-05（L2）取消本轮的持久扫描与恢复** | 2026-09-01 | 本 commit | ✅ 代码完成（X-01~X-05；目标 10/10、全量 Android JVM **371 tests / 0 failures / 4 skipped**；`just ci` 全绿） | 新增 `backup/CancellationRoundController.kt` 与 `ARCH01CancellationRoundTest`：只在 ARCH-03 已 Pause 且 fetch lease 已停时开启取消轮；原有 `QUEUED`/`FAILED_NEEDS_USER` 项与活动轮中新发现项均携带轮次 id 并直接转为 `CANCELLED_BY_USER_ROUND`，`CONFIRMED` 完成凭据不被覆盖。重启继续沿持久 `CancellationRound` 与 DiscoveryCursor；结束轮次后新增项恢复普通 `QUEUED`。Restore 只重排指定轮次且结束活动轮；Discard 只删除该轮恢复资格，不会自动复活取消项。旧 WorkManager/UI/Rust/desktop/传输协议零改动。 |
| **ARCH-03（L2）严格消费者、暂停与条件等待** | 2026-08-31 | 本 commit | ✅ 代码完成（C-01~C-05 5/5；全量 Android JVM 通过；三条反证实际变红） | 新增 `backup/StrictConsumer.kt` 与 `ARCH01StrictConsumerTest`：消费者只按持久 UploadCursor 严格消费队头，fake delivery port 验证 Pause 停止当前项并保留 partial、唤醒不能清用户 Pause、Continue 只恢复 #18；Wi-Fi/电量/Desktop 条件等待不消耗失败预算、恢复自动续队头；永久错误第 3 次才将 #18 写为 `FAILED_NEEDS_USER` 并允许 #19 开始。旧 WorkManager/UI/实际 iroh adapter 零改动；ARCH-04 已移入可接队列。 |
| **ARCH-02（L2）手机账本与发现页原子提交** | 2026-08-31 | 本 commit | ✅ 代码完成（Android JVM **351 tests / 0 failures**；D-01~D-04 4/4；两条反证实际变红） | 新增 `backup/DiscoveryLedger.kt` 与 `ARCH01DiscoveryLedgerTest`：一个发现页最多 500 个候选，`TransferItem` 入账与复合 `DiscoveryCursor` 前移只通过单一 JSON 快照 tmp+rename 提交；崩溃注入发生在替换前时，两者都不改变，重启可重发现；稳定 `(sourceRef, sourceVersion)` 去重防重放重复；活动 `CancellationRound` 期间新发现项直接记为 `CANCELLED_BY_USER_ROUND`，游标仍前移。账本同时持久化 `ScopeRevision`、`UploadCursor`、consumer gate 与 fetch lease 边界，供 ARCH-03 严格消费者使用。反证真跑：临时提前单独落游标 → D-02 红；移除稳定身份去重 → D-03 红，均已恢复。旧 `BackupWorker`、WorkManager、UI、Rust/desktop 与传输协议零改动；ARCH-03 已移入可接队列。 |
...
| **DEV-01b** | 2026-08-10 | `b7c6b48` | ✅ 已实现（android 全量绿；巡检轮 126/126 复验） | 重装识别入口先隐藏（L0，用户拍板 2026-08-12 走查批次）：**UI 入口删净**——Android 设置页「重装识别」开关行隐藏、桌面允许对话框「替换旧的 X」选项默认关（编译期 flag）；**底层不拆**——pair.request 的 device_hint 照发照存（数据继续积累，未来开 flag 即用）。巡检轮抽检（c2ee128）✅ PASS：UI 入口删净、device_hint 照发照存、桌面 flag 保留反证路径。 | 
| **DESK-02** | 2026-08-10 | `e6dfee5` | ✅ 已实现（Rust 233/233 + Android 126/126 + arch-check 绿；巡检轮抽检 PASS） | 桌面走查修复三项（L1，2026-08-12 走查批次）：**①更新通道选项删除**——环境由构建推导（PPF_BUILD_VERSION 含 `-test.` → test 通道，页脚完整版本串 + 琥珀「测试版」徽标，正式构建只显版本号；Android 设置页通道行一并删除，推导函数单测两分支，stable URL 红线不变）；**②已移除设备不再展示**——devices.list 默认过滤 revoked（include_revoked 参数默认 false + 语义单测），桌面零改动；**③二维码弹窗不让路 → 移交 IPC-02**（本卡不做临时轮询）。⚠️ 纪律：直推前没跑 fmt → main 红 Format check（8/7 同款，验收人 a5a9c7e 修复）；nit：徽标文案写死组件未走 i18n 归 T-042 收编债。 | 
| **ICON-01b** | 2026-08-10 | `a33a9ed` | ✅ 已实现（三星真机实锤修复；巡检轮像素级 PASS） | Android 自适应图标安全区缩排（L1，三星真机 2026-08-12 反馈：launcher 图标像被放大、脸展示不全）：ICON-01 前景层全幅渲染（兽面占画布 ~77%）被系统遮罩只显示中心 ~66% 安全区 → 视觉=放大裁切。修法：前景层按 108dp 网格出图，兽面内容缩放到中心 66dp 安全圆内（相对全幅 ~0.61 缩放）；背景层纯色 paper 不变；monochrome 层同步复查。巡检轮抽检（c2ee128）✅ **PASS（像素级实测）**：内容占比 0.52×0.39 落在 0.61 安全区内、纸底正确剥离给背景层；模拟器遮罩截图已补交（2026-08-11）。 | 
| **MOB-03** | 2026-08-10 | `e9731d9` | ✅ 已实现（android 全量绿；巡检轮 126/126 复验） | 相册选择页权限链修复（L1 加急，三星真机 2026-08-12 实锤：MOB-02 删手动备份按钮时把权限申请链一起删没了——无权限直接进列表 = MediaStore 空查询 = 全白，备份主流程死）：进入相册选择页前走完整权限链——未授权 → 发起系统权限请求 → 完整授权后进列表；部分授权 → MOB-02 §二引导态（「只授权了部分照片…」+去设置）；拒绝 → 人话引导不崩不白屏。onboarding「配对成功→选相册」入口同样过链。巡检轮抽检（c2ee128）✅ **代码 PASS**（Home 与 onboarding 两入口统一走 enterBucketPicker 权限链，完整/部分/拒绝三分支齐、无白屏路径）；模拟器「全新安装零权限」截图证据已补交（2026-08-11）。 | 
...
| **H-10b-release** | 2026-08-09 | `1242b64` | ✅ 已合并（0.3.1 正式发布） | bump 0.3.1（xixi 拍板 2026-08-09，正式发布）：T1-T7 全量（版本号显示、相册级备份范围、配对状态机、QR 瘦身、审计事件流、dmg 拖拽布局、macOS 签名公证、Windows NSIS 图形界面）；bump-version.sh 三端同步（Cargo/Android/desktop，versionCode 3→4）。0.3.1 的 Android 真机六项验收 + T-082/091/092 桌面真窗口走查挂验收人（NEXT「等用户」欠账不变）。 |
| **H-10b-T7** | 2026-08-09 | `e8a5a78` `3097580` `9925d87` | ✅ 代码已合并（真机验收待验收） | Windows GUI——桌面壳 NSIS 安装包进 release 管线（xixi 拍板 8/9）：Windows job 增建桌面壳（Tauri+svelte）NSIS 安装器，daemon.exe 作 externalBin sidecar、UPDATE_SIGNING_KEY 签 updater 产物、安装包进 SHA256SUMS 和上传；未签名（Authenticode 证书属 H-02，SmartScreen 会警告，文档如实说明）。**两个 test tag 实锤修复**：①`3097580`——Tauri externalBin 解析 `binaries/ppf-daemon-<triple>.exe`（x86_64-pc-windows-msvc），裸 `ppf-daemon.exe` 报 resource path doesn't exist（test.8 实锤）→ 带 triple 名复制 daemon；CHANGELOG 补 0.3.1 条目。②`9925d87`——upload-artifact v4 多顶级路径时**按 top-level path 展平**，加 nsis 路径破坏了 release job 的 `windows-x64/daemon.exe` glob（test.9 实锤 no matches found）→ 拆两个 artifact（windows-x64 binaries / windows-x64-installer nsis）。验证=CI Windows job 本身（本 mac 无法本地构建 Windows）。验收状态：待验收（Windows 真机走查归 H-09/H-10b 验收人）。 |
| **H-10b-T6** | 2026-08-09 | `7c77e84` | ✅ 代码已合并（真机验收待验收） | 相册级备份范围——选相册、再备份（两个动作）（xixi OPPO 实机反馈 8/9：「只能固定选照片或视频文件，要么全量，无法按相册处理」+「选择备份内容和备份是两个动作」+ T2「再点备份不能没有任何反应」）：MediaScanner `listBuckets()`（BUCKET_* 相册 id/名/count）+ `scanSince(watermark, bucketIds?)` + `countAll(bucketIds?)` 按选中相册过滤（null=全量，旧行为）；`BackupScopeStore` 持久化选中相册 id；UI 设置行「备份范围」→ BucketScreen（相册列表+复选框+全选/清空+「备份这 N 个相册」），Home 显示「全部相册/N 个相册」；手动备份重扫**选中相册全部**（since=0 忽略水位）后按确认缓存过滤只传新图，显式「已是最新」反馈（T2）；水位 commit 后推进，自动备份续跑；BackupWorker 自动路径同范围。**⚠️ review 实锤 3 个问题另立卡**（空集语义反转、三元组口径打架 → FIX-T6；性能 → PERF-01）。验收状态：待验收（T6 范围语义修复 FIX-T6 未合并前不验）。 |
| **H-10b-T5** | 2026-08-09 | `c26d5dd` `35b44ac` | ✅ 代码已合并（真机验收待验收） | 审计事件流（xixi 反馈「log 居然不包含设备连接时候的数据，只审计了传输」）：audit_log 表本来就 extensible（action/detail TEXT）只是没人喂——补 pair.requested（每次扫码落审计）/ pair.denied（owner 拒绝/UI 超时也审计，pair.accepted 已有）/ backup.started+finished（session 级，ingested=N duplicates=M，互补 asset 级 commit 审计）+ `audit.list` IPC（{ts, action, actor, detail}，只读，上限 1000）；桌面「活动记录」页改渲染 audit.list 事件流（pair.requested/accepted/denied、backup.started/finished、device.revoked/unpaired、backup.commit、external.delete，人类可读行+相对时间，最新在前，未知 action 回退原始类型不吞），activity batches 保留兼容但页面以审计驱动。daemon 82/82。验收状态：待验收（桌面活动页真窗口走查归 T-082/091/092 验收人）。 |
| **H-10b-T4** | 2026-08-09 | `3a935f7` | ✅ 代码已合并（真机验收待验收） | 配对状态机（xixi 反馈「二维码在我连接完成后还持续展示」+「允许完成之后这个状态不消失隐藏」）：QR 改弹窗（点「添加设备」→ 360px QR popup，2x 渲染），非常驻卡片；关闭即弃码（下次打开新铸）；扫码落地（pending 0→>0）时 QR 弹窗自动关、允许/拒绝弹窗自动出；处理完请求（刷新见 pending==0）弹窗关——无残留状态、无常驻卡片；确认弹窗未起时保留 fallback pending 卡。验收状态：待验收（真窗口走查归验收人）。 |
| **H-10b-T3** | 2026-08-09 | `fd16970` | ✅ 代码已合并（真机验收待验收） | 配对 token 32B→12B（QR ~170→~120 字符）：一次性配对 token 32B（64 hex）过度设计（10 分钟 TTL 一次性），12B（96-bit 熵）足够——token hex 64→24 字符，QR 总长 ~120，QR version ~11（61x61）→~8（49x49），模块密度约减半；IPC socket token 保持 32B（不同用途不动）；Android 透传 token（parsePairingQr→pair.request）零改动。daemon 82/82（含 12B token 的 qr_string_carries_node_and_token + pairing_flow expiry/used-token 用例）。验收状态：待验收（新码真机扫码实测归验收人）。 |
| **H-10b-T1** | 2026-08-09 | `b390c9c` | ✅ 代码已合并（真机验收待验收） | 版本号显示（xixi 反馈「没有版本号信息，每次都不知道安装的什么版本」）：桌面固定 footer「P-Pass v0.3.0」（tauri app API getVersion）；Android 设置行「版本 0.3.0 (3)」（BuildConfig.VERSION_NAME/VERSION_CODE）——报 bug 能报出确切构建。验收状态：待验收（目视，归验收人）。 |
| **H-10b-fix 批次** | 2026-08-08 | `75db7b2` `a569361` `54bb77f` | ✅ 代码已合并（真机验收待验收） | H-10b 真机反馈三修：①`75db7b2` **QR 密度**（xixi 实机扫不出）——`&a=` 完整 PeerAddr（id+relay+直连 IP，base64 100-180 字符，QR 总 300+）改 `&r=` 只带 relay URL（明文 ~30 字符，总 ~170）；Android 端 buildAddrToken(node, relay) 重建存储 token（backup manifest 解析零风险），旧 `&a=` 码双向兼容；daemon Pairing 加 relay_provider（transport PeerAddr::relay_url() 惰性）；桌面 QR 320px + 纠错级 L；**手机补手动输入入口**（ScanScreen「手动输入」粘贴配对码→校验 ppf://pair 前缀→同一配对流，strings en/zh）。②`a569361` **dmg 拖拽布局**（「dmg 不是无脑拖拽到 application 那种吗？」）——bundle-desktop-macos.sh 第 6 步重做：可写 UDRW 卷→挂载→丢 /Applications 符号链接→Finder 摆窗口布局（图标位置/图标视图/隐藏工具栏）→.DS_Store 烤进镜像→转 UDZO；osascript 失败非致命（无头 CI/TCC），Applications 链接已足够让拖拽路径明显。③`54bb77f` **QR 刷新按钮**（「二维码还无法刷新」）——token 活 10 分钟但步骤 3 无重铸入口，唯一出路是关向导或复制死 token；抽 generateQr()（首取和新按钮共用）加「刷新二维码」进向导导航，minting 中禁用。验收状态：待验收（新码扫码/拖拽安装真机走查归验收人）。⚠️ 2026-08-10 巡检发现 `75db7b2` 改 QR 格式但 testclient 解析器没跟上 → FIX-SC1（已修，见上）。 |
...
- **T-080 Android**（7b10f555）：两 tab 对齐设计稿；杀掉两个真机缺陷
  （「都存好了」vs 待备份 31 矛盾、epoch 0 假日期）。单测 6/6+反证红过，
  模拟器假配对法视觉验收（run-as 注入 files/pairing.json）。
- **T-081 桌面**（fcfc386b）：侧栏四页+hash 路由；IPC 集合逐字未变。
...
- **T-082 桌面**（f63acfa9）：7 项走样修复。定性：token 无罪（64 处正常
  使用），设计稿无罪（等高/148QR/1140×720 都画了），是实施走样+窗口尺寸
  在上一卡禁区里没人动。
- **T-083 Android**（5a1293fb）：4 处旧代码遗留（副标题/立即备份黑按钮等
...
- 期间环境：subscribe_flow 全量并发跑一次偶发红（REV-01 的 revoke 计时断言），
  ...
  动 ci-rust 成功）未复现。
| **桌面走查补漏（≥1440 居中）** | 2026-08-13 | 本 commit | ✅ 已实现（vite build 绿 176 modules；实测 1920px 右侧死区 486px→0） | e5c08a0 的 `@media(min-width:1440px)` 给 `.page` 加 `max-width:1180px` 但漏了 `margin: 0 auto`——只限宽不居中，超宽窗口下内容左贴、右侧留死区（用户实测反馈「右边留死区」的同类症状，只是阈值从 880px 上移到了 1180px）。补 `margin: 0 auto` 后内容居中，1920px 死区从右侧 486px 变为对称留白。 |
...
## 2026-08-25 MOB-29：删掉的照片照旧传回来，但两端各给一句告知（commit 561dd2d）
...
## 2026-08-25 MOB-34：被删的老照片真的被补回来，K 能归零（commit d879008）
...
## MOB-36 移进已选相册的老照片进本轮候选（2026-08-26，commit 57a8ff0 · 🟡 等真机验收）
