...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **ARCH-09（L2）P1 对账分页协调与源存在性裁决接线** | 2026-09-01 | 本 commit | 🟠 已认领，尚未写生产代码 | 将当前 epoch 已确认项按 queueSequence 组为 ≤500 hash 页，接 ARCH-08 presence；只有 Desktop 缺失时才通过独立 source-presence port 检查 `sourceRef`，再由 ARCH-07 写 `NEEDS_DECISION` 或 `UNRECOVERABLE`。明确隔离旧 `BackupWorker`/`BackupRunner`、WorkManager、UI 与自动补传。 |
