...
- [x] 修复后 `bash tools/check-rust-toolchain-ci.sh` 退出 0，并继续验证所有
  实际 Cargo job 都含 `tools/setup-rust-toolchain.sh`。
- [x] 反证：移除 `e2e.yml` 的 `build` job 工具链 setup 或将实际 Cargo job
  的 setup 改坏，检查必须退出非零。
- [x] CI Rust 的该检查步骤与整条 workflow 成功。
- [x] 收尾：更新 PROGRESS.md、docs/QUEUE.md 与 ROADMAP.md。
