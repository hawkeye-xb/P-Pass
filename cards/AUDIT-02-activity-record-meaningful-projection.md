...
> 🟡 状态：代码完成，待真实桌面验收 · 协同分支：`main`
> 当前节点：canonical `audit.list` 已返回 evidence summary；活动页已改为用户结果投影并过滤诊断噪音
> 下一步：从 current-main 重建 Desktop，跑一张隔离测试图，核对活动记录显示 `已备份 1 张照片` 且没有「连接了」/「一批传输完成」
> 级别：L2 · 阻塞：无
...
- 正常 Flow 终态按 `audit_operation.evidence_summary`（daemon 由 `audit_item_evidence` 重算，不能相信手机自报 `final_counts`）显示数据结果：`已备份 N 张照片`；有非零异常时同一行追加 `；M 张需处理`、`；K 张已跳过`。不得出现「一批」「round」「传输」等内部词，也不得逐照片刷行。
...
- [x] RED：给 `flow.round.finished` 的 `evidenceSummary={confirmed: 1}` → 活动页文案严格为 `已备份 1 张照片`；改回「一批传输完成」测试必须失败。
