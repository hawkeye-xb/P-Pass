...
- [ ] **ARCH-01 首批 P0 实施卡** — **2026-08-31 ARCH-02 已完成**：D-01~D-04 的失败测试与最小手机账本/发现页原子提交已落地（351 Android JVM tests / 0 failures；游标先行与去重移除反证均实际变红后恢复）；`ARCH-03`（C-01~C-05 严格消费者）现可接 → `ARCH-04`（E-01~E-04 完成凭据/范围）→ `ARCH-05`（X-01~X-05 取消本轮）。每卡先写新失败用例；后续卡等待前置，不重开 ARCH-01 产品语义。
