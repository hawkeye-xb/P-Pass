...
- [ ] **REBUILD-05 Flow 范围扩展补扫接线** — **2026-09-02 发现，未开工**：范围保存仍只重置 legacy watermark，未向新 Flow 写 ScopeRevision/backfill；扩大范围时游标之前的旧项会永久漏入队。修复并以独立测试相册验证后，释放 REBUILD-04 的 Pause → kill → Continue → Cancel 验收。
