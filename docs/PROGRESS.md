...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **ARCH-08（L2）P1 Desktop 存在性探测与分页协议** | 2026-09-01 | 本 commit | ✅ 代码完成（Rust/Android 合同通过；`just ci` 全绿） | 新增 side-effect-free `backup.presence` / `BackupPresenceQuery`，≤500 个 hash 只查索引、返回缺失集合；Android `RemotePresenceProbe` 校验页面并通过 `DaemonClient` 解码结果。反证实际变红：临时委托旧 `backup.manifest` 后错误创建 session，测试报 `left: 1`, expected `0`；已还原。 |
