...
| **备份核心重建主线（REBUILD-00~04）** | 2026-09-01 | 本 commit | 🟠 R3 进行中 | ARCH-02~09 被重新定性为未接生产的状态骨架，旧 `BackupWorker → BackupRunner → manifest/push/commit` 仍是唯一真实路径。legacy/flow 边界、Android blobs provider、Desktop fetch/receipt 与 Flow runner 已完成；当前将 Worker 降为 wake adapter、投影最小账本状态并准备三星测试相册验收；P2/P3 不再占用主线。 |
