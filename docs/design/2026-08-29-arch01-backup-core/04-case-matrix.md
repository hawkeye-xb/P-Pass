...
How to read a row: **Given + When = trigger**; **Then = expectation**; **Negative proof = which protection, once removed, must turn the test red**; **Device acceptance = the reviewer’s steps and visible result**. Rows marked “Open” map to undecided questions in the design archive and are not implemented until decided.
...
| O-11 | #K is `CONFIRMED`, missing on Desktop, and the phone original is also deleted | slow path | **Open:** state and presentation undecided | Open | Open |
...
| D-07 | device on API 26–29 (no `GENERATION_MODIFIED`) | fast path | **Open:** fast-path key and whether photos can be missed are undecided | Open | Open |
...
| C-12 | Desktop explicitly reports it cannot receive or save (peer failure) | transfer #18 | **Open:** handling undecided | Open | Open |
...
| X-04 | #18 is transferring | user changes the album scope | no order written and no scan on the spot; the next fast / slow path query uses the new scope. **Open:** whether #18 in flight is interrupted on the spot | synchronously scanning and bulk-rewriting orders on scope change must fail | primarily automated |
...
| ID | Given | When | Then | Negative proof | Device acceptance |
|---|---|---|---|---|---|
| P-01 | Wi-Fi/battery/album settings plus unfinished orders and partials | pair a new Desktop | settings stay; partials and runtime ownership clear; the new Desktop starts a new backup history. **Open:** how orders are separated by `pairingEpoch` | clearing settings or carrying old partials to the new Desktop must fail | switch Desktop; settings remain and old transfer progress does not follow |
