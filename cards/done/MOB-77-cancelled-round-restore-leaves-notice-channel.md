...
- [x] 真机或模拟器截图对比：备份卡内新增的恢复行、「关于」标签渲染
      正确，且取消轮次后不再弹出警告条。
...
2026-09-14，三星 SM-S9210（RFCX1040SNE），当前 main 构建
versionCode 22：

- 设置页「备份哪些相册」卡内新增「已跳过的照片　4张·点击恢复」一行，
  与警告条完全不同的普通 cell 样式（灰字+`›`），页面顶部无任何常驻
  警告条。
- 点击后：该行立即从卡片消失（`cancelledRoundCount==null` 不渲染验证
  成立），绿色进度卡从「5/9」变为「9/9 张已回家 · 最近成功 刚刚 ·
  照片都存好了」——4 张跳过照片真实重传完成，非仅 UI 状态切换。
- 「其他」标签确认已改为「关于」。
