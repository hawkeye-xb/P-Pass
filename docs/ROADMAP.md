...
- [ ] **REBUILD-04 三星生产首包补验** — **2026-09-02**：已选测试相册 30 项中，Flow ledger 26 项均 `CONFIRMED`，其中 25 个 hash 不同；Desktop durable index 也为 25 个不同 hash，所以 26→25 是预期内容去重。其余 4 项在已选范围、但位于 DiscoveryCursor 之前且未入 ledger；当前无可传队头。真实验收尚未通过，等待 REBUILD-05。
- [ ] **REBUILD-05 Flow 范围扩展补扫接线** — **2026-09-02 代码/JVM/debug APK 已验证，待三星测试相册**：范围扩大时，持久 ScopeRevision 与 historical-backfill boundary/progress；历史页追加新 stableId 而不移动 live cursor 或打断 strict head，已确认项不重复入队。RED→GREEN 覆盖 revision、boundary、cursor、strict-head 次序；Android JVM 255 tests / 0 failures / 4 skipped。真机确认 30 项状态后，释放 REBUILD-04 Pause → kill → Continue → Cancel 验收。
