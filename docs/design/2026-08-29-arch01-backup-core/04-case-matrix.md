...
| O-02 | Retired: the former local backstop for missed photos is gone | — | Which pick source admits unsent photos with generation ≤ G is to be confirmed | — | — |
...
| D-06 | Retired: the former local incremental diff (hashing only changed photos) is gone | — | File reads are covered by group R | — | — |
