...
| **备份核心重建主线（REBUILD-00~04）** | 2026-09-01 | 本 commit | 🟡 R0/R1/R2 完成，R3 已释放 | ARCH-02~09 被重新定性为未接生产的状态骨架，旧 `BackupWorker → BackupRunner → manifest/push/commit` 仍是唯一真实路径。legacy/flow 边界、Android blobs provider、Desktop fetch/receipt 与 Flow runner 已完成；下一步是 Worker wake adapter、最小 UI projection 与三星 debug APK 验收；P2/P3 不再占用主线。 |
