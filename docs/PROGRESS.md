...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **UI-09（L2）聚合状态必须读 Flow 账本** | 2026-09-06 | 本 commit | 🟠 已认领开工 | 验收人真机实证（test.4）：换内核后首页状态全程无变化——传输中/暂停/取消无展示窗口、传完 K 与上次成功纹丝不动。根因源码锤死：六态投影已迁账本，但聚合层（K/M/lastSuccess）仍读 LEGACY `ConfirmedStore`，新内核零写入（`recordRun` 唯一调用方是冻结的 `BackupRunner`；REBUILD-03 注释自述 deliberately 未迁且从未开卡）。方案：账本派生纯函数 + `completedAt` schema 补字段 + AllSafe 重定义，RED 用例（20 项传完 K 归零）先行。UX-13/UX-14/MOB-13 真机验收挂起待本卡；UI-10（runtime 空快照装死 + 重传死 UI/归属过滤/哨兵尾巴）同批开卡挂账。 |
