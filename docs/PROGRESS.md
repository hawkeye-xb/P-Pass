...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **MOB-47（L2）视频资产在查看器不可预览** | 2026-09-03 | 本 commit | 🟡 代码已合并，真机/真窗验收欠账 | 桌面大图查看器按 `media_type` 分流：视频改走 `asset.path` 原文件绝对路径 + Tauri `asset` 协议（`convertFileSrc`，新命令 `allow_media_scope` 按需授权被点开视频所在 `<node>/YYYY/MM` 目录层、非递归）+ 原生 `<video controls>`，不再把整段视频 base64 拉进内存；图片分支零改动。Android 播放器层从系统 `VideoView` 换成官方 Media3 `ExoPlayer`/`PlayerView`（钉 1.9.4，1.10+ 需 compileSdk 36），`DisposableEffect` 释放，自带播放/暂停/进度/seek/错误态，download-then-play 语义不变；新增 VideoScreenTest 3 条源码守卫。验证：桌面 src-tauri **16 passed** + vitest **26 passed** + vite build 绿；Android JVM **260 / 0 / 4 skipped**（49 XML 全本次生成）；`just ci` all green；Rust nextest 332 passed / 1 skipped。APK 体积对照（ICON-02 纪律）：**45,655,913 B → 50,608,204 B，+4.72 MiB ≈ +10.85%**。剩欠账：桌面弹窗真播放、Android logcat 无 `MediaCodec`/`ExoPlayer` 泄漏。 |
