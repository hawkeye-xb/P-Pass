...
| **备份核心重建主线（REBUILD-00~04）** | 2026-09-01 | 本 commit | 🟡 R0 完成，R1 并行启动 | ARCH-02~09 被重新定性为未接生产的状态骨架，旧 `BackupWorker → BackupRunner → manifest/push/commit` 仍是唯一真实路径。重建拆为：legacy/flow 边界、Android blobs provider、Desktop fetch/receipt 并行，随后 Flow runner、Worker 切换和三星 debug APK 验收；P2/P3 不再占用主线。 |
