...
状态：⏸ 暂缓（2026-09-17 验收人拍板：不提工单，接受现状，出问题再说）
...
**2026-09-17 拍板：不提工单，本卡暂缓。**

理由与代价都已摆在台面上后作出的选择：提工单能让那 6 个只读 ref 真正消失，
但代价是 **60 个 PR（#3–#58、#65–#68）的 diff 视图永久消失**——波及面远大于
最初估计的 6 个，因为 GPG 签名被剥离把改写起点推早到了 2026-08-01。权衡后
认为不值得。

**当前残留（明确接受，不是遗漏）**：`refs/pull/53|56|65|66|67|68/head` 仍可
走到改写前的 6 个旧提交。正常浏览、clone、搜索、贡献者列表都看不到；要取到
必须明知故犯地 `git fetch origin 'refs/pull/*:refs/pull/*'` 再 grep。

**重新启动的条件**：若将来确有需要，工单草稿在
`~/personal/P-Pass-backup-20260917/github-support-ticket.md`，提交前务必用
修正后的数字（受影响 PR **60 个**、First Changed Commit **`bd948f28`**），
不要用草稿里最初那版。核查命令见上方验收标准。
