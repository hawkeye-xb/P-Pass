...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **AUDIT-01（L2）Flow 审计 v2：持久 outbox 与直接切换** | 2026-09-10 | 本 commit | 🟢 代码完成，本地全量 `just ci` 绿；待真机验收 | `audit_event` v2 成为唯一长期审计事实源，整表 DROP 旧 `audit_log`（migration `0005_audit_event_v2.sql`），不迁移不兼容历史行。Android：`DiscoveryLedger` 的用户事实与 outbox event 原子快照提交（新 `AuditOutboxEvent`/`AuditKinds`），`AUDIT01LedgerOutboxTest` 锁死该契约；pause/continue/scope/epoch-invalidate/permanent-failure/reconciliation-resolved 六类事件全部接线。Rust：`crates/storage/src/audit_repo.rs` 重写为 v2 repository（`event_id` 幂等键 + `INSERT OR IGNORE`），daemon 全部生产路径审计写入（pairing/reconcile/router/ipc）改为 v2 `kind`/`payload`；旧 batch `backup.started`/`backup.finished` 会话审计随之下线（不在 v2 事件矩阵内）。Desktop：`App.svelte` 活动页改读 v2 字段，依赖旧会话审计的「本周新备份/去重跳过」统计与「备份耗时」派生 UI 一并移除（无对应数据源，不编造数字）。验证：Android 328/328、Rust nextest 346/346（`just ci` 全绿）、desktop-rust 18/18、desktop-vitest 57/57。真机验证（新活动文案/Flow 真实终态）显式列后续批次。 |
