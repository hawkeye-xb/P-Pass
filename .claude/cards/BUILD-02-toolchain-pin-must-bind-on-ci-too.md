# BUILD-02 工具链钉在本地了，但 CI 侧不一定生效　级别 L2

> ⛔ 未实施。开卡原因：`BUILD-01` 的同一个病在 Rust 侧刚咬了一次（CI 红），
> 本地已修，**CI 侧能不能钉住我无法验证**（没有 gh 认证）。
...
⚠️ **这个 action 是否导出 `RUSTUP_TOOLCHAIN`，我不确定。** 如果导出了，
...
涉及的 workflow：`ci-rust.yml` / `ci-desktop.yml` / `e2e.yml` / `release.yml` /
`artifacts.yml`——**凡是跑 cargo 的都要**，漏一个那一条就还在浮。

⚠️ **版本号只许出现在 `rust-toolchain.toml` 一处。** 在 workflow 里写死
`toolchain: 1.98.0` 是把同一个数抄两遍，那是制造下一次漂移，不是修它。
...
- 反证：把 toml 改成一个更老的版本 → CI 日志里的版本必须跟着变
  （证明真的是 toml 在管，不是 action 在管）
