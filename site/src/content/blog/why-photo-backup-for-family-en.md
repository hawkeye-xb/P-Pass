...
So the question became very concrete: **can we build a zero-threshold version, where phone photos land automatically on the computer that's already running at home? Scan one code, then never think about it again.**
...
- No subscriptions: paying monthly to ask "are my photos safe?" sounds absurd. Backing up at home is free, forever.
...
This stance isn't marketing. Every design decision in the project answers to it. The site you're reading counts as evidence: no analytics scripts, no trackers. A privacy product shouldn't make an exception for its own website.
...
Simple, but not a black box. There are three layers of control:
...
This was written right after the first milestone. The design process, the pits we fell into, the trade-offs will all show up on [this blog](/blog/) over time: how the icon went through nine iterations, how the desktop turned 3-second polling into 36-millisecond events.
