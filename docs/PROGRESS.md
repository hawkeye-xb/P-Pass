...
| **NET-01（L2）蜂窝热点 relay 对照** | 2026-09-11 | 本 commit | 🟡 未复现旧失败 | 三星 SM-S9210 连手机蜂窝热点、Mac 保持原网络；当前 main daemon 的 `transport=debug` 记录 inbound 为 n0 `Relay`。强制生产 `MediaWatchJob` 后，已选范围的隔离照片 `CONFIRMED`（attempt 0），daemon 审计有 `ingest.new`，两端无 timeout/`ConnectionLost`。这是 relay 成功对照，不是原始 15 秒失败的反证；NET-01 保持待调查，仍等 OPPO 原生 Android 的失败 logcat。手机测试源、后台开关和 device-idle 白名单均已恢复。 |
