...
| **REBUILD-05（L2）对账语义：迟到完成回执被 Pause/Cancel 竞态误判丢弃** | 2026-09-02 | 本 commit | 🟡 代码全绿，三星真机竞态复现待验收 | 只读比对 Desktop `flow_delivery`（sqlite）与手机 `discovery-ledger.json`（adb run-as，无写操作）定位根因：`CompletionAndScope.acceptCompletionReceipt` 把「fetchLease 已被 Pause/Cancel 清空」误判为「被更新尝试取代」，直接丢弃 Desktop 迟到的完成回执，产生卡面所述的「Desktop 多 1 条历史 completed grant」。RED：新增测试模拟 Pause→Cancel 后 Desktop 迟到回执，先跑出真实 `AssertionError`。GREEN：仅当存在**另一个活跃 lease**（同队列位、不同 token）才算被取代拒绝；lease 已清空不再算取代，迟到回执正常确认并清除 `cancellationRoundId`。反证：真正被新尝试取代时仍必须拒绝（防恒真式）。验证：Android JVM **260 tests / 0 failures / 4 skipped**（时间戳核对为本次跑出）、`assembleDebug`、`just ci` 全绿；新 debug APK 已装三星 S9210。待办：三星上主动构造一次新的同类竞态验证收敛为 `CONFIRMED`，旧 33 号项已是终态不可复用。 |
