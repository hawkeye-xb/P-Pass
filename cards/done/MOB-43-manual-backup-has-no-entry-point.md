...
**状态**：✅ 不做——2026-08-27 验收人拍板：现有「设置页『选择备份的相册』
→ 开始备份」路径（常驻渲染，不受 Idle/AllSafe/Trouble 门控；点「开始
备份」无条件 `saveScope()` + `triggerUserPresentBackup()`，见
`MainActivity.kt:684-723`）已经是可用的手动触发入口，**正式放弃**再建
一个独立「立即备份」常驻按钮的想法——以后任何 agent 看到 `BackupWorker.kt`
里"设置页保留低调立即备份入口（狗粮）"这类注释，都不许当成待办去实现，
那是本卡关闭前留下的过期设计意图，已随本次关闭一并清理。
