# E2E-02 DaemonHelloTest 断言一个 2026-08-08 就废弃的契约，e2e 门禁常红

> 🟡 状态：代码已合并（e2e 门禁已解红），等下次 tag 复核
> 级别：L1 · 阻塞：等下次 tag 复核（卡未移入 `done/` 是簿记漏项，2026-08-21 补横幅）
...
根因：测试停在旧契约上。`DaemonHelloTest.kt` 第一步就断言配对码必须带 `a=`：
...
断言改成验证**新契约**（H-10b 是有意的产品决策——完整 PeerAddr 二维码密到
扫不动——不许为了让测试变绿而退回去加 `a=`）：
...
- [ ] `bash tools/android-hello.sh` 本地跑通（需要 `target/release/daemon`，先 `cargo build --release -p daemon`）→ 期望输出 `HELLO OK: …`
- [ ] 反证：把重建地址那一步改坏（例如丢掉 relay）→ 测试必须红，证明它真的在守 H-10b 的重建链路，而不是恒真式
...
### 修的时候发现：全仓四处同形（2026-08-20）

修的时候发现**全仓四处同形**（DaemonHello / DaemonBackup / NetProbe /
DeviceBackup 都在断言死契约 `parsePairingQr(qr).addr!!`），上一轮只修
一处就宣布"解红"。已抽成共用 `addrOf(qr)`（`PairingQrAddr.kt`），四处
全走它。
