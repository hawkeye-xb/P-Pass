...
旧入库策略把 `originals/` 当成「只有我们能写」的目录：凡是不符合 canonical
日期布局（`<device>/YYYY/MM/`）的文件，入库时一律 `mv` 搬回日期目录。用户
在 Finder 里自建目录给照片分类、或手放新照片进来，都会被我们「纠正」——
第二天发现照片全跑回 `2026/08/`，比丢照片更让人不敢用。

同时 `asset` 表 `rel_path` 没有唯一约束：用户在 Finder 里编辑一张已入库的
照片（内容变了 = 新 hash = 插新行），老行还指着同一路径 → 同一个文件被
两条行占着，照片墙出现两次，其中一张的缩略图和原图都取不出来（thumb 按
hash 存）。旧的严格布局靠「编辑后文件被 mv 走、老路径空了、对账清老行」
意外躲过这个坑——严格布局在这里是个歪打正着的补丁，一旦宽容落位就必然
引爆。

## 期望行为

- `originals/` 底下任何位置的媒体文件都照原样入索引（就地采纳）；`mv` 到
  canonical 日期布局只对「我们自己从手机收到的文件」执行——那些文件此刻
  还在 staging，本来就得给它找个家。
- 一个路径只能被一条索引行占用：入库发现目标路径已被别的 hash 占着 → 删
  老行 + 审计 `asset.replaced_in_place`。
- 手放的文件归属本机（不留空），重建口径与之一致，不破 ADR-006「光靠
  目录树就能完整重建索引」。

## 验收标准

- [ ] 真机：在 Finder 里给已入库照片建目录分类 → 照片留在用户放的位置，不被搬回日期目录
- [ ] 真机：在 Finder 里往 `originals/` 拖入新照片 → 秒级入索引，原地不动
- [ ] 真机：在 Finder 里编辑一张已入库照片 → 照片墙上同一位置只有一份，缩略图/原图都能打开
- [x] `just ci` 全绿，Rust 305/305（合并时证据，见实施记录）
- [x] 反证 5/5 有效（见实施记录）

## 范围

- 只准动：`crates/core-index`（ingest / rebuild / device_of）、`crates/daemon`（watch_flow 测试前提重写）
- 不准动：`.ppf/thumbs` 布局与 daemon 侧 `Reconcile` 职责划分；UI 展示层（目录名对展示零影响，已核过）

## 阻塞与依赖

无。产品方向已由用户 2026-08-21 裁决（见下），按裁决实施。
...
**规则（一句话）**：`originals/` 底下任何位置的媒体文件都照原样入索引；
`mv` 到 canonical 日期布局只对「我们自己从手机收到的文件」执行。
...
这条**不是可选项，不做宽容落位就是引 bug**。机制见「问题」段。改法：入库时
发现目标路径已被别的 hash 占着 → 删老行 + 审计 `asset.replaced_in_place`。
