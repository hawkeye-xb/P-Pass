...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **ARCH-01（L2）备份核心流程：发现队列与严格单张消费** | 2026-08-29 | 本 commit | 🟡 设计已收口，待拆实施卡 | 用户逐轮收口：单文件是上传/确认/失败/暂停单位，批次只作 500 项发现窗口与汇总；触发合并为 `discoveryRequested`，发现器用复合 DiscoveryCursor 原子入队，上传消费者用严格 UploadCursor 单张处理；Pause 只停消费者，hash 仅在轮到当前项时计算；网络/电量为自动等待而非用户暂停；对账独立低频分页，Desktop 外部缺失默认 `NEEDS_DECISION`，不自动补传或删手机。下一轮仅定当前上传项 Pause 时的 Desktop staging / offset / abort 协议；未改生产代码。 |
