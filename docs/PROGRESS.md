...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **ARCH-08（L2）P1 Desktop 存在性探测与分页协议** | 2026-09-01 | 本 commit | 🟠 已认领，尚未写生产代码 | 从 ARCH-01 P1 拆出独立只读 presence query：现有 `backup.manifest` 的 bare-hash 探测会创建/触碰上传 session，旧 `BackupRunner.existCheck` 属冻结批次校准，均不可复用。新卡确定 `backup.presence` + `BackupPresenceQuery(hashes≤500)` / `BackupMissing`：只查询索引，不写 session/audit/watermark/staging/blob，不触发 fetch；Android adapter 不触碰账本、MediaStore 或旧校准。 |
