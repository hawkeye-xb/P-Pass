...
状态：✅ 已归档（实现 `b4713ed`，真机验收 2026-09-17）——**结论：推送在真实传输路径上确实送达，无需修复**
...
**结论先说：推送是好的。** 真实传输路径上 12 项里 **6 项直接由 `flow.delivered`
推送结账**——这不是推断，是日志里 6 行 `Flow resolved by=push`。本卡问的问题
（"推送到底送没送达"）到此有了正面答案，**不需要修复**。

### 真机验收（2026-09-17，三星 SM-S9210，0.5.4-test.6 / versionCode 27）

两端版本已核实：桌面运行中二进制 md5 `9b8ca7cd…` 与新构建逐字节一致、
`lib/` 6 个、`codesign --verify --deep --strict` 过；手机 `-r` 覆盖安装，
`firstInstallTime` 11:18:40 未变、只有 `lastUpdateTime` 变 → 身份与数据保留。

验收人选「Lark」相册。一次操作自然产生了两轮，正好把主验证和反证一起跑了：

| 轮次 | 时间 | 项数 | 判别结果 |
|---|---|---|---|
| ① 真实传输 | 13:28:49–13:28:57 | 12（seq 14–25） | **6× `by=push`** + 6× `by=status` |
| ② 去重命中（重新配对后重发同一批） | 13:29:37–13:29:43 | 25（seq 1–25） | **25× `by=offer_reply`** |

daemon 侧 37 条推送**全部** `peer_subscribed=true`。资产总数 13 → 25
（新增 12，与轮①逐项对上）；`audit_operation` 同窗口 12 条 `ingest.new`。

### 那 6 条 `by=status` 不是推送丢了

六条全长这样：`local=Completed(hash=…) elapsedMs=1`。`elapsedMs=1` 说明等待
循环才转第一圈；`local=Completed` 说明**手机自己的 iroh 发送端事件在循环起步
之前就已经终态了**。`flowWaitStep` 里本地终态直接返回 `CheckStatusNow`
（NET-14 定的「本地判活优先于任何远端回声」），推送连上场机会都没有。
所以这是**本地信号赢了竞速**，不是推送失败。全程 **0 项**碰到 30 秒兜底。

### 反证成立

卡里写的反证判据是「同一台机器再发一批已备份内容，必须读到 `by=offer_reply`；
两种场景日志长一样 = 判别无效」。轮②正是这个场景，25 项全部 `by=offer_reply`，
与轮①的 `push`/`status` 完全分开——**判别真的能区分，不是走过场**。

### 一条必须写明的局限

`peer_subscribed=` 是**按 NodeId** 查的，不是按本次尝试查的。连续轮次里，
第 N 项 offer 时第 N-1 项的订阅可能还挂在表上，所以它可能「陈旧地为真」。
**它不能用来否定 NET-24**——NET-24 说的是「本次尝试自己的订阅还没建立」，
两者不矛盾。本卡的结论站在 6 行 `by=push` 上，那是直接证据，不依赖这一维。

### 顺带产出

同一份日志坐实了一个新缺陷：重新配对完成后不自动开始传输，必须杀掉 App 重开。
已拆 [MOB-87](../MOB-87-repairing-does-not-wake-flow-until-app-restart.md) 挂号。
