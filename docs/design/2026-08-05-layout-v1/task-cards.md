...
## 卡号 T-092  级别 L2 — 桌面接真数据·第二批（T-090 新字段接线）

**前置**：T-090 已合并进 main（634f8a3a）。

**目标**：把 T-090 暴露的四类数据点亮到桌面 UI：
1. 设备行 sub 槽接 `devices.list[].connection`：direct→「已直连」safe 绿点；
   relay→「经中继连接——内容加密，中继无法读取」WAIT 色点（设计稿原文话术）；
   offline→「离线，最后在线 <人性化时间>」IDLE 点；unknown→保持 T-082 中性占位。
   哨兵亮红（T-091 的 >5 天规则）优先级高于连接态展示
2. 活动记录页接 `activity.list`：批次行「<设备名> 备份了 N 张 · <人性化时间>」
   倒序；空态文案保留；页脚设计稿原文「记录只在本机保存 90 天，不上传。」
   暂不上（90 天保留未实现，不许承诺）——改为「记录来自本机照片库，不上传。」
3. 总览页副标题接 `status.photo_count`：「已配对设备 N 台 · 照片库 M 张」
4. 设置页照片库卡接磁盘水位：`disk_free_bytes/disk_total_bytes` 渲染为
   「可用 X GB / 共 Y GB」+ 细进度条（token 色）；字段为 null（非 unix）时整行隐藏

**范围**：`apps/desktop/src/**`。**不准动**：`src-tauri/**`、`crates/**`、已有测试。

**可执行验收**：
  - `pnpm --dir apps/desktop build` → 退出码 0
  - 纯函数（字节→GB 格式化、connection→文案映射）node 断言脚本 ≥6 用例全过
  - 反证：connection 映射断言故意换错色 → 红 → 改回
  - 真 daemon 实机复核 = 验收人做
**证据要求**：报绿附命令+输出。收尾：分支 `t-092-desktop-wire` 提交，停下等 review。
