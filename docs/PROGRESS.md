...
| **MOB-68（L2）可选授权映射到备份设置** | 2026-09-10 | 本批次 | ✅ 真机通过 | 首装时媒体范围仍是唯一必需授权；选完范围只保存、不扫描不传输，用户点“进入 App”后才串行处理电池白名单、通知权限，二者结束后才启动首轮备份。后台备份移入备份设置，三星实测开启时后台截图可同步、关闭时后台不触发，重新打开 App 由前台路径补传。失败通知发送独立留给 MOB-67。 |
