...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **REBUILD-04（L2）Worker Flow cutover 与最小状态呈现** | 2026-09-01 | `a325208` | 🟡 代码完成，待三星测试相册验收（Android JVM **250 tests / 0 failures / 4 skipped**；debug APK） | `BackupWorker` 已是 Flow wake adapter，不再 scan/hash/manifest/push/commit；UI 从 ledger 区分 user pause、constraints waiting、transfer 与 cancelled round，Continue/Cancel 走 strict consumer。旧批次机制测试按 REBUILD-00 冻结；唯一欠账是三星的 Pause → kill → Continue → Cancel。 |
...
| **备份核心重建主线（REBUILD-00~04）** | 2026-09-01 | 本 commit | 🟡 生产纵切代码完成，待三星首验 | legacy/flow 边界、Android blobs provider、Desktop fetch/receipt、Flow runner 与 Worker cutover 已完成；仅欠测试相册上的 Pause → kill → Continue → Cancel。P2/P3 不再占用主线。 |
