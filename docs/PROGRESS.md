...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **E2E-01** | 2026-08-04 | — | 🔄 PR 待 review | android live 剧本进 CI（M3）：`.github/workflows/e2e.yml`（新增）——nightly cron 03:30 UTC + **release tag 产物构建前门禁**（被 release.yml 以 reusable workflow 调用，macos/windows/android 三 job `needs: [e2e]`）+ PR 打 e2e label / 手动 dispatch；**每 commit 不跑**（PR 常规 push 不触发）。三剧本脚本（tools/android-hello/pair/backup.sh）最小适配：`PPF_BIND_ADDR` 0.0.0.0:0 → 127.0.0.1:0（卡面已知坑，回环绑定）。已知坑排查：**iroh Maven jar 1.1.0 确认含 linux-x86-64/libiroh_ffi.so**（JVM 测试无需模拟器）；ubuntu codecs 抄 scenarios job（libheif-dev + libheif-plugin-libde265 + ffmpeg + libnuma-dev）；JAVA_HOME 由 setup-java 提供（脚本 `${JAVA_HOME:-brew}` fallback 在 CI 不触发）。本地验证：三剧本 127.0.0.1:0 下 3/3 全绿（HELLO OK / PAIR OK / BACKUP OK pushed=12 ingested=12 幂等 0）+ actionlint 1.7.7 双 workflow 零报错。验收（待 dispatch）：run 全绿 + HELLO/PAIR/BACKUP OK 12/12 见于日志 + 反证改坏 hello 响应 job 必红。 |
