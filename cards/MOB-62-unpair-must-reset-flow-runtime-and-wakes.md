...
> 🟡 状态：代码完成，待完整断开→重扫三星回归 · 当前节点：native 初始化已移出 `flowRuntimeLock`；下一步：重扫同机后验证无 ANR/旧 offer 且新轮传输 · 协同分支：`main`
> 级别：L2 · 阻塞：无
...
- 2026-09-14 修复：`runtimeFor()` 先在短锁内只读现有 runtime；耗时的 native provider 初始化完全在锁外完成，最后再以短锁发布候选 runtime（若另一线程已发布或配对 epoch 已变化则关闭候选）。新增 RED→GREEN 合同测试锁定“native open 必须发生在第一段 runtime-map 锁之后”。三星在修复版上选择一张隔离相册图片并开始备份，`1/1` 成功、无 ANR；随后取消选择并删掉测试图片。完整断开→重扫仍待回归。