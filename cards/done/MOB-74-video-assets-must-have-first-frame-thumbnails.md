...
> ✅ 状态：真机验收通过（2026-09-14，macOS 本机 P-Pass.app 覆盖安装，daemon versionCode 0.5.1-test.1，三星 SM-S9210 消费端）
> 级别：L2 · 阻塞：无
...
- [x] 真机：至少两个不同视频在照片墙中显示各自可辨认封面；打开任一个仍可
      正常播放，不把图片解码器错误套到视频上。（macOS 本机构建 daemon
      0.5.1-test.1 覆盖安装，接三星真机核实：Windows 不在 B
...
2026-09-14，macOS 本机验收：

1. **构建/部署**：`cargo build --release --bin daemon` →
   `tools/bundle-macos.sh`（自包含 dylib 打包，ad-hoc 签名）→
   `tools/bundle-desktop-macos.sh`（因无 `TAURI_SIGNING_PRIVATE_KEY`，
   updater 签名步骤失败属预期——本地无凭据路径，非 bug；`.app` 本体已
   在该步骤之前产出，手动补做 lib 嵌入 + 重签），覆盖安装
   `/Applications/P-Pass.app`。`launchctl print` 确认 launchd 托管的
   `com.p-pass.daemon` 已用新二进制重启，`ppf-daemon --version` 报
   `0.5.1-test.1`（此前跑的是 `0.5.0-test.8`，不含本卡修复）。
2. **发现两个真实视频资产**（个人库 `~/Pictures/P-Pass 家庭照片库`）：
   `VID_20260329_151742.mov`、`Health20240811161633complete.mp4`，两者
   `thumb_state=2`（失败），磁盘缩略图为 256×256/3.6KB 灰色占位图——与
   卡内根因描述的「静默占位图」完全吻合。
3. **验证步骤**：删除两个视频的 `.ppf/thumbs/` 缓存文件、
   `UPDATE asset SET thumb_state=0`，重启手机 App 强制拉取。
4. **结果**：两个视频在 Android 照片墙分别显示为可辨认首帧（考拉爬树、
   地图截屏），不再是空白格；磁盘缩略图文件从 3.6KB 占位图变为
   19–221KB 真实图像数据，`thumb_state` 变回 1（成功）；点开考拉视频
   确认正常播放，画面清晰，未把图片解码器错误套用到视频。
5. **顺带发现（不在本卡范围，已开新卡 MOB-78）**：照片墙里另有一张白框
   来自 `MOB-67` 测试期间留下的假 JPEG 测试文件（59 字节、混进生产库
   目录），与视频白框现象类似但成因完全不同，不可归入本卡验收证据。
