...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **ARCH-07（L2）P1 远端对账事实与恢复裁决** | 2026-09-01 | 本 commit | 🟠 已认领，尚未写生产代码 | 从 ARCH-01 P1 拆出 R-01/R-02 本地账本卡：完成凭据需携带可对账内容身份；远端缺失分别写为 `NEEDS_DECISION`（手机源仍在）或 `UNRECOVERABLE`（两端均缺）。卡面明确禁止沿用旧 `ConfirmedStore` / `ReuploadQueue` 批次校准、自动重传、WorkManager、UI、proto 或 Rust/desktop；下一步先写两条失败合同与反证。 |
