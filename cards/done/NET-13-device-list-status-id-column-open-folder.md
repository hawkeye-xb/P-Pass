...
> ✅ 状态：已完成 + 本机真机验证通过（真实库 5 台设备、含 4 台
> Flow-only 备份场景实锤），待多设备/跨版本长期回归观察
...
- [x] 真机/真实多设备场景回归：本机重新构建 release daemon +
      桌面壳，替换 `/Applications/P-Pass.app`（`launchctl unload/load`
      让 LaunchAgent 接管新二进制，md5 核实与本次构建产物一致），
      真实打开桌面 App 截图确认：5 台已配对设备（4 台三星 SM-S9210 +
      1 台华为 Mate60pro12）ID 列显示 8 位短指纹（`#fa1c2181` 等）、
      在线时间独立成列（`12 小时前在线`）、"最近备份"列显示真实时间
      （`昨天 21:39` 等）。其中一台 `#a16218d4` 显示"还没备份过"——
      直接查询本机库 `index.sqlite`：`asset` 表按该 `src_device`
      过滤行数为 0，`backup_watermark` 也无该设备记录，确认是真的
      从未传过照片，不是误报；第 1 条根因修复的判定逻辑在真实多设备
      数据（含新旧两种来源共存的场景）上表现正确。打开文件夹图标
      正常渲染（未逐台点击验证 Finder 揭示行为，退回路径逻辑见代码）。
      **关键实锤**：直接查询本机真实库，5 台在用设备的 `backup_watermark`
      记录数全部为 0（该表整体是空的），其中 4 台 `asset_cnt > 0`——
      本机库 100% 走 Flow-only 备份，改前这 4 台全部会被误判"还没
      备份过"，改后全部显示真实备份时间，与截图结果一致。
