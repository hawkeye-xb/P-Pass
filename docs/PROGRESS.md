...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **NET-20/NET-21（L1）Flow 单通道两处 NET-06 重构漏项：offer 传输前哈希对齐（开卡）+ flow-staging 孤儿回收（已修复）** | 2026-09-16 | 本 commit | 🟡 NET-21 已修复+`just ci` 全绿；NET-20 仅开卡未做 | 用户对话核实发现 NET-06 把 Flow 重构成单通道（offer 秒回+后台 spawn+轮询 status）时漏掉旧批量管线两项能力。**NET-20**（未做，仅开卡）：`offer_inner` 从不查 `db.get_asset(&hash)`，内容已存在也会整份重传字节，事后才靠 ingest 判重丢弃。**NET-21**（已修复）：Flow 导出中转目录 `.ppf/flow-staging` 没有孤儿回收——`export_to` 落盘的文件只在 ingest 成功分支被删，ingest 真失败/materialize 中途 cancel/daemon 崩溃都会留下孤儿，且此前无任何定时任务扫描（旧上传收件箱 `.ppf/staging` 有 `sweep_orphans`+`reclaim_staging`，新装卸台没接上）。修复：`inbox.rs` 新增 `sweep_flow_staging_orphans`（判据同构：命名不合契约/落地不足宽限期/`protected` 内的 content_hash 均不碰），`main.rs` 启动时+每小时各调一次，保护集直接复用 `Db::active_flow_content_hashes()`（与 flow-blobs 的 iroh GC 回调同一张持久表，不是另起判据）。RED 先行 5 条纯函数单测+1 条反证（临时禁用保护判据后 `flow_staging_orphan_sweep_respects_every_guard` 从 9 变 16，证明测试真的锁住防线；已恢复）。`cargo test -p daemon --lib inbox::` 10/10、`cargo test -p daemon` 全量无回归、`just ci` 全绿。版本 bump `0.5.4-test.1`→`0.5.4-test.2`。**留白**：NET-21 未做真机长跑验证（故意制造 materialize 失败/cancel 场景确认目录清零）；NET-20 需要用户拍板"命中已存在内容时 receipt 语义"后才能开工。 |
