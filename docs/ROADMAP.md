...
- [ ] **REBUILD-04 Worker 切换、最小状态呈现与首包验收** — **2026-09-01 代码完成，待三星首验**：Worker 已降为 Flow wake adapter，UI 从新账本投影 user pause / waiting / transferring / cancelled round，Continue / Cancel 走严格消费者。Android JVM 250 tests / 0 failures / 4 skipped、debug APK 成功；仅剩测试相册的 Pause → kill → Continue → Cancel 真机验证。
- [ ] **REBUILD-04 三星生产首包补验** — **2026-09-01**：三星测试相册 26 项已通过新 Flow 原生 provider → Desktop fetch/receipt，Desktop durable index 25 项（去重）；实测修复独立 provider endpoint、receipt 自动推进 strict head 与 legacy watch crash 隔离，Android JVM 254 tests / 0 failures / 4 skipped、`just ci` + deny 通过。仍只欠 Pause → 杀 App → Continue → Cancel 的精确真机行为。
