...
| **MOB-50（L1）取消本轮后 upload cursor 卡死** | 2026-09-02 | 本 commit | ⬜ 未开工 | REBUILD-05 三星真机验收发现的独立缺口：`CancellationRoundController.startPausedRound` 不重置 `uploadCursor`，而 `StrictConsumer.headOf` 要求游标指向的队列位精确匹配 `QUEUED`，取消后该队列位状态已变，导致取消本轮之后新入队的任何后续项永久卡在 `QUEUED` 不被消费（真机复现：item #34 在 #33 被取消后加入，多轮自动 wake 均未触发传输）。 |
