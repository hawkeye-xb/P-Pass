...
## 2026-09-15：家庭真机回归，鸿蒙 5G 关 Wi-Fi 场景疑似再次撞见本卡

用户当天在家用 Mac mini + OPPO + 鸿蒙 4.2 三机回归后，把鸿蒙关闭 Wi-Fi、
切 5G 测试：观察到「加载元数据慢，传输内容慢」，用户判断疑似走了中继。
**这是本卡此前"暂缓，等可持续蜂窝窗口"要等的场景**，但本次只是定性观察，
未采集 `devices.list` 的 `connection` 字段、daemon debug 日志或耗时数字，
不能作为判决证据回填本卡因果链，也不能据此判断是否已被后续改动（如
