...
- [ ] schema migration 将 Flow grant 身份限定为 `(node_id, pairing_epoch, queue_sequence)`，已存在库升级不丢 receipt。
- [ ] RED→GREEN：同一 NodeId 的旧 epoch `completed` 序号 1 存在时，切换到新 epoch 后同序号 1 的 offer 可建立独立 grant；旧 receipt 仍可按旧 epoch 读取。
- [ ] 反证：临时移除 epoch 维度后，上述同序号重配对用例必红。
- [ ] 当前 epoch 以外的 offer/fetch 仍返回 GuardMismatch；同 epoch recovered lease 的 completed receipt 仍可重放。
- [ ] `cargo nextest run -p storage`、`cargo nextest run -p daemon --test flow_delivery` 与 `just ci` 全绿。
