# NEXT — 当前状态与下一步（2026-08-20 深夜，WATCH-02/03 已修）

> 交接件，随每次收口更新。历史结论已并入 ROADMAP/PROGRESS。

## 〇、下个 session 从这里接（2026-08-20 深夜收口）

### 刚做完（已推送）

| 卡 | 事情 | 证据 |
|---|---|---|
| `WATCH-02` 🟡 | Finder 删照片索引无反应 —— **根因是一个斜杠** | 三种删除形状全覆盖；反证 8/8 全红；Rust 301/301 |
| `WATCH-03` 🟡 | **Finder 挪动照片导致照片凭空消失**（比 02 严重） | hash 是身份、rel_path 只是住址；库内移动改指不删行 |

**WATCH-02 根因**：整棵子树被删时 `affected_dirs` 收敛到 `originals` 本身 →
`rel` 是空串 → `prefix = "originals/"` → `list_asset_paths_under` 再补 `/%` →
`LIKE 'originals//%'` → **命中 0 行**。卡面三条假设全错。删单文件 `rel` 非空
所以测试一直绿——**测试形状和用户操作形状不一样**。

**WATCH-03**：用户在 Finder 里给照片分类（拖进自建目录）→ `ingest` 返回
`Duplicate` 不更新 rel_path → 对账见旧路径不存在 → 删行删缩略图 → 照片消失且
**再也回不来**。顺带补掉「手机重传一张曾被外部删掉的照片永远补不回来」的洞。

### ⚠️ 等用户拍板的一件产品决定

**用户新拖进 `originals/我的相册/` 的照片（索引里还没有这份内容的），要不要被
我们搬到 `originals/<device>/2026/08/`？**

- 现在的行为：**会搬**（`place()` 无条件按日期布局落位）
- 已修的 WATCH-03 只覆盖「索引里已有这份内容」的情况（那种就地采纳，不搬）
- 这是产品语义不是 bug，**没动，等拍板**

### 遗留：移动识别的性能（未开卡）

识别「移动」现在靠重算 hash。用户一次拖 5000 张进新目录 = 重读 5000 个文件。
正确但不便宜。省掉需要把 `(dev, inode, size, mtime)` 存进索引，stat 没变就不
重算——Finder 移动**保留 inode**，可直接判定。schema 变更，另立卡。

### 遗留：watcher 没有健康检查（未开卡）

`spawn()` 只在启动时挂一次监听，之后 keep-alive 线程只是 `sleep(3600)`。
FSEvents 流意外中断（卷卸载、监听根被删后重建）**没有任何人发现**，唯一的网是
SYNC-01 每小时全量对账。要「及时」就得有个心跳去确认监听还活着，断了重挂。

### 手上的环境状态

- 桌面壳：`npx tauri dev` 在跑；**daemon 二进制需要重新编译加载**（本轮改了
  daemon）
- 手机：`0.3.5 (10)`，全新安装，需要重新扫码配对
- 库：`~/P-Pass NAS`，被用户手动删空
- **旧数据副本 `~/P-Pass NAS.bak-blob01-20260820-1615`（1.1G，含 549M 照片）
  还在，等你确认后自己删**——我不动它

### 上一轮（2026-08-20 傍晚）做完的

`MOB-27` ✅ / `MOB-28` 🟡 / `MOB-19` 🟡 / `E2E-02` 🟡 / `BLOB-01` 🟡 / `DESK-09`。
**上线阻塞清单三件必做全部完成**（见 `docs/product/2026-08-20-launch-blockers.md`）。
真机验收欠账：MOB-28（强制停止流程）、MOB-19（暂停/再试一次）、MOB-09、MOB-13、
ICON-02、BLOB-01。
