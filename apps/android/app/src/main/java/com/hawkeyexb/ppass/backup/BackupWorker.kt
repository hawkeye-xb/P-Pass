...
// 下一个触发事件。首页「现在备份」主按钮删除；MOB-43（2026-08-27 关闭）
// 已明确不建独立的常驻「立即备份」入口——手动触发走「设置页选相册→
// 开始备份」（见 [triggerUserPresentBackup] 的调用点），本函数（手点
// 立即备份）目前只服务暂停后「继续」/失败重试等有 UI 上下文的场景。
...
/** MOB-19 事件⑥：用户手点「立即备份」（暂停后「继续」/失败重试等场景，
 *  不是常驻设置页入口——MOB-43 已拍板不建那个入口）。
 *
