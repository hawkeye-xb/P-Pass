...
| **MOB-49/MOB-50（L1）取消本轮两处生产接线缺口开卡** | 2026-09-02 | 本 commit | ⬜ 未开工 | REBUILD-05 三星真机验收过程中发现的两个独立衍生问题，均已记录真机证据并开卡，未在 REBUILD-05 内修复：**MOB-49**——`CancellationRoundController.finishRound/restoreRound/discardRound` 生产代码零调用点，取消本轮后 UI 永久卡在「当前轮已取消」，只有配对 epoch 变化才会清除；**MOB-50**——`CancellationRoundController.startPausedRound` 不重置 `uploadCursor`，而 `StrictConsumer.headOf` 要求游标指向的队列位精确匹配 `QUEUED`，取消后该队列位状态已变，导致取消本轮之后新入队的任何后续项永久卡在 `QUEUED` 不被消费（真机复现：item #34 在 #33 被取消后加入，多轮自动 wake 均未触发传输）。 |
