...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **批次 A（REL-03 / I18N-01 / DESK-09 / BUILD-02）** | 2026-09-01 | 本批次合并 commit | 🟡 代码已合入；等一次批次 CI 与 2 项真机验收 | 并行 worktree 完成四张独立卡后统一合入：REL-03 给每个版本目标独立读取/断言并验证 lock；BUILD-02 让 5 个 workflow 的 13 个 Cargo job 从 `rust-toolchain.toml` 派生工具链且有 TOML 反证；I18N-01 将空相册名交给资源渲染，英文真机验收欠账；DESK-09 透出 daemon stderr 并为迁移不兼容给出升级提示，旧 daemon/新库真机验收欠账。批次本地验证：`just ci`、版本脚本正反例、工具链脚本反证、Android JVM **385 tests / 0 failures / 4 skipped**、desktop vitest 26、vite build、desktop lib 16 均通过。 |
