# NEXT — 当前状态与下一步（2026-08-20 晚，三件上线阻塞已清 + 两处新发现）

> 交接件，随每次收口更新。历史结论已并入 ROADMAP/PROGRESS。

## 〇、下个 session 从这里接（2026-08-20 晚收口）

### 这一轮做完的（都已推送）

| 卡 | 事情 | 证据 |
|---|---|---|
| `MOB-27` ✅done | 监听与干活分家 | 真机监听空窗 **32 毫秒**（旧实现 = 整个备份时长）；你实测连拍/备份中拍照/重启三项通过 |
| `MOB-28` 🟡 | 强制停止后只提示不自动恢复 | 真机端到端：force-stop → 打开 App → 看门 job **没被自动装回去** + 提示卡出现 → 点「恢复备份」→ job 回来。**你还没肉眼确认过** |
| `MOB-19` 🟡 | 备份只有一条管线，手动是又一种触发 | `BackupUiStateHolder` 298→199 行，第二条管线删除；手动档零约束（点了就跑，不过 Wi-Fi 检测） |
| `E2E-02` 🟡 | 发版门禁解红 | 起临时 daemon 实跑 `HELLO OK`。⚠️ 全仓**四处**同一死契约，第一轮只修一处，第二轮才补全 |
| `BLOB-01` 🟡 | 占盘翻倍 | **实测 2.05x → 1.00x**。真实数据：186 张 561MB，收件箱 **0 字节** |
| `DESK-09`（未开卡，代码已改） | 照片墙同步 + 活动记录布局 | 桌面端从零建了测试地基（vitest，15 测试 / 11 反证），挂进 ci-desktop |

**上线阻塞清单三件必做全部完成**（见 `docs/product/2026-08-20-launch-blockers.md`）。

### ⚠️ 两处新发现，下个 session 的主话题

**① `WATCH-02`（新开卡）—— 手动删 originals 里的照片，索引与界面无反应**

用户 Finder 删光 185 张，`index.sqlite` 里 186 条**一条没减**。当场取的证据已
排除三个环境因素（daemon 在跑、二进制确认是新代码、防抖只 500ms）。卡里列了
三条待验假设（监听句柄在整棵子树被删后失效 / 前缀对账口径对不上 / 被"ingest
瞬态 remove"的过滤吞掉）和为什么现有集成测试没拦住（它删的是单文件，用户删的
是整棵目录）。

**这个 bug 一直没被发现的原因**：SYNC-01 每小时全量对账能最终一致——等一小时
就自己好了。

**② 照片墙同步的判据这一轮重写过，但 daemon 侧不发事件时它也无能为力**

`photoWall.js` 的三向合并已经能正确处理删除，但前提是 daemon 发出
`timeline.invalidated`。①没修好之前，②在删除场景下看不出效果。

### 手上的环境状态

- 桌面壳：`npx tauri dev` 在跑，托管的 daemon 二进制已换成新编译版
  （原来是 **8月14 的旧包**；8月17 起的那个进程攥着已移走的旧库，已停掉）
- 手机：`0.3.5 (10)`，全新安装，需要重新扫码配对
- 库：`~/P-Pass NAS`，本轮备份过 186 张后被用户手动删空
- **旧数据副本 `~/P-Pass NAS.bak-blob01-20260820-1615`（1.1G，含 549M 照片）
  还在，等你确认后自己删**——我不动它

### 这一轮踩到的方法论问题（都已写进相应文件，防重犯）

1. **卡的位置不是事实，代码才是** —— 一次盘点里连续误报三次（DESK-06 /
   SYNC-04 / ICON-02 代码早就合了，只因卡还在待办目录）。已在
   `.claude/cards/README.md` 立状态横幅规矩。
2. **切片断言必须夹出边界** —— Kotlin 的 `substringAfter` 找不到分隔符时返回
   **整个字符串**；即使找到了，不给右边界也会把后面所有函数带进来。两次恒真式
   都是这么来的。
3. **"这个测试挂了"要先问"还有几个同形的"** —— E2E-02 的死契约有四处，只修
   撞到的那一处等于没修。
4. **zsh 不对未加引号的变量分词** —— 反证脚本 `for f in $ALL` 让备份/还原静默
   失败，18 次破坏叠加在工作区上。驱动已改成 Python 内存快照
   （`scratchpad/cp.py`）。
5. **反证驱动只认 FAILED 行会把编译失败当成绿。**
6. **`grep -c` 数 dumpsys 的 job 会把历史记录算进去**，必须按 `JOB ` 分块解析。

### 队列（可后置）

- `SYNC-05` 分类器（用户已降级）
- Windows 从没在真机跑过（流水线里有 job）
- 视频缩略图 daemon 侧生成失败 → 灰块
- 「ingest 后批量生成缩略图」挂账未做——现在是按需生成，你库里 171/186 张
  还没生成过
- `MOB-09` / `MOB-13` / `ICON-02` / `MOB-19` / `MOB-28` 的真机验收挂你那边
- `BucketScreen.kt:81` lint 红（CI 不跑 lint）
- `manual_backup_entry` 死文案去留待拍板
