...
Telemetry intake (T-035/T-061 dictionary v2 — OBS-02 裁决 2026-09-10): strict
zod validation → Cloudflare Analytics Engine. The Rust client
...
遥测入口（T-035/T-061 字典 v2，OBS-02 裁决 2026-09-10）：严格 zod 校验 →
Cloudflare Analytics Engine。Rust 客户端（`crates/daemon/src/telemetry.rs`，
T-035）POST 一个 JSON **数组**（扁平事件对象）；本 Worker 对每个事件做严格
schema 校验，通过后整批写入 Analytics Engine。裁决记录见
`cards/done/OBS-02-telemetry-event-dictionary-usefulness-review.md`
（删除 v1 的 `ipver`/`country`/`isp_hash`/`files`/`trigger`，新增 `error`）。
...
漂移）——`conn`=[ts,ms]、`flow_item`=[ts,bytes,dur_s]、
`first_byte`=[ts,ms]、`daemon_alive`=[ts,uptime_h]、`error`=[ts]
（double1 恒为 ts，double2 为主指标；新增数值字段只允许追加到行尾）；
`blobs: [完整事件 JSON]`（自描述、无损）。
