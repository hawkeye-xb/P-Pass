...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **MOB-67（L2）失败系统通知接回 Flow 终态跃迁** | 2026-09-11 | `332e7ee` | 🟡 代码完成，待三星真机 | 根因考古：不是从未实现——UX-02（PR #36）曾在 BackupWorker 有完整通知设施，REBUILD-04（`a325208`）删 1124 行时随批次管线一起消失，`NotifyOnFailurePrefs` 只剩设置页自读。修法：`StrictConsumer.recordPermanentFailure()` 在 `ledger.update` 突变内部捕获 attempt-3 终态跃迁并返回；`FlowRunner` 跃迁后且开关开启时从账本数 `FAILED_NEEDS_USER` 总数发一条固定 id 通知（第二次失败是更新不是再响；`runCatching` 吞发送异常——账本先落盘，通知是补充渠道，MOB-37 裁决同构）；新 `FailureNotifier` 接口 + `SystemFailureNotifier`（沿用历史渠道 `ppass.backup.failed`/id 2027，用户已调的渠道设置不丢），渠道名进 en/zh 字典。瞬态重试（attempt 1/2）保持沉默。新增 `MOB67FailureNoticeTest` 3 例；全量 JVM **337/0/4 ignored**（65 XML 本次生成）、`just ci` 全绿、queue-check 绿。衍生发现：SENT-01/DOG-02b/MOB-29 三条通知发送端同批被删且至今零调用点（字符串与 ROADMAP 状态均与代码漂移），开 MOB-69。 |
