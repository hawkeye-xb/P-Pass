...
若第 2 问答案为否（同网段也打不通），iOS 的后台备份就只剩「手机把字节交给一个公网 HTTP 端点，再由它转给桌面」。这条路要先回答四件事，回答之前不要开卡：

| # | 问题 | 为什么它是拦路的 |
|---|---|---|
| R1 | **它和现有 relay 不是一回事**。`infra/relay/` 的三区 relay 是**无状态**的，只转发加密 QUIC，要求两端同时在线；而上传扩展是一次性 HTTP PUT，桌面大概率不在线 → 必须**落存储**（有状态、有留存期、有成本） | 名字都叫 relay，语义完全不同。混淆会导致「我们不是已经有 relay 了吗」这种错误前提 |
| R2 | **端到端加密怎么保住**。照片落在我们的存储上，必须是只有配对桌面能解的密文（密钥由配对派生），否则产品的隐私claim 不成立 | 一旦明文过境，P-Pass 就从「本地备份」变成了「云相册」，这是产品定位级别的改变 |
| R3 | **`CONFIRMED` 语义冲突**。ARCH-01 写死「一张照片只有在 Desktop 完整接收、验证、可靠保存并明确确认本地拥有后才是 `CONFIRMED`」。spool 只能回执「spool 收到了」，回执不出「桌面拥有了」 | 需要在账本里新增一个中间态（已入 spool ≠ 已确认），并定义 spool 丢失/过期时该项回到哪一态 |
| R4 | **成本与留存期**。桌面一个月不开机，照片在 spool 上放多久？谁付存储与出网费用？超期怎么办 | 这是运营决策，不是工程决策，定不下来就没法定容量与判据 |

R2 与 R3 各自都可能否掉整条路线，所以它们排在任何实现工作之前。
