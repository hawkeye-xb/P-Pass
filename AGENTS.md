...
## 任务状态诚实与拆分纪律（2026-09-16 用户定调：禁止"进行中"僵尸卡）

> 事故：处理 NET-06 时发现方案不对，讨论中开出 NET-14 去做修正，NET-14
> 做完归档后 NET-06 却还挂着"进行中"——下次对齐任务又撞上 NET-06，且它的
> 剩余项与 NET-14 混在一起说不清谁该收尾谁。根因：session 上下文有限，
> 处理 A 时延伸出的新问题被顺手开成 B 就去改 B，A 自己没有被同步归位，
> QUEUE 里的"进行中"从此失真——看着像有人在做，其实没人管。

1. **"进行中"只在有人真实盯着的这次会话内成立。** 会话结束、上下文耗尽、
   或转去处理会话内新分出的卡时，若原卡未到可归档状态，必须把
   `docs/QUEUE.md` 里它从「进行中」挪回准确类别（可接队列 / 待共享回归 /
   待你拍板等），如实写清"做到哪、剩什么、下一步"。不允许一张卡挂着
   「进行中」却没有人在动它——那只会让下一个 agent 或验收人对齐时反复
   撞见同一张僵尸卡。
2. **处理 A 时发现的可独立验证/独立排期的新问题，必须新开卡，不许在同一
   次 diff/会话里直接改去修 B 而放着 A 不管。** 新卡必须显式标注"从
   <A 卡号> 分出"；原卡也要反向加引用并回答两件事：哪些验收项仍然是 A
   自己的、A 现在的 QUEUE 状态是否也要因此改成「可接队列」或「阻塞」——
   不能只做"防止 B 被误认成 A 已完成"这一半（NET-06/NET-14 今日互加的
   双向引用就只做了这一半，是本条要补全的反面教材），不能靠新卡的存在
   就把 A 无限期挂起在"进行中"。
3. **有父子/引用关系的卡，任何一方开卡或关卡（状态终态↔非终态互转）时，
   必须同一次提交里同步检查并更新所有关联卡的横幅引用与 QUEUE 对应行**，
   不能只更新自己那张——这正是今天 NET-06 卡内容已经更新但 QUEUE 索引行
   仍停在开工时的旧描述、导致状态漂移的复发点。
4. **一张卡若因为"被拆出去的子卡还没验收完"而暂时不能关，不等于它可以
   继续挂"进行中"**：此刻没有人在实际处理它，应准确标为"可接队列"或
   "阻塞：等 <子卡> 验收"，让队列反映真实的"谁在做/没人做"，而不是
   "看起来有人在做"。
