...
- [ ] **REBUILD-04 Worker 切换、最小状态呈现与首包验收** — **2026-09-01 代码完成，2026-09-02 真机验收阻塞**：Worker 已降为 Flow wake adapter，UI 从新账本投影 user pause / waiting / transferring / cancelled round，Continue / Cancel 走严格消费者。Android JVM 250 tests / 0 failures / 4 skipped、debug APK 成功；范围扩展补扫修复前，不能开始 Pause → kill → Continue → Cancel 真机验证。
- [ ] **REBUILD-04 三星生产首包补验** — **2026-09-02**：scope backfill 已将游标前 4 项补入，ledger 从 26 收敛至 30 `CONFIRMED`。为 Pause 新增的测试媒体随后遇到 `flow.fetch` 15 秒无响应和 Desktop `err.not_authorized`，严格队头仍可重试但不能进入 Pause 流程；真实验收尚未通过，等待 REBUILD-06。
...
- [ ] **REBUILD-06 新 Flow 对新媒体的 offer/fetch 授权与回执一致性** — **2026-09-02 发现，未开工**：有效配对下的新 Flow 队头先 timeout、再被 Desktop `err.not_authorized` 拒绝；手机严格队头保持可重试，但 Desktop/手机事实不能解释收敛。定位 authz 与 receipt 幂等后，才继续 REBUILD-04 Pause → kill → Continue → Cancel。
