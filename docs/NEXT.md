...
> ⚠️ 这份文件**不再是待办入口**。「现在该干什么、按什么优先级」唯一看
> [`docs/QUEUE.md`](QUEUE.md)。本文件只保留诊断过程/根因/教训这类背景，
> 随收口追加，历史结论已并入 ROADMAP/PROGRESS。

## 〇、2026-08-27 验收人 7 条真机反馈 → 6 张新卡 + MOB-26 解冻

全部按模板开卡并进 QUEUE.md 可接队列，明细见 PROGRESS 2026-08-27 行。
⚠️ 编号说明：与并行 session 撞号（MOB-42/43、NET-02 已被占），本批最终
编号为 **MOB-44/45/46、NET-03、UI-07/08**。要点：

- **MOB-26 解冻**：验收人重提「开源图片查看库」，8-19 的「拍板暂不做」作废，
  已从 backlog 移回待做队列，卡内补了候选库调研（Telephoto / ZoomImage /
  metadata-extractor 等）。MOB-25 维持暂不做。
- **两张 L1 优先**：MOB-46（相册计数说谎，恒 +4）与 MOB-44（鸿蒙恢复备份
  无后台）——后者需要鸿蒙真机取证窗口，等验收人约时间。
- **NET-03 取证第一**：闲置时审计被连接事件刷屏，先定性「真抖动 vs 误记」
  再动手；⚠️ PRES-01 在读 `device.connected` 做 10 分钟去重，记录口径是
  业务输入，不能当展示噪音随手删。
- **UI-07 当天定性**：不是徽章丢了，是小 icon 用错版本（引用了不带闪电
  标识的 icon），阻塞在等验收人给修改指示。

---

## 〇ter、2026-08-26 test.8 真机回归（24 步）+ UX-14 已实施

**MOB-40 通过** —— 只选那个 11 张的相册，全程只传 11 张，配对到选完相册之间
零传输。这是本轮的主判据。

**UX-14（L1，当天修完）**：验收人「怎么传一半自己暂停了，按道理我没碰到」。
不是误触 —— 失败走 `Result.retry()`，WorkManager 的 retry **结构上拿不到
outputData** → 那一轮不盖 `KEY_FINISHED_AT` → UX-13 的判据认为那次暂停「还没
被覆盖」→ 又显示「继续」。判据锚点从「最新完成时刻」扩到「最新**开跑**时刻」
（新增 `RunStartPrefs`，worker 在抢到互斥门之后、扫描之前落盘）。
46 类 / 347 tests / 0 failures，反证两红。

**另外三张新卡（都由日志定性，不是猜的）**：

- `DOG-03`（L1）：`FreecessController: ppass (Frozen, Reason: uidIdle)` ——
  退到后台约 20 秒三星就冻进程，看门 job 在清理里丢失（`W/JobScheduler: Job
  didn't exist in JobStore: #u0a366/20260819 MediaWatchJob`）。**这条同时纠正
  了我上一轮的错误判断**：我把这句警告判成「大概无害」，理由是「那几轮备份都
  成功了」——成功是别的通道补上的，警告本身就是「看门 job 死了」的直接证据。
  出路不在代码（冻结期间我们不运行，检测不到），在把电池白名单提成 onboarding
  必经一步。
- `NET-01`（L2）：半小时内三次传输层失败（两次 `backup.begin` 卡满 15000ms
  超时，一次传到 54/198 时 `IrohError ConnectionLost(TimedOut)`）。回归步骤
  #18「计算后等待较长时间才发起重传」大概率就是这个 —— 不是计算慢。
- `I18N-01`（L3）：`MediaScanner.kt:84` 硬编码 `"未命名"`，选相册页唯一一处
  没走 `stringResource`。

**进 backlog（验收人明确「暂时接受」）**：`UI-05`（扫描阶段一个裸的 0 +
过早出现「暂停」按钮）、`UI-06`（进度条在走而大字不动）、`DESK-11`（断开配对
后审计显示 id —— ⚠️ 若那串是**完整** hex，则不只是可读性问题，还是 DESK-10
漏掉的一处脱敏，先确认这一点）。

---

## 〇bis、2026-08-26 MOB-40 已实施（L0 · ✅ test.8 真机通过）

**验收人真机打回**：「我就选择了一个 11 张的相册，你给我同步几百个？又出现
bug 了，我都不用往下测试了！后续的移动和拍照意义不大了啊！」

**logcat 实锤**（`/tmp/ppass-logs/logcat.txt`）：

```
15:53:05  全新安装 0.4.0-test.6（新 uid → SharedPreferences 全空 → 范围 = null）
...
**根因是一条语义**：`BackupScopeStore.selectedBucketIds()` 返回 `null` 表示
「从未选过范围」，而全链路把它解释成「全量备份」（T6 给升级用户留的兼容）。

**修在管线咽喉**（`BackupWorker.runBackup` 读范围那一处）而不是逐个堵五条
触发通道：MOB-33/34/35/38 四个 bug 全是「漏接一处」，那个形状不能再复制。
触发侧的收拢归 MOB-39。

**测试**：45 类 / 340 tests / 0 failures（XML 16:08:45），反证真跑变红。
**顺带开了 MOB-41**（L2）：重传提示发在范围过滤之前，清理那 243 张范围外的
照片时每轮都会弹一条假的「正在重传」。
...
水位之下、增量扫描永远看不见——真机现象是「触发了但什么也没传」。

落地选卡面 **A 路**（按 bucket 定向查）：每轮多一次「已选 bucket 里、水位之下」的
**元数据**查询（新 `MediaScanner.scanScopeBelow`，一个 collection 一次；范围为 null
或水位为 0 时一次都不发），返回集靠 `ConfirmedState.files` ∪ `HashCache` 两张现成的
表筛掉已确认的那些——**已备份过的不开流不哈希、不重复上传**。稳态零候选零哈希，
开销 ∝ 变化量而非库大小。**不选 B 路**（bucket 归属快照表）的理由：B 也得每轮发同
一条范围查询，成本一样却多一份要落盘要清理的状态；A 的「快照」已经在盘上了。

- 绿：Android **39 类 / 302 tests / 0 failures**（`--rerun-tasks`，XML 时间戳
  2026-08-26 11:11:51 本次生成；基线 38 类 / 290）+ `assembleDebug`。只动
  `apps/android/**` → 受影响 CI 域只有 **ci-android**。
  rebase 到 `MOB-33` / `MOB-37` 之上后复跑（两张卡都动了 `BackupWorker`）：
  **42 类 / 322 tests / 0 failures**（XML 11:19:37）+ `assembleDebug` 绿；
  多出的 3 类 / 20 条是那两张卡的，不是本卡的。
- 反证 4 条真跑（不合并进管线 → 2 红；不补齐 → 4 红；去掉「已确认的跳过」→ 3 红；
  查询去掉 `BUCKET_ID IN` → 1 红），摘录在卡里，之后全部还原。
- **顺带治好**「新勾选相册里的存量照片自动备份永远够不着」（同一根因另一面）。
  ⚠️ 代价：勾选大相册后第一轮自动备份会一次 offer 整个存量（与手动全量重扫同量级）
  ——看到「第一轮传了几百张」不要当回归。
- ⚠️ 明写的取舍：范围内、水位之下、hash 未知、**文件打不开**的坏记录现在每轮多一次
  失败的 `open`（上界 = 坏记录条数，逐条隔离保证挡不住整批）。刻意不加负缓存。
- **等用户**：真机验收一条，见 `docs/CHECKLIST.md` §一新增的那行。
...
一步没开始也能校准完）。**`manifest`/`missing` 与 proto 零改动**，反墓碑判据
落成 daemon 集成测试。欠**真机验收一条**：访达删若干张 → 桌面端出警告 +
手机端出通知 + 照片确实被传回来（见卡「实施记录／还差什么」）。

**MOB-29 定调翻转（2026-08-25，落地依据）**：墓碑方案整条撤销。用户新定调三条——
①重传是正确行为，存储端不该丢数据，不拦；②删除的正确姿势是「先删手机原图、
再删库」，两次删除表达意图，源被消灭则重传自然不发生，**不需要任何「记住我
不要它」的状态**；③要做的只是让用户知道，且**不做精确归因**（换库概率低，
一句「资源在客户端丢失，正在重传。如果是主动删除，请先删除移动端的数据」
在人删/换库两种成因下都成立）。明确不做：proto 加字段（「侵入太多，单一性
不够强」）、内置垃圾桶、恢复入口、废纸篓恢复的多余副本处理（系统行为）。
警告只针对 **delete**（add/move 对我们影响为零，WATCH-04 定调不变）。
卡已按此重写，**「已备份」数字口径 A/B 那道裁决随墓碑一起消失**。
...
### ✅ MOB-32 真机已验（2026-08-21 14:08，用户机器实测）

```
13:59:41  backup.started
13:59:43  backup.finished  ingested=12 duplicates=0    ← 12 张的相册 → 12，数字对了
          staging 收尾 0 字节 / 0 个文件                 ← MOB-32 的现场判据
14:07:29  asset.removed_external ×5，索引 12 → 7        ← WATCH-02 对了
14:08:05  backup.started
14:08:07  backup.finished  ingested=11 duplicates=7
现在      索引 18 行 / originals 18 文件 / staging 0     ← 三者一致
```

MOB-30（逐张浮现）/ MOB-31（进度分母 = 12）/ MOB-32（staging 归零）/
WATCH-02（Finder 删除对账）四条**都在真机上成立**。

⚠️ 同一份数据里抓到两件事：

1. **`MOB-29` 真机证实**：14:07 手动删掉的 5 张，14:08 那轮**原样全回来了**
   （查文件名，索引里每个 1 行）。墓碑必须做，卡还卡在数字口径的决定上。
2. **新卡 `WATCH-07`**：每批备份之后紧跟 N 条 `ingest.duplicate`
   （12 张 → 12 条，7 张 → 7 条，累计 37 条）。审计 actor 查实是
   **本机存储端 node id**（`1F1D1E38…`，手机 `backup-state/` 目录名就是它），
   不是手机 —— 文件监听在重新 ingest 备份管线刚 place 进去的文件。活动流是
   用户看的界面，传 200 张就是 200 条「重复」把有用条目全冲走。

### 🟡 MOB-32 代码细节

**校准把正在跑的备份会话清空，186 张照片传上来后被静默丢弃。**（L0，本轮最重）

三处修完 + 一处审出来的回归：

| | 事情 |
|---|---|
| ① | `begin` 改成 `entry().or_default().touch()`，不再顶掉活会话；生命周期归 `commit` 与新 janitor `sweep_sessions(1h)` |
| ①附 | commit 的拉取回退加 `provider` 门 —— 不加的话「幽灵 item」会让每次 commit 都报错、会话还收不走，备份一直红 |
| ② | 独立于 session 的 `delivered` 台账；commit 在「交付 N 张、入库 0 张」时返回 `NothingIngested`，水位不推 |
| ③ | `inbox::sweep_orphans` 回收 staging 孤儿（启动 + 每小时），判据 = 裸文件 ∧ 不在活会话保护集 ∧ 落地超宽限期 |

手机端顺手删掉 `existCheck` 里那次多余的 `backup.BEGIN`。**daemon 侧的修复
独立成立**：旧 APK 打新 daemon 也安全。

反证 9/9 全红，`just ci` 全绿，Rust 313/313，Android 253/253。

**真机验收剧本（欠用户）**：跑一次完整备份，**中途打开 App**（触发校准）→
照片数必须全部到位；收尾后 `du -sh "<库>/.ppf/staging"` 必须是 0。

⚠️ **我在这件事上犯过一个判断错误，记着别重犯**：上一轮我对用户说「不是在
执行全量同步，是显示问题」。证据是真的（`WorkProgress` 零行、检查那一刻没有
work 在跑），但我把「此刻没在跑」外推成了「那次没发生过」。真相是全量上传
确实发生了，只是 commit 把它扔了，所以入库审计里看不到。
**审计里「没有 X 发生」不等于「X 没被尝试过」——判断"传了没有"要去看中转区
和磁盘占用，被丢弃的工作恰恰不会留下入库记录。**

### 已修等真机（这轮全部）

| 卡 | 事情 | 证据 |
|---|---|---|
| `MOB-32` | 校准清空活会话 → 照片被静默丢弃（L0） | 反证 9/9，Rust 313/313 |
| `MOB-31` | 界面从五条通道的历史终态里随机挑一条（`lastOrNull` + 集合无时间序） | 反证 4/4，Android 253/253 |
| `MOB-30` | 入库跟着上传走，不再攒到 commit；顺带解掉断线整批重传 | 反证 4/4 |
| `WATCH-02/03/04` | 删除对账的斜杠 bug / 库内移动丢行 / 宽容入库 | 见 ROADMAP |

### 等拍板

`MOB-29`（墓碑 + 常驻提示，不封禁设备）：「已备份」的数字口径 —— 只数库里
真有的（诚实，会从 188 掉到 3，配一句「另有 N 张已按你的要求删除」），还是
数「已交代过的」= 库里有的 + 墓碑里的（稳定但仍误导）。**卡里倾向前者。**

### backlog（用户明确说先不做）

- `WATCH-05` inode 身份缓存 —— 用户已拍板「是需要的」（目标量级几万张），
  实施前重开讨论。stat 4.6 µs/张 vs hash 2.8 ms/张 = **620 倍**
...
| 卡 | 事情 | 状态 |
|---|---|---|
| `WATCH-02` 🟡 | Finder 删照片索引无反应 —— 根因是一个斜杠 | 真库实测 3→0 一秒 |
| `WATCH-03` 🟡 | Finder 挪动照片导致照片凭空消失 | hash 是身份，路径只是住址 |
| `WATCH-04` 🟡 | **宽容入库：`originals/` 是用户的目录** | 反证 5/5，Rust 305/305 |

**WATCH-04 一句话**：`originals/` 底下任何位置的媒体文件都照原样入索引；
`mv` 到 canonical 日期布局只对「我们自己从手机收到的文件」执行。手放的文件
归属记**本机**（不留空）。前置条件「一个路径只能被一条索引行占用」同批落地。

### 已实测的三条平台事实（`docs/product/2026-08-21-macos-fs-events.md`）

1. 事件的**类型**基本没有信息量，只有路径可信 → 只能「事件=去看这个路径，
   磁盘=真相」
2. 「拖进来」和「拖出去」长得一模一样 → 判断只能靠 `stat`
3. 根目录删掉重建，监听**不会死** → 我在 WATCH-02 里列为「最可能」的假设
   是错的，已加断言钉住

### backlog（用户明确说先不做）

- `WATCH-05` inode 身份缓存 —— **用户已拍板「是需要的」**（目标量级几万张，
  一台手机就有几千张）。成本已量：stat 4.6 µs/张 vs hash 2.8 ms/张 =
  **620 倍**；5 000 张 ≈ 14 秒、50 000 张 ≈ 2.4 分钟，且是热缓存乐观值。
  排期不在这一轮，**实施前重开讨论**
- **范围限定（2026-08-21 拍板）**：文件备份/文件同步整个不在范围内，
  **只做图片**。`WATCH-06` 里「加种类维度」那段是记账不是待办
- `WATCH-06` 相册 = 目录 —— 照 Android 学（`BUCKET_DISPLAY_NAME` 就是文件夹名）。
  ⚠️ 卡里写明**不要用软链物化视图**，理由齐全

### 挂着的欠账

- **`rebuild()` daemon 里没人调用** —— 它只是个 T-012 契约，靠测试覆盖。
  WATCH-04 给它加了 `local_node_id` 参数，要真用上得先接进 daemon
- **孤儿缩略图**：路径让位后老 hash 的 thumb 没人删（与已接受的孤儿 blob 同类）
- **`asset.replaced_in_place` / `asset.removed_external` 在活动记录里是机器原文**
  —— 用户抱怨过这类文案
- `we_moved_it` 回滚分支无测试覆盖（防御性，构造 insert 失败代价过高）
...
MOB-28（强制停止流程）、MOB-19（暂停/再试一次）、MOB-09、MOB-13、ICON-02、
BLOB-01。上线阻塞清单三件必做已全部完成
（`docs/product/2026-08-20-launch-blockers.md`）。

## 一、2026-08-20（续二十五）：MOB-28 区分「重启」与「被清」（当前状态）

**代码完成、真机端到端验过、已装机（0.3.4(9)）。**

用户上一轮的三连追问，其中一条是对我越界的批评（"强制关闭后重启
application 自动启动，这个不是先不做吗？你这搞什么？"）——**批评成立**，
我在 MOB-27 里顺手删了那道闸，应该先问。本卡把它做成用户当初真正要的形态。

### 做出来的语义

| 场景 | 行为 |
|---|---|
| 重启手机 | 自动重挂（开机 receiver 让它立刻回来，不等 5h） |
| force-stop / OEM 清理 | **不自动恢复**，设置页琥珀提示卡，点「恢复备份」才重挂 |
| 正常运行 | 看门 job 自己管自己 |

判据是**开机时刻**（`currentTimeMillis - elapsedRealtime`，同一次开机内稳定、
重启后变，零权限）。MOB-18 当初做不到是因为监听是 WorkManager 的 work、
...
相册变化（探针）        → 进程未被拉起（stopped 态，符合预期）
打开 App              → 看门 job 仍为 0 ← 没有被自动装回去 ✓
                        interruptedUnacknowledged: true ✓
设置页                 → "后台备份被停掉了…照片都还在，一张没丢。" + "恢复备份" ✓
点「恢复备份」          → job 回来 + 标志清除 + 卡消失 + 立刻补跑一次 ✓
```

234/234 绿，18 条反证全红（MOB-27 的 9 条一起复跑）。

### ⚠️ 一条已知边界（如实记录，本卡不做）

**force-stop → 重启 → 打开 App** 会被判成"重启"而自动恢复，不提示。
原因：force-stop 之后我们一行代码都跑不了，"被清过"这件事没人记下；
等用户终于打开 App，开机时刻已经变了。要覆盖它需要一个"force-stop 本身"
的信号，应用层拿不到。主路径（force-stop → 打开 App）已验过，那正是你
实测抱怨的那条。

### 还欠的真机验收（MOB-27 的三项，需要真实拍照）

1. **连拍**：100ms 级连拍 5 秒 → logcat 只出现 **1 次** `auto backup: offered=…`
2. **备份期间拍照**：大批量传输中拍 3 张 → 第一轮结束后立刻接着传
...
**P0（真实用户会直接撞到）**
1. `BLOB-01` **占盘翻倍**（新开卡）：用户机器实测 originals 549M / blobs 554M
   = 2.05 倍。ingest 之后 blob 永不回收，transport 里没有任何删除路径。
   备份产品吃双倍盘 = 会被卸载。
2. `MOB-19` 手动备份仍会被一条坏 MediaStore 记录炸掉整批（改法照搬 MOB-09）
3. `DESK-06` 桌面照片墙不跟随变化（用户 8/13 已反馈过；根因已定位，单文件改动）

**P1（卡住"打 tag 发版"这个动作）**
4. `E2E-02` e2e 门禁常红，每次打 tag 都撞（一行断言）
5. `SITE-02` 三篇博文等你审稿 + DNS CNAME 还指着旧占位

**建议顺序**：① → ③ → ② → ④，四条都有现成卡或明确根因，不需再调研。

**P3 是你自己定的两道闸**（跟代码无关）：M2 一周家庭狗粮 100% 完成率、
M4 5–10 户私测两周。

### 队列

- `MOB-19` 手动备份链路有与 MOB-09 同形的坏记录炸批问题
- `E2E-02` DaemonHelloTest 断言 2026-08-08 就废弃的 `a=` 契约，**下次打 tag 还会红**
- 未开卡：四条备份通道是四个独立 unique name，理论上可并行跑两个 BackupWorker
  扫同一水位、重复推字节（MOB-27 之前就存在）
- 未开卡：`BucketScreen.kt:81` lint 红（`ProduceStateDoesNotAssignValue`），
...
1. **连拍中途断**（"前面的出去了，后面的就没有同步"）——监听 work 与
   干活 work 是同一个，备份跑多久监听就断多久。当轮只做了治标（重挂
   延迟 15s→1s、rearm 后按需补捞）；**用户当场指出这是治标**："你强行
   用时间来做判断的话，是不太合适的。"根治见 MOB-27。
2. **查看页只剩图片**——`fillMaxSize()` 把底部「保存/分享」顶出屏幕，
   照片页与视频页同病，均改 `weight(1f)`。与 ICON-02 无关。
3. **「已回家」显示 0**——数据一直对（范围内 142 条），是 UI 不刷新：
   后台 BackupWorker 跑完不通知 UI。改为按 tag 订阅 worker 状态流。
4. **新选相册不同步**——水位归零逻辑验证通过。
5. **保存到相册重复写文件**（同 hash 存两份）——保存前查 DISPLAY_NAME
   去重，文案区分「已保存」/「已经在相册里了」。
6. **force-stop 检测撤除**——用户拍板 pending，理由见 backlog 卡。

### 队列

**待讨论/实施**：
- `MOB-27` 监听与干活解耦（**下个 session 的主话题**）
- `MOB-19` 手动备份链路有与 MOB-09 同形的坏记录炸批问题
- `E2E-02` DaemonHelloTest 断言 2026-08-08 就废弃的 `a=` 契约，**会一直
  挡着 e2e 门禁，下次打 tag 还会红**

**backlog**（用户判定暂不做）：
- `MOB-18` force-stop 检测（WorkManager 抢先自愈，语义做不到）
- `MOB-25` 查看页尺寸显示 `0×0`
- `MOB-26` 照片查看器缺翻页/缩放，应改用 HorizontalPager 等成熟方案

### 真机验收欠账

MOB-09/13 的真机验收仍未做（当轮设备时断时续）。MOB-13 有前置：升级后
先手动按一次备份补齐文件级记录，再验"复制一张已备份照片 → 待备份归零"。

## 〇、2026-08-19（续二十二）：四卡批次代码完成，真机验收全部欠着
...
⚠️ **验收 Android 测试请用 `--rerun-tasks`**：本轮发现 main 上的
`DiagTextTest` 漂移守卫已经红了一段时间，被 gradle 增量构建掩盖
（只有改动触发资源重编才会跑到它）。已修。

**待用户拍板的队列**（按"能不能用 > 快不快"排序）：

1. **MOB-10**（默认策略在真实设备上失效）——不关「仅充电」，开着电池
   保护的手机一张都传不了。这是唯一还挡着"开箱可用"的东西。
2. **MOB-09**（一条坏 MediaStore 记录卡死整批备份）——定时炸弹。
3. 失败后的探测节奏（现在失败 2 次就等 6h 兜底；用户实测撞到过电脑
   没响应导致 12 分钟延迟。建议 30min→2h→6h 递增，仍是 mobile 主动
   touch，不需要新通道）。
4. 暂停后台备份的入口（机制在，UI 被上一轮隐藏「更多」卡时一起藏了）。
5. ContentObserver 快路径——1.6 秒之后边际收益已大幅下降，待观察。

## 〇、2026-08-18（续十九）：MOB-08 后台自动同步——三根因定位 + 修复
...
恒为 `NOT_CHARGING`，插墙充也一样——产品默认 `chargeOnly = true` 在
...
用户真机装上续十六那批后逐条挑出具体问题，逐条修复：

1. **M2 摄像头预览重做**——之前摄像头画面铺满全屏，只在上面叠一个
   240dp 装饰框，黑底区域全被摄像头画面占了；改成摄像头本身也裁到
   240dp 圆角 20dp 的窗口里，窗口外是纯黑（`PPScreen(SurfaceDark)`），
   跟设计稿"小窗口+大片纯黑"的比例一致。
2. **M3 输入框真 bug**——之前用 `Text` 纯展示当输入框，打不了字、长按
   也不出系统粘贴菜单；改成真实 `BasicTextField`（自带长按选择/粘贴
   工具条），"粘贴"按钮保留做发现性入口。
3. **M4 去掉 Joined 中间页**——用户提议"扫完等 desktop 允许自己跳选
   相册页面"；桌面点"允许"后不再停一个要点按钮的确认页，直接
   `enterBucketPicker`。删掉 `Screen.Joined`/`JoinedScreen`/3 个孤儿
   字符串。
4. **M6 两处**：①删掉标题上方那颗绿色装饰圆点（用户反馈"没用处"）；
   ②只在 `firstTime=true`（onboarding 首次选相册）才显示，设置页
   重选相册直接回 Home——`Screen.Buckets`/`enterBucketPicker` 新增
   `firstTime: Boolean` 显式传递，两个调用点（配对成功 vs 设置页
   重选）区分清楚，不靠额外的持久化状态。
5. **M10 cell 全部重做**——真机反馈"行高行间距都不对，功能分布也不对"：
   行高统一到设计稿原文 52dp（`CellRowHeight`，原来是 56dp）；"备份"
   卡改成设计稿的 4 行——相册导航行 + 仅充电/仅Wi-Fi/失败通知 3 个
   直接开关行（不再折进"什么时候备份"子页——那是上一轮自己想当然
   加的一层，删掉 `BackupTimingDetail` + 死掉的 `policySentenceKey`/
   `timingSummaryKey` 纯函数和它们的 5 个单测）；"备份失败时通知我"
   落地成真实偏好 `NotifyOnFailurePrefs`（默认开），`BackupWorker`
   发失败通知前会读这个开关，不是摆设；"存储电脑"+"版本"独立成
   "其他"卡（对齐设计稿分组）；"暂停"/"立即备份"挪到第三张"更多"卡
   （设计稿没画但是现有功能，不能删）。
6. **M11/M12 信息卡重做**——之前是"存储电脑｜名字"重复行（跟设计稿
   的状态点+富文本完全不像）；改成状态点（绿=已连接/红=配对失效）+
   设备名 + "已连接·最近同步 X\n配对日期·存了 N 张照片"两行文本。
   "存了 N 张"复用已有的 `triplet.m`（该 remote 已确认条数），"最近
   同步"复用 `triplet.lastSuccessAt`；"配对日期"是新字段——给
   `Pairing` 加 `pairedAt: Long`（配对成功时打时间戳，老 pairing 升级
   上来是 0，诚实显示为"不知道"——即跳过第二行，不倒推瞎编）。
   断开按钮/二次确认卡按钮高度统一成设计稿的 48dp。

**测试**：android 全量 **177/177** 绿（182 - 5，删掉的是上面提到的
`policySentenceKey`/`timingSummaryKey` 自己的死测试，不是漏测）+
`assembleDebug` 绿 + `StringsSymmetryTest` 2/2 绿。真机验收待用户。

## 〇、2026-08-18（续十六）：M2/M3/M5 结构性重做——上一轮审查不够穷尽（完成）

**背景/教训**：用户真机装上续十五那批（4 处小修）后反馈"完全对不齐"，
质问是否每页都核对过。**如实承认**：续十五那轮不是逐页系统走查，是
挑了几个我主观觉得可疑的点抽查（M9 按钮/tab 图标/M13 红点/M6），
没有真正把 M1-M13 十三张状态稿逐张跟代码比对。这次补做了真正的逐张
核对，发现前 5 步 onboarding 路径（用户装完新机走的第一遍流程）里有
3 张结构性对不上，比上一轮任何一条差距都大：

1. **M2 扫码取景页结构性重做**——原来是大号衬线标题"扫码"+简单方框
   描边+底部两个并排按钮（手动输入/取消）；设计稿是暗底顶部窄标题栏
   （✕ 关闭代替"取消"按钮 + 居中标题"扫码连接"）+ 四角括号取景框
   （不是简单方框）+ 底部一行纯文字链接"无法扫码？手动输入配对串"
   （不是按钮）。`ScanScreen.kt` 头部/取景框/底部三处全部重画，取景框
   用 `Canvas` 手绘四角 L 形括号 + 中线。
2. **M3 手动输入配对串——从内联展开改成独立子页**——原来是扫码页里
   一个多行文本框就地展开；设计稿是完全独立的一页：返回箭头 + 标题
   "输入配对串" + 说明段（"在电脑的「添加设备」里点...粘贴到下面"）+
   单行占位框（显示 `PP-XXXX-XXXX-XXXX-XXXX`）+ 胶囊"粘贴"按钮 + 有效期
   提示 + 全宽"连接"主按钮。新增 `ManualPairScreen` composable。
3. **M5 选相册页——从竖排复选框列表改成 2 列封面卡片网格**——原来是
   `LazyColumn` 逐行"勾选框+封面缩略图+名字+张数"；设计稿是 2 列封面
   卡片网格，右上角圆形勾选角标（选中=墨底白勾，未选=半透明纸底描边
   圈），选中卡片本身描边变墨色，底部总结句"将备份 N 张 · 默认插电 +
   Wi-Fi 时自动进行，可在「设置」里更改"+单个"开始备份"主按钮（不再是
   "取消/备份N个相册"两按钮）。**这个改动是全局的**——`BucketScreen`
   是 onboarding 首次选相册与设置页"备份哪些相册"重选共用的唯一
   composable，两个入口都会看到新网格；原来的"取消"按钮改成头部返回
   箭头（跟 M3/M11 同款交互，不丢失设置页重选时的退路）。

**顺手检查、确认已对齐、不用动**：M1 欢迎页（逐字比对完全一致）、
M4 等待确认页文案（图标+电脑名部分卡协议缺口，续十五已记录）、
M6/M10/M11/M12/M13（前几轮已核对，这次复查未发现新问题）。

**仍未查完、下一步**：M7/M8 照片页网格缺月份分组标题（"8 月"）+ 网格
项右上角 ↑/↓ 状态角标（待备份/仅在电脑）+ 底部图例说明（"↑ 待备份 ·
↓ 仅在电脑...·成功不打标"）——这三个都是纯 mobile 端能做的，本轮
时间关系没做，下一批优先做这个；家人姓名 chip/大图归因真名字仍卡
SYNC-05（续十五已记录，没有变化）。

**测试**：android 全量 **182/182** 绿（跟上一轮持平，纯 UI 重画，没有
新增可测纯函数）+ `assembleDebug` 绿。真机验收待用户。

## 〇、2026-08-18（续十五）：对照新到的《P-Pass 全页面状态稿》补 4 处 mobile UI 差距（完成）

用 `claude_design` MCP 拉到项目里新增的 `P-Pass 全页面状态稿.dc.html`
（M1-M13 = 手机端每页每态的静态稿，D1+ 是桌面端）逐页跟当前代码核对，
发现 7 处差距，其中 4 处是 mobile 端能独立完成的（已实现），另外 3 处
明确记录为依赖后端/协议、本轮不做：

**已实现（4 处）**：
1. **M9 大图页底部按钮改回"保存+分享"**——之前是"保存到相册"+"用其他
   应用打开"，分享挪在右上角图标；改成设计稿的两按钮布局（保存到
   手机/分享），头部图标删除，`ViewerOp.OpenWith` 连带删除（`open_with_app`
   字符串在 `VideoScreen.kt` 仍有用，没删，只是 `PhotosScreen.kt` 不再
   用它）。
2. **Home 页 tab 栏加图标**——`TwoTabs.kt` 之前纯文字，设计稿 M7-M13
   张张都画了图标（相机/齿轮）。新增 `TabIcons.kt` 用 Compose `Canvas`
   手绘（不引入图标库，跟正在另外排期的 ICON-02 迁移卡分开，避免两条
   线打架）；齿轮用环+8 颗规则短线简化设计稿原本的贝塞尔外轮廓。
3. **设置 tab 哨兵态红点**——`TwoTabs` 新增 `settingsAlert` 参数，
   `pairingLost=true` 时齿轮图标右上角一个纯图标角标（红点）。**这跟
   续十四否掉的"备份/备份!"文字变红方案是两回事**——那个是文字变色，
   已经被官方最新设计稿否定；这个是图标角标，是这次新看到的、当前
   最新稿子里的要求。
4. **补 M6 选相册完成页**——之前 `Buckets.onDone` 触发首次备份后直接
   跳 `Screen.Home`，设计稿在这中间有一张绿底"{N} 张照片，正在回家"
   安心收尾页。新增 `Screen.Started` 状态 + `BackupStartedScreen`
   composable；不区分是不是 onboarding 第一次进来（选相册页本来就是
   onboarding 和设置页共用同一入口，不额外加 fromOnboarding 分叉，
   跟续十三"整删 onboarding 步骤分叉"的方向一致）。

**明确记录、本轮不做（依赖后端/协议，超出 mobile 范围）**：
5. **M9 归因文案"来自 XX的手机"**——`proto.AssetMeta` 没有 `src_device`
   字段，现在只能"我传的/家人的"二元区分，等 SYNC-05 落地才能换真名字
   （`PhotosScreen.kt` 里早有注释挂着这笔账，这次只是重新确认还没变）。
6. **M8 家人过滤 chip 按具体姓名分（妈妈/外婆各一个）**——同样卡在
   第 5 点，没有 per-device 归因数据就拿不到"妈妈"这个名字。
7. **M4 等待确认页显示找到的电脑名+图标**——查了 `PairFlow.kt`：新配对
   走 `pair.request` 时，`deviceName` 只在对方点"允许"之后的
   `PairAccepted` 里才返回，等待阶段协议上就是拿不到；QR 本身
   （`ppf://pair?node=...&t=...&r=...`）也没有编码设备名字段。要做
   这条，得往 QR 里加一个非鉴权的展示用 `n=` 字段（触及 daemon
   `pairing.rs` + 桌面端 + Android 三处），风险和改动面明显大于纯
   UI 改动，先不在本轮做，留给单独一张卡评估。

**测试**：android 全量 **182/182** 绿（跟上一轮持平，本轮是布局/接线
改动，没有新增可测纯函数）+ `assembleDebug` 绿。真机验收待用户。

## 〇、2026-08-17（续十四）：用官方 claude_design MCP 核对设计稿——断开连接改回点按钮确认 + tab 文案改回"设置"（完成）

用户要求用官方 `claude_design` MCP（`https://api.anthropic.com/v1/design/mcp`，
`get_file` 方法）直接读 claude.ai/design 项目 `P-Pass 设计稿交付`
（projectId `dcc1862c-337d-4590-8fc4-4c8381027a76`）的
`P-Pass 布局与交互.dc.html` 原始文本源码——比之前"另开一个 Chrome 调试
端口渲染 8MB atomics 导出文件"的土办法精确得多（纯文本、可 diff，不需要
浏览器渲染）。核对后发现续十二那轮（照用户 3 张截图 + 一份更早的设计稿
快照实现）有两处真的做反了：

1. **"断开连接"不是滑动确认**——`P-Pass 布局与交互.dc.html` 原文的
   "三层防误触"实际是：入口藏在「存储电脑」详情页最底部（第一层）→
   一个红色描边文字按钮"断开与这台电脑的连接…"（第二层）→ 点了原地
   展开一张"确定断开吗？"确认卡 + "取消"/"确认断开"两个按钮（第三层）——
   **完全没有滑动手势**。上一轮的 `SwipeToConfirm` 自绘拖拽控件是照着
   另一份更早的设计稿快照（"离线版3"）做的，已删除，改成 `armed`
   布尔状态本地切换按钮/确认卡。**顺手清理一个真的历史遗留**：
   `MainActivity.kt` 里还有一套更老的 `showDisconnectDialog` +
   `AlertDialog` 二次确认（这次改动前用户实际会经历"详情页滑动条→
   滑完再弹一个系统对话框"两层确认叠加，一直没人发现是因为没人真的
   走到断开这一步去点)——既然确认已经在 `StorageComputerDetail` 内部
   完成，这层 `AlertDialog` 整个删掉，`onDisconnect` 直接就是真正执行
   断开的逻辑（unpair + 清本地 pairing + 回 Welcome）。
2. **tab 文案改回"设置"**——`dc.html` 原文里 tab 图标下面写死的就是
   "设置"两个字（纯静态文本，不是模板变量），不会因为配对失效/电池
   未加白等状态变成"备份"/红色"备份 !"。上一轮照的是"离线版3"这份
   更早快照，那份快照的 tab 确实是"备份"，但这份最新的官方项目源码
   已经改回"设置"——`TwoTabs.kt` 的 `backupNeedsAttention` 参数 +
   `TabCell` 的 `alert` 分支整个删除，回到恒定的黑/浅灰两色。

**设计稿本身也在持续迭代**：同一份项目里 Android onboarding 的旁注
（标注⑤）在本次会话期间已经从"三步"（含系统权限页）→改成"两步"→
再改成"一步到位"（选相册→直接开始备份，备份条件给默认值+一行小字
说明），跟续十三那轮已经落地的实现完全一致，不用再改。**已把这份
`P-Pass 布局与交互.dc.html`（+ `support.js`/`android-frame.jsx`）的
纯文本快照落库到 `docs/design/2026-08-17-layout-v3/`**（离线归档，
详见该目录 README）——后续 UI 卡验收改以这个目录为准，且因为是纯
文本源码，往后可以直接 `diff` 看设计稿具体改了哪一行，不需要每次都
重新渲染截图核对。

**测试**：android 全量 **182/182** 绿（跟上一轮持平，本轮是 Compose
UI 层改动，未新增/删减可测纯函数）+ `assembleDebug` 绿。已重新打包、
卸载重装+`force-stop`到真机（`adb install -r` 不会杀掉正在跑的旧进程，
这是本 session 另一个真实踩过的坑，见上一条 PROGRESS 记录）。
