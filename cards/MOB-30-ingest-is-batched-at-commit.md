...
现在的流程（代码核过），手机侧 `BackupRunner.run`
（`BackupRunner.kt:72/81/103/112`）：
...
用户裁决（2026-08-21）：「上传是主动的，我觉得入库也应该是主动的，而不是
说批量。」→ **逐张入库**：每张传完校验通过、落进 staging 后立刻 ingest 进
索引；进度自然可见，中断只丢当前这张，已落地的不重传。

## 验收标准

- [x] 集成：传 N 张的过程中，索引行数必须**单调递增**而不是最后一跳
  （`index_grows_with_each_upload_not_only_at_commit`，钉住后果①）
- [x] 集成：传到一半杀掉连接 → 重跑时**已 staging 的那些不再重新上传**
  （`a_session_cut_short_does_not_re_upload_what_already_landed`，钉住后果②）
- [x] 反证两条都要有（2026-08-21 反证 4/4 有效，见验收证据）
- [x] `just ci` 全绿，Rust 307/307；BLOB-01 的两条已验证性质与
  `interrupted_commit_rerun_converges` 都仍然绿（2026-08-21）
- [ ] 真机验收（具体动作未记录）

## 范围

- 只准动：daemon 备份引擎（`backup.rs`）、上传平面装配（`main.rs`、集成
  测试）、`Session` 记账。（实施实动点见实施记录。）
- 不准动：别破坏 BLOB-01 的两条已验证性质——同一 hash 重复提交不重传也不
  报错；传输中触发回收不影响正在传的那批；以及
  `interrupted_commit_rerun_converges` 这条既有集成测试的语义。
