...
| **MOB-68（L2）后台备份状态真实化** | 2026-09-14 | 本批次 | 🟡 代码完成，待鸿蒙 4.2 回归 | 用户意图、Android 标准白名单、MediaWatch 健康拆成独立事实；开关仅在三项满足时亮，拒绝/撤销/监听被清均转为设置页可处理状态，不再走跨 tab 黄条。onboarding 仅在用户选“开启后台备份”时申请白名单；通知权限完全移出 onboarding，仍只由失败通知设置开关主动申请。新增 `BackgroundAuthorizationAdapter`，纯状态 RED→GREEN；Android JVM 353/0/0、debug APK、`just ci` 通过。 |
