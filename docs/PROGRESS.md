...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **UPD-01** | 2026-08-04 | — | 🔄 PR 待 review（密钥前置挂账） | 自更新通道（M3，非密钥部分）：①**release.yml**——release job 产出 tauri 风格 `manifest.json`（`tools/make-update-manifest.mjs`：compose 模式收集平台资产 URL/sha256；sign 模式用 UPDATE_SIGNING_KEY 对资产 Ed25519 签名，seed→PKCS8 DER→KeyObject（Node 裸 seed 报 Invalid digest），base64 64B 签名）→ 作为 release 资产上传；签名 step 门控 `HAS_UPDATE_KEY`（T-071b 模式，secret 只进该 step env），无凭据路径 signature 留空 + notes 标注。②**android 自研更新流**（脱店 App 无标准库）——`update/UpdateChecker.kt`：启动时 GET `releases/latest/download/manifest.json`（draft/无 release 404=静默无更新）→ semver 三段比较（`isNewer`，预发布后缀跨主版本算更新/同主版本不算）→ 弹窗提示 → 下载 APK → **FileProvider + 系统安装器**（PackageInstaller 强制同签名校验兜底——不嵌公钥，manifest 被篡改指向恶意包也装不上）；MainActivity LaunchedEffect 接入 + manifest provider + `buildConfig=true`。**测试**：UpdateCheckerTest 6/6（isNewer 四态 + parse 五态），全量 android 55/55 绿（JDK 21；本机 JDK 26 jlink transform 崩是 AGP 兼容问题）。**签名验证**：verify true + 篡改一字节 false。**挂账（等用户）**：Ed25519 密钥对生成（命令在 PR #30）→ 私钥进 secret UPDATE_SIGNING_KEY → 公钥替换 update.rs OFFICIAL_PUBLIC_KEY 全零占位 + 补"可解析成 VerifyingKey"测试 + desktop tauri-plugin-updater 接入（pubkey 入 tauri.conf）。 |
