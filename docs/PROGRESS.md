...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **REBUILD-04（L2）Worker Flow cutover 与最小状态呈现** | 2026-09-01 | 本 commit | 🟡 三星首包已实传；待 Pause → 杀 App → Continue → Cancel（Android JVM **254 tests / 0 failures / 4 skipped**；debug APK） | 三星测试相册的 26 项均经新 Flow 账本确认，Desktop durable index 25 项（内容去重）。真机暴露的独立 native provider endpoint / receipt 不推进 strict head / legacy watch crash 已修；失败项现显示 Retry，显式重试重开失败队头。`just ci` 与 deny 通过；两端已安装 0.5.0。 |
