...
| **FIX-SC2** | 2026-08-10 | 待推 main | 📌 第 1 步取证桩已落（卡留队列，等证据） | blobs_resume 300s 超时 flake 根治（L2，2026-08-10 用户拍板改方向：禁止放宽时间/缩数据/加 retries 绕过，取证桩+复现+根因三步走）。**第 1 步（本 commit）**：`crates/transport/tests/blobs_resume.rs` 落带时间戳的阶段打点（stamp()，eprintln——nextest 失败时才显示）——setup / provider ready / 每 attempt receiver bound / pull spawned / 传输进度逐 MB（waiting for kill threshold: N bytes on disk）/ kill landed / restart rebinding / durable bytes / resume pull started / completed+verified / verify ALL GREEN。本地实证 7.80s 全绿，桩零侵入（blobs_resume 4/4，9.83s）。**效果**：下次 CI 再 TIMEOUT，日志直接指出卡在哪个阶段（attempt N waiting = 传输前 stall；resume pull started = 重拨/续传 stall）——变 rerun 碰运气为每次失败积累证据。第 2/3 步（本地复现→根因）等证据到位再做。 |
