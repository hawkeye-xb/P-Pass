...
| **ARCH-07（L2）P1 远端对账事实与恢复裁决** | 2026-09-01 | 本 commit | ✅ 代码完成（R-01/R-02；Android JVM **377 tests / 0 failures / 4 skipped**；`just ci` 全绿） | `TransferItem` / `CompletionReceipt` 新增持久内容身份与远端/源存在性、恢复裁决；`RemoteReconciliation` 对当前 epoch 的已确认项只写事实：源仍在 → `NEEDS_DECISION`，两端缺失 → `UNRECOVERABLE`，远端仍在则无恢复裁决。不会自动重新入队、fetch、递增失败计数、改 UploadCursor 或删除源。新合同 3/3；反证实际变红：临时重新入队→R-01/R-02 两条失败，源缺失误裁可恢复→R-02 失败，均已还原。旧批次校准、WorkManager、UI、proto、Rust/desktop 零改动。 |
