...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **MOB-79（L2）照片网格视频播放角标** | 2026-09-14 | 本 commit | ✅ 真机验收通过 | 用户实机反馈：网格里视频和照片长得一模一样，点进去才知道是视频。`PhotosScreen.kt` 的 `ThumbCell` 新增 `asset.mediaType.startsWith("video")` 判断，为真时叠加自绘 `VideoBadge`（`Canvas` 画半透明黑底圆 + 白色实心三角，22dp，右下角），跟主流图库同一套视觉语言，不引入图标库依赖。同步补 `androidx.compose.foundation.layout.size` 缺失 import。Android JVM **69 类 / 346 tests / 0 failures / 0 errors / 4 skipped**（与改动前一致，纯 UI 叠加层不影响既有断言）、`just ci` 全绿。三星真机确认：2026.03/2024.08 两个视频缩略图右下角均出现播放三角，同屏照片缩略图无角标，视觉可清晰区分。 |
