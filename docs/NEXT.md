# NEXT — 当前状态与下一步（2026-08-21，真机三条：两条已修一条待拍板）
...
| 卡 | 问题 | 状态 |
|---|---|---|
| `MOB-31` | 选了 12 张的相册却显示 186 —— **不是在跑全量，是从历史终态里随机挑了一条** | 🟡 已修，等真机 |
| `MOB-30` | 收到的照片攒到 commit 才入库 | 🟡 已修（逐张入库），等真机 |
| `MOB-29` | 库里删掉的设备照片会被悄悄传回来 + 「已备份」是谎话 | ⛔ **等一处拍板** |

**MOB-29 是唯一被 gate 住的**：方向已定（墓碑 + 常驻提示，不封禁设备），
但「已备份」这个数字的口径要拍板 —— 只数库里真有的（诚实，但会从 188 掉到 3，
需配一句「另有 185 张已按你的要求删除」），还是数「已交代过的」= 库里有的 +
墓碑里的（数字稳定，但仍会误导）。卡里倾向前者。

**MOB-29 仍是三条里最严重的**：产品全部价值是「照片安全」这个承诺，显示虚高的
...
**MOB-31 已查明并修复**：`uiStateOf` 里 `infos.lastOrNull { isFinished }` 取的是
列表最后一个，而 `getWorkInfosByTagFlow` 不保证按时间排序（Room 顺序，实际按
UUID），五条通道的终态同时躺着最多五条 → 随机挑一条历史记录，186 是 8/20 那次
全量运行的旧终态。已改为按 worker 盖的 `KEY_FINISHED_AT` 挑最新。
⚠️ 凡是「最近一条」这种语义，都要先问「按什么排序」——集合 API 不给你时间顺序。
