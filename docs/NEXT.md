# NEXT — 当前状态与下一步（2026-08-21 清场重来，重点 MOB-32）

> 交接件，随每次收口更新。历史结论已并入 ROADMAP/PROGRESS。

## 〇、下个 session 从这里接（2026-08-21 清场重来）

### ⛔ 当前唯一重点：MOB-32（L0，未实施）

**校准把正在跑的备份 session 清空，186 张照片传上来后被静默丢弃。**

清场前当场取的证据：

```
.ppf/staging     547M   ← 186 个已校验文件（.upload = 0，全部通过 BLAKE3）
其中已进索引         1
纯孤儿             185   ← 传上来了，从没入库，永远不会被回收
originals          3.4M  ← 库里只有 5 个文件
.ppf/blobs/data      0B  ← BLOB-01 仍成立，泄漏点搬到了 staging

11:22:05  backup.finished   ingested=0 duplicates=0   ← 186 张全丢
```

链条（代码核实）：`backup.begin` 无条件 `insert(peer, Session::default())`，
而**漂移校准也走这条路**（`existCheck` = begin + manifest(empty)）；session 按
设备 NodeId 索引，校准与备份同一把钥匙。用户打开 App → `calibrateFromDaemon()`
→ `begin` → 正在上传的那个会话 items 被清空 → commit 循环零次 → 报
`ingested=0` 却**返回 ok** → 手机据此把 186 张全标记「已备份」。

三个独立的错都要修：①`begin` 不该清活跃会话；②commit 报 0 入库不该算成功；
③staging 没有回收路径（`is_partial_upload` 只认 `.upload` 后缀，已校验完成的
孤儿碰不到）。

**MOB-30（已合并）堵住了主漏**——逐张入库让文件在上传完成那一刻就进索引，
不再等 commit。但本卡仍必须做：`begin` 清活跃会话仍会弄坏 commit 报的数字与
水位推进，即时入库失败的兜底路径仍依赖 commit 的 items，存量 staging 孤儿仍
没人回收。

⚠️ **我在这件事上犯过一个判断错误，记着别重犯**：上一轮我对用户说「不是在
执行全量同步，是显示问题」。证据是真的（`WorkProgress` 零行、检查那一刻没有
work 在跑），但我把「此刻没在跑」外推成了「那次没发生过」。真相是全量上传
确实发生了，只是 commit 把它扔了，所以入库审计里看不到，我就误判了。
...
| 卡 | 事情 | 证据 |
|---|---|---|
| `MOB-31` | 界面从五条通道的历史终态里随机挑一条（`lastOrNull` + 集合无时间序） | 反证 4/4，android 252/252 |
| `MOB-30` | 入库跟着上传走，不再攒到 commit；顺带解掉断线整批重传 | 反证 4/4，Rust 307/307 |
