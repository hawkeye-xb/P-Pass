...
real-device dogfood. Samsung-on-cellular-hotspot testing reproduces NET-01:
a 288 MB Flow video hits `flow.fetch`'s shared 15-second deadline three times,
despite a 72 MB control succeeding. With no sustained relay window available,
that fix-and-regression lane is deferred rather than blocking other dogfood work. NET-04 pause/cancel/retry remains
...
missing.
MOB-68 is code-complete but awaits HarmonyOS 4.2 regression: notification
permission is no longer requested in onboarding; background backup now separates
user intent, standard Android battery authorization, and MediaWatch health, and
never presents a false enabled switch. Actual failure notifications remain MOB-67.
...
- [x] **REBUILD-04 Worker 切换、最小状态呈现与首包验收** — **2026-09-02 完成**：Worker 仅为 Flow wake adapter，UI 投影 user pause / waiting / transferring / cancelled round。三星隔离测试相册：新队头确认后，512MB 测试项 Pause → kill App → reopen 仍 Pause → Continue 原队头 → 再 Pause → Cancel Current Round 通过；32 `CONFIRMED` 未受 Cancel 影响。
- [x] **REBUILD-04 三星生产首包补验** — **2026-09-02 完成**：REBUILD-06 修复 completed grant 的 recovered lease rebind 后，新队头从 30→31 `CONFIRMED`；真实 Pause / restart / Continue / Cancel 已通过，未清手机或 Desktop 数据。
...
- [x] **REBUILD-07 同一设备重配对的 receipt 生命周期隔离** — **2026-09-10 代码完成，待同机重配对真机验收**：稳定 NodeId 继续关联历史照片；Flow grant 的唯一身份改为 `(node_id, pairing_epoch, queue_sequence)`，使新授权可安全从 sequence 1 重建，同时保留旧 epoch 的 immutable receipt 历史。SQLite 迁移原样搬迁既有记录；真实 iroh Flow RED→GREEN、storage 24/24、daemon flow 8/8、`just ci` 均绿。
  - **2026-09-10 真机通过并归档**：现场 v3→v4 升级保留 24 条历史 receipt；同一 Android 安装删除后重扫，以新 epoch 从 sequence 1 连续传 21 张，21/21 completed、无 `err.not_authorized`。同轮 BLOB-01/02：`originals` 292 KiB→45,520 KiB、`.ppf/blobs/data` 始终 0；`flow-blobs` 47,788 KiB 峰值经 70 秒回到 492 KiB，实际 `.data` payload 47,468 KiB→0，只余 136,832 B outboard 元数据。
- [x] **AUDIT-01 Flow 审计 v2：持久 outbox 与直接切换** — **2026-09-10 代码完成，待真机验收**：`audit_event` v2 成为唯一长期审计事实源，`audit_log` 整表 DROP（migration `0005_audit_event_v2.sql`），不迁移、不兼容读取历史行。Android `DiscoveryLedger` 的用户事实与 outbox event 在同一原子快照提交（`AuditOutboxEvent`/`AuditKinds`），新增 `AUDIT01LedgerOutboxTest` 锁死该契约；pause/continue/scope-change/epoch-invalidate/permanent-failure/reconciliation-resolved 六类事件全部接线，正常项按 `roundId` 聚合成一条 `flow.round.finished`，不逐项写长期审计。Rust `audit_repo.rs` 重写为 v2 repository（`event_id` 幂等键 + `INSERT OR IGNORE`），daemon 全部生产路径（pairing/reconcile/router/ipc）改写为 v2；旧 batch `backup.started`/`backup.finished` 会话审计随之下线（不在 v2 事件矩阵内）。Desktop `App.svelte` 活动页改读 v2 `kind`/`payload`，依赖旧会话审计的「本周统计」「备份耗时」两处派生 UI 因数据源消失一并移除（不编造数字）。验证：Android 328/328、Rust nextest 346/346（`just ci` 全绿）、desktop-rust 18/18、desktop-vitest 57/57；真机验证（新活动文案/Flow 真实终态）列后续批次。
  - **2026-09-10 review 后补齐投递管线**：首轮遗漏了手机 outbox → daemon 的投递/落库本身（`acknowledgeAuditEvents` 生产代码零调用点）。补齐 `proto::FlowAuditSubmit`/`FlowAuditAccepted` + `flow.audit.submit` 方法、daemon `handle_flow_audit_submit`（按手机 event_id 幂等落库）、Android `AuditOutboxDispatcher`（每个 Flow 触发点后异步 flush，仅对 daemon 确认的 id 摘除 outbox，未确认留存重放）。新增合同测试 `crates/daemon/tests/flow_audit_submit.rs` + `AuditOutboxDispatcherTest`。复测：Android 333/333、Rust nextest 354/354（`just ci` 全绿）、desktop-rust 18/18、desktop-vitest 57/57，均未回归。
- [x] **AUDIT-04 审计核心重建：operation/item evidence/tombstone/decision** — **2026-09-10 三星真机通过并归档**：直接替换 AUDIT-01 的 `audit_event` 泛用表（新 migration `0006_audit_canonical.sql`，不迁移历史行），新建四张 canonical 表并以 `crates/daemon/src/audit_route.rs` 作为手机 Flow outbox 的唯一分发点。`flow.round.finished` 的 `evidence_summary` 从实际持久化的 item evidence 重新聚合、不信手机自报的 final_counts（故意造假 payload 的测试验证真实计数覆盖谎报值）；`flow.reconciliation.resolved` 按 disposition 分流到 tombstone（UNRECOVERABLE）或 item evidence（NEEDS_DECISION）；全部路径幂等。顺带修正两处遗留：`device.connected` 此前违规写入 `audit_operation`（card decision #5 明令连接性从不进长期审计），已删除该写入路径，presence 改为纯 `last_seen` 刷新零副作用；`device.revoke` 补齐 `audit_decision`（此前只有兼容层 operation 行，未满足"撤销授权是决定"）。验证：Rust nextest **377/377 passed 1 skipped**；Android **333/333，0 failures/errors**（`--rerun-tasks` 64 个 fresh XML）；`just ci` 全绿。验收标准全部满足：current-main 重建 Android/daemon 跑隔离 Screenshots Flow，新增 receipt 后 item evidence=1 且关联 operation=1；精确外部删除后 asset=0、evidence=1、`external_delete` tombstone=1。
- [~] **AUDIT-02 活动记录用户投影** — **2026-09-11 代码完成，待真实 Desktop 验收**：`audit.list` 改直接读 canonical operation，返回 daemon 按 `audit_item_evidence` 重算的 `evidenceSummary`；Desktop 只按该可信汇总显示「已备份 N 张照片；M 张需处理；K 张已跳过」，不采用手机自报 final counts。连接、控制、epoch、逐项 ingest 噪音不再进入活动记录；本机显示【本地】，其它设备显示名称与短指纹。daemon IPC 造假计数反证、desktop projection 5 条、desktop Vitest 62/62、Vite build 与 `just ci` 均绿；欠 current-main Desktop 隔离 Flow 实窗复核。
...
2026-09-14 交付记录：MOB-79 完成并真机验收通过（用户实机反馈发现）。照片网格里视频和照片此前长得一模一样，点进去才知道是视频。`ThumbCell` 新增按 `mediaType` 判断的播放三角角标——`Canvas` 手绘半透明黑底圆+白色实心三角，右下角，跟系统相册/主流图库同一套视觉语言，不引入图标库依赖。Android JVM 69 类/346 tests/0 failures/0 errors/4 skipped（与改动前一致，纯 UI 叠加不影响既有断言）。三星真机确认两个视频缩略图均出现角标、照片无角标，视觉可清晰区分。移入 `cards/done/`。附带讨论：iPhone Live Photo（HEIC+MOV 双文件靠 content identifier 配对）与 Android Motion Photo（单 JPEG 内嵌 MP4）格式完全不同、不能直接互放；现有 asset 表是单文件模型，若未来支持保留跨平台 Live Photo 效果需新增资产关联数据模型，属架构级决策，暂不处理。

2026-09-14 取证记录：三星真机 32 张相册停摆并入 MOB-54，升 P0 进行中。审计未出现本次 delivery failed；4 张取消在 82 秒后已 restore，不能归为本次停摆原因。手机账本为 21 confirmed、OPEN/IDLE、无 lease、discoveryRequested=true。旧「瞬态失败漏 wake」补丁不能直接解释此次停摆，后续先重放最后 receipt 与已请求 discovery wake 的交错，再按 MediaStore/账本/canonical audit 三方对账未确认项，决定修复点。

2026-09-14 交付记录（纯文档/卡）：NET-08 全仓「同步等长任务」焊点普查完成（只读，零生产代码 diff）。五域逐域打勾：daemon 全部 RPC handler 与外呼、Android 全部 14 处 `client.call` + WorkManager + 预览/下载/更新检查、src-tauri 16 command、tools/、infra/workers。已知答案自检通过（fetch 链 N1/N2/N3 全命中并标 NET-06/07 覆盖）。新焊点开卡 4 张：NET-09 数据面字节停滞看门狗（L1）、NET-10 pair.request 拆提交≠等人（L2，复用 NET-06 status 形状）、NET-11 桌面壳 IPC 读超时保险丝（L1）、TEL-05 遥测 HTTP 超时+队列封顶（L1）；豁免登记 2 项（update worker 外呼、tools/ 冒烟脚本，理由写死在 NET-08 卡内）；legacy 批量管线定性=REBUILD-00 已冻结不再开卡。QUEUE 同步，`just queue-check` 通过。

2026-09-15 交付记录：NET-13 完成（用户报告 + clarify 拍板）。桌面「家人与设备」页明明已通过 Flow 备份过的设备显示"还没备份过"——根因非展示口径而是真 bug：`device.watermarks.last_backup_at` 只读遗留批量扫描表 `backup_watermark`（`backup.commit` 才写入），Flow 交付路径（`flow_delivery.rs::run_fetch_body`）只会真的 `ingest()` 落地资产，从不调用 `set_watermark`。`list_device_watermarks` 改为同时读 `asset.added_at`（按 `src_device` 分组取 max）与 `backup_watermark.updated_at` 取较大值。RED 先行（新测试改前真红），`cargo test -p storage --lib device_repo` 11 passed；`cargo test -p storage -p daemon` 全绿无回归。同批 UI 三项：设备 ID 列（8 位短指纹+悬停全量 hex）、打开该设备存储目录按钮（`originals/<node_id>/`，无照片时退回根目录）、在线状态从行内文案挪到固定宽度独立列（不做真表格）。i18n 两新 key 中英对称，`crates/diag/src/keys.rs` 注册表同步（100→102）。`apps/desktop` `pnpm run build` + `pnpm test`（10 files/62 tests）全过；`just ci` 全绿。待真机/真实多设备场景回归。

2026-09-15 交付记录（NET-13 本机真机验证追加）：`cargo build --release -p daemon` + `tools/bundle-macos.sh` + `tools/bundle-desktop-macos.sh` 重新打包 `/Applications/P-Pass.app`，`launchctl unload/load` 让 LaunchAgent 接管新二进制（md5 核实一致）。真实打开桌面 App 截图确认：5 台已配对设备（4 台三星 SM-S9210 + 1 台华为 Mate60pro12）ID 列显示 8 位短指纹、在线时间独立成列、打开文件夹图标正常渲染。直接查询本机真实库 `index.sqlite` 实锤：5 台设备 `backup_watermark` 表记录全部为 0（该表整体为空），其中 4 台 `asset_cnt>0`——改前这 4 台全部会被误判"还没备份过"，改后全部显示真实备份时间，与截图结果一致；唯一显示"还没备份过"的第 5 台经 SQL 核实 `asset_cnt=0`，确认真的从未备份，非误报。NET-13 移入 `cards/done/`。

2026-09-15 交付记录（UI-12/UI-13）：用户对话中反馈后台备份系统限制提醒"起不到通知作用"，排查全 Android UI 组件与 Material Design 规范的差距后开卡。UI-12 把后台备份降级状态接入全局 `NoticeHost`（此前绕开了它自己声明的"全局唯一提示宿主"契约），非阻断类通知新增独立"知道了"忽略动作，阻断/非阻断视觉分级。UI-13 逐项核对全 15 个 Android UI 文件后替换手写控件为 Material3 标准组件：tab 图标（Canvas 手绘→`Icons.Filled.Settings`）、设置角标（手写圆点→`Badge`）、`FilterChip`（自定义→Material3 同名组件）、`HeroSecondaryButton`（手写→`OutlinedButton`）；`NavigationBar`（尺寸冲突 80dp vs 64dp）与 `ListItem`（56dp vs 52dp）因量出的视觉冲突记录理由保留现状，未静默跳过。Android JVM 378/378 绿，三星 SM-S9210 真机截图确认视觉未走样、忽略/处理动作正常工作。两卡状态均为"代码已合并，等真机验收"，未移入 done/。

- [x] **NET-21 P1 flow-staging 装卸台孤儿回收** — **2026-09-16 完成**：用户追问"NET-06 之后是不是没有 GC"促成核查，发现 Flow 单通道的导出中转目录 `.ppf/flow-staging` 只在 ingest 成功分支删文件，ingest 真失败/materialize 中途 cancel/daemon 崩溃都会留下孤儿，且此前无任何定时任务巡检（旧上传收件箱早在 MOB-32 就有 `sweep_orphans`+`reclaim_staging`，新装卸台没接上）。`inbox.rs` 新增 `sweep_flow_staging_orphans`（判据同构：命名不合契约/落地不足宽限期/`protected` 内 content_hash 均不碰），`main.rs` 启动时+每小时各调一次，保护集复用 `Db::active_flow_content_hashes()`（与 flow-blobs 的 iroh GC 回调同一张持久表）。RED 先行 5 条纯函数单测+1 条反证（临时禁用保护判据后用例从 9 变 16，证明反证成立）。`cargo test -p daemon --lib inbox::` 10/10、`cargo test -p daemon` 全量无回归、`just ci` 全绿；版本 bump `0.5.4-test.1`→`0.5.4-test.2`。**未做真机长跑验证**（故意制造失败/cancel 场景确认目录清零），已记入卡内留白。
