...
> 🟡 状态：代码已合并，等真机验收。**根因已查明并修复**：进度条那个 186
> 是显示挑错了历史终态记录。
>
> ⚠️ 但本卡曾经断言"不是在执行全量同步"，**那句是错的**——同一次操作里确实
> 发生了一次 186 张的全量上传，只是 commit 把它扔了（见 MOB-32）。本卡只解释
> **数字为什么是 186**，不解释那批照片去哪了。
...
到 staging 并被丢弃（MOB-32），那是另一条独立的缺陷。
