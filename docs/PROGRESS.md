...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **DOG-01** | 2026-08-04 | — | 🔄 PR 待 review（真机验收挂账） | 备份恒真三元组 + per-device 水位（狗粮周阻塞卡，L2）：**daemon**——`storage.list_device_watermarks`（device LEFT JOIN backup_watermark + asset.src_device 计数子查询，revoked 排除）+ IPC `device.watermarks`（每设备 {node_id, name, last_backup_at, asset_count}，验收③数据源）；storage 测试 2 个（对照 + revoked 排除）。**android**——`TripletStore`（backup.triplet.json 崩溃安全持久化，tmp+rename，损坏返回 null 不崩）+ `tripletOf` 提纯（N=扫描范围、M=ingested+duplicates、K=N-M clamp ≥0）+ BackupUiStateHolder 成功后才写（断网/失败保留缓存值）+ HomeScreen 显示「手机 N 张 · 已备份 M · 待备份 K + 最后成功时间」（人性化：刚刚/x 分钟前/x 小时前/日期）+ MainActivity 传参 + strings en/zh 对称。**测试**：TripletStoreTest 6 个（含**反证**：exist-check 全 missing → K 必须=N 且 M=0）+ android 全量 **55/55** + workspace **195/195**。挂账（真机验收）：①杀 App 重开不归零 ②断网重开显示缓存值+不可达 ③ipc device.watermarks 与 sqlite 直查对照。 |
