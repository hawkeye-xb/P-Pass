...
| **MOB-61（L2）手机源删除时终态跳过** | 2026-09-08 | 本 commit | 🟡 代码完成，待真机 | 用户三星实证：照片已入 Flow 队列后从系统相册删除，`hashSource`/`openFileDescriptor` 异常越过终态，造成闪退或重传循环。新增 `SKIPPED_SOURCE_MISSING`：缺源原子写入 `MISSING`+`UNRECOVERABLE`、清 lease、推进 cursor 并唤醒下一项；不进入失败重试、取消轮或重传。首页只读告知「已跳过 N 张已从手机相册删除的照片，不会再重传」。Android JVM **310/0/0/4**、debug APK、`just ci` 均绿；待隔离照片真机验收。 |
