...
| 卡片 | 日期 | Commit | 状态 | 摘要 |
|------|------|--------|------|------|
| **DESK-09 向导把 daemon 真实启动错误吞成「没有在 10 秒内就绪」** | 2026-08-25 | `1e1359f` | 🟡 代码已合并（desktop vitest **31 passed**、src-tauri **17 passed**、`vite build` 208 modules；反证真跑；**真机验收欠验收人一条**） | 真机事故：旧版本包（0.3.0）打不开被新版迁移过的索引库，daemon 在自己 stderr 上重复 8 次 `Error: migration: migration 2 was previously applied but is missing in the resolved migrations`，界面却只说「后台服务没有在 10 秒内就绪」——排查方向全跑偏（dmg 布局→只读卷→Gatekeeper→TCC）。修法：新增 `daemon_err_offset` / `daemon_startup_error` 两个 Tauri 命令，**启动前记下 LaunchAgent stderr 日志长度，超时后只读新增字节**（launchd 的 `.err` 是 append 的，不记偏移就会把几天前的旧错误当成这次的原因——跟「导出包里只有一条四天前的事件」同一种病）；日志路径从 plist 的 `StandardErrorPath` 读，**不硬编码 `~/Library/Logs`**，plist 不存在就如实说「未注册」。新 `lib/daemonStartup.js` 负责文案：**超时不再当结论**（明说「超时只是我们等不下去了」，「没有在 10 秒内就绪」这句被测试禁掉），daemon 原文照登（不翻译不截断），迁移不兼容 → 「装回新版本」、库目录不可写 → 「换个有写权限的文件夹」、端口占用 → 原样透出（daemon 自己的错误已经是人话），不认识的只给原文。反证：删掉原文透出那一行 push → vitest 7 条里 5 条变红。 |
| **DESK-10 「导出日志」不含 daemon 日志，且 daemon 挂了它自己也不工作** | 2026-08-25 | `1e1359f` | 🟡 代码已合并（`just ci` nextest **319 passed**、src-tauri **17 passed**、desktop vitest 31 passed；反证真跑；**真机验收欠验收人一条**） | 同一场事故的另一面：验收人按「导出日志」把 zip 发来求助，**包里只有 1 条四天前的 diag 事件、489 字节**，而真实错误一直躺在 LaunchAgent 的 `.err` 里。更致命的是 `export_logs` 挂在 `logs.export` 这个 **daemon IPC** 上——daemon 起不来时这个按钮压根不工作，而那正是最需要日志的场景。修法：导出改为**桌面壳本地组装**（新命令 `export_logs_bundle`），收集与落盘拆出 `assemble_export`，daemon 那部分以 `Result` 传进去——**Err 不是失败路径**，只是少三份文件、多一份 `daemon-unreachable.txt`，zip 照出。包里现在有：daemon 的 stdout/stderr 日志（路径从 plist 读，取尾部 256 KiB）、`versions.txt`（App + daemon 版本；daemon 不可达时问内置服务 `--version`——真机事故里正是这一句拿到真相）、`config-summary.txt`、`log-sources.txt`、`README.txt`（先看哪个），daemon 可达时再附它那三份（daemon 侧 `export_logs` 顺带补上 `audit.json`）。脱敏不回退：家目录 → `<DATA>`，长 hex（NodeId/配对令牌）只留前 8 位，新加的文件全过同一道 scrub。文件名与落盘位置不变（`<库目录>/ppf-logs.zip`）。反证：把不可达分支改回 `return Err` → 「daemon 不可达也要出包」那条立刻变红。 |
