...
| **BLOB-01（L2）ingest 收件仓不再留双份** | 2026-09-10 | `b683e04` + 真机 | ✅ 真机通过，归档 | 同一 Android 设备重新配对后连续传 21 张：`originals` **292 KiB → 45,520 KiB**，`.ppf/blobs/data` **0 KiB → 0 KiB**，`.ppf/blobs` 总量稳定 **1,032 KiB**（空 redb store 固定开销）。新照片只落 originals，没有隐藏第二份 payload。 |
| **BLOB-02（L2）flow-blobs 周期 GC** | 2026-09-10 | `c6c0bb6` + 真机 | ✅ 真机通过，归档 | 自动 2 秒采样捕获 Flow 总仓 **564 KiB → 47,788 KiB 峰值 → 492 KiB**；payload `.data` **452 KiB → 47,468 KiB → 0 KiB**。完成后等待 70 秒（GC 周期 60 秒），44/44 completed；只余 19 个 `.obao4` outboard 元数据（136,832 B），无照片内容。 |
| **BLOB-03（L2）Android 发送端 provider store 回收** | 2026-09-10 | `563a4bc` + `793ee13` + 三星验收 | ✅ 真机通过，归档 | 根因不是「没接 GC」，而是导入走 `AddProgress::with_tag()`（落持久 named tag，GC 永不回收）。修复：`with_config` 用 `FsStore::load_with_opts` 打开 60s 周期 GC；`register_path_async`/`register_file_async` 改 `.temp_tag()`（ephemeral），`ActiveProvider.retained: Option<TempTag>` 在 `activate` 写入、`revoke()` 释放；新增 `release_retention()` 只掉 lease 的 tag、保住 endpoint/ALPN 复连（决策 5）。Kotlin 成功边界在 `NativeFlowDeliveryPort.acceptReceipt` 四字段校验通过后、推进 strict head 前调 `bridge.releaseRetention`。自动化反证：Rust `android_provider` **5/5**（含旧 named tag 升级迁移回收），Android JVM 4/4 + 10/10。三星覆盖安装修复版后，原 7 个 `CONFIRMED` provider data 文件 **4,100,446 B → 0 B**，ledger 7 条确认不变；余项仅为下次自然新增来源的机会性回归。**最终真机验收**：验收人授权隔离 Screenshots 测试，正常 Flow 后 ledger **7→9**，9 项均 `CONFIRMED` 且有 receipt；receipt 后与 75 秒后 provider `data` 都为 **0 文件**，三张测试截图已清理，归档。 |
| **REBUILD-07（L2）同一设备重配对 receipt 隔离** | 2026-09-10 | `b683e04` + 真机 | ✅ 真机通过，归档 | 现场旧 schema v3 升 v4，旧 24 条 completed receipt 保留；移除同一 Android 安装后重扫，新 epoch sequence **1–21** 全部 completed、无 `err.not_authorized`。稳定 NodeId 保持历史照片归属，epoch 隔离临时 Flow 序号。 |
...
| **CI-02（L3）e2e nightly release 二进制只编一次** | 2026-09-09 | `f7bb429` | ✅ 完成 | `e2e.yml` 新增 `build` job（`cargo build --release -p daemon -p testclient` 只跑一次，之前 `e2e`/`scenarios` 两个并行 job 各编一遍，同 run 内 cache 互相看不见），打包 `release-bins.tar.gz`（tar 保留可执行位，规避 upload-artifact zip 打包丢 +x 的已知坑），两个测试 job 各自 `needs: build` + `download-artifact` + `tar -xzf` 解包并 `test -x` 断言。`e2e`/`scenarios` 仍是两个独立 job，失败隔离不变（未合并成一个 job）。`actionlint` 零告警。实跑 `workflow_dispatch`（[run 34317704548](https://github.com/hawkeye-xb/P-Pass/actions/runs/34317704548)）：三 job 全部 `success`，两个下游 job 的 unpack 步骤均确认可执行位存活。 |
...
| **MOB-62（L2）断开重扫清空旧 Flow 会话** | 2026-09-08 | `11a2885` / 本 commit | 🟡 本轮三星验证通过，待共享回归 | 三星实证主动断开→重扫会 ANR；清理路径原先只删 pairing/confirmed cache 和一个 periodic work，遗留 `flow-state/<NodeId>`、native provider/runtime 及 catchup/manual/process/media-watch wakes，重扫同 NodeId 时旧 offer 重放。现已关闭/移除 runtime、删旧 Flow ledger、取消全部 wake 与 media watch，并把清理移到 IO；Android JVM + debug APK、`just ci` 通过。本轮 Samsung SM-S9210 已完成主动断开→新单次配对串→daemon pending 立即允许→选相册，无 ANR、崩溃或旧 offer UI；未点开始备份，统一回归补首轮传输。 |
...
| **批次 A（REL-03 / I18N-01 / DESK-09 / BUILD-02）** | 2026-09-01 | `43f4225` | 🟡 自动 CI 成功；仅欠 2 项真机/真环境验收 | 并行 worktree 完成四张独立卡后统一合入：REL-03 给每个版本目标独立读取/断言并验证 lock；BUILD-02 让 5 个 workflow 的 13 个 Cargo job 从 `rust-toolchain.toml` 派生工具链且有 TOML 反证；I18N-01 将空相册名交给资源渲染，英文真机验收欠账；DESK-09 透出 daemon stderr 并为迁移不兼容给出升级提示，旧 daemon/新库真机验收欠账。批次本地验证：`just ci`、版本脚本正反例、工具链脚本反证、Android JVM **385 tests / 0 failures / 4 skipped**、desktop vitest 26、vite build、desktop lib 16 均通过；用户确认 Actions 成功。 |
| **ARCH-09（L2）P1 对账分页协调与源存在性裁决接线** | 2026-09-01 | `af6f5cf` | ✅ 代码完成（Android JVM **381 tests / 0 failures / 4 skipped**；`just ci` + CI Android #104 全绿） | `ReconciliationCoordinator` 只取当前 epoch confirmed hash 的 ≤500 项，remote present 零 source probe；remote missing 才调用独立 source probe，ARCH-07 落 `NEEDS_DECISION` / `UNRECOVERABLE`，不改队列状态。`ContentResolverSourcePresenceProbe` 仅开关流不读内容；反证让 present 项 probe 源时 R-01 实际变红。 |
| **ARCH-08（L2）P1 Desktop 存在性探测与分页协议** | 2026-09-01 | `9d5337e` | ✅ 代码完成（Rust/Android 合同通过；`just ci` + CI Rust #71 + Dogfood #204 全绿） | 新增 side-effect-free `backup.presence` / `BackupPresenceQuery`，≤500 个 hash 只查索引、返回缺失集合；Android `RemotePresenceProbe` 校验页面并通过 `DaemonClient` 解码结果。反证实际变红：临时委托旧 `backup.manifest` 后错误创建 session，测试报 `left: 1`, expected `0`；已还原。 |
| **ARCH-07（L2）P1 远端对账事实与恢复裁决** | 2026-09-01 | `12f2f63` | ✅ 代码完成（R-01/R-02；Android JVM **377 tests / 0 failures / 4 skipped**；`just ci` + CI Android #100 全绿） | `TransferItem` / `CompletionReceipt` 新增持久内容身份与远端/源存在性、恢复裁决；`RemoteReconciliation` 对当前 epoch 的已确认项只写事实：源仍在 → `NEEDS_DECISION`，两端缺失 → `UNRECOVERABLE`，远端仍在则无恢复裁决。不会自动重新入队、fetch、递增失败计数、改 UploadCursor 或删除源。新合同 3/3；反证实际变红：临时重新入队→R-01/R-02 两条失败，源缺失误裁可恢复→R-02 失败，均已还原。旧批次校准、WorkManager、UI、proto、Rust/desktop 零改动。 |
